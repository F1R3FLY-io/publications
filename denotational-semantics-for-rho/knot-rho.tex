...
\newcommand{\Tab}{\mathsf{T}}                  % table / RSpace functor
\newcommand{\bag}[1]{\{\!|\,#1\,|\!\}}         % multiset braces
\newcommand{\Mfin}{\mathcal{M}_{\mathrm{fin}}} % finite multiset functor
\newcommand{\uplusk}{\mathbin{\uplus}}         % keyed bag union
\newcommand{\resbis}{\sim_{\mathsf{r}}}        % resource bisimilarity
\newcommand{\supmap}{\mathsf{supp}}            % support nat transf (overloaded name avoided in prose)
...
swap, not an axiom about the model. We give the dynamics as an RSpace semantics:
parallel composition is interpreted as keyed multiset union, so the denotation is a
monoid homomorphism and the parallel laws of structural congruence become literal
equalities of tables --- hashtables from channels to polarity-homogeneous bags of
produces and consumes, which is exactly the RSpace store. Communication is eager
reaction, the zip of dual bags at a key, and it is here, in the zip, that the swap
fires, sealing each sent process into the received name; RSpace is the store monad
together with this zip, a bialgebra --- the precise content of ``strictly more than
a monad''. Following the GSLT discipline we label transitions not by primitive
actions but by \emph{contexts} --- the minimal store-insertions enabling a
reaction, after Leifer and Milner --- and express bisimulation over these context
labels; the labels are the program/environment cuts, simultaneously the indices of
the OSLF-generated modal logic. The model factors: a resource layer that counts
parallel copies, projecting (forget multiplicity and keys) onto the
context-behaviour layer. We prove a \emph{full abstraction} theorem at each ---
$\sem P\resbis\sem Q$ iff $P\resbis Q$ above, $\sem P_{\B}=\sem Q_{\B}$ iff $P\bis
Q$ below --- by identifying bisimilarity with the kernel of the unique morphism to
the final coalgebra; because the labels are contexts, the behavioural equivalence
is a congruence by
...
\paragraph{The dynamics are an RSpace semantics (Section~\ref{sec:denotation}).}
Modulo structural congruence the parallel fragment is the free commutative monoid,
hence the finite-multiset construction; interpreting $\Par$ as multiset union makes
the denotation a monoid homomorphism on the nose, so the parallel laws of $\scong$
become literal equalities. Bucketing the multiset by channel yields a hashtable
from names to polarity-homogeneous bags of produces and consumes --- precisely the
RSpace store. Following Milner's abstraction/concretion reading \cite{polyadic},
inputs install consume bags of abstractions $\abs y P$ and outputs produce bags of
data $Q$; communication is \emph{eager} reaction --- when a key carries both
polarities, zip the bags and fire each matched pair by application
$\abs y P\cdot Q=P\{\quo Q/y\}$, the surplus remaining --- and it is in this zip
that the swap of Section~\ref{sec:swap} fires, sealing the sent process into the
received name. RSpace is the store monad together with the zip: a bialgebra, which
is the precise sense in which it is ``more than a monad''. The model factors ---
the table projects, forgetting multiplicity and keys, onto the context-behaviour
universe of the previous sections --- so the resource-sensitive RSpace layer sits
above the bisimulation layer and maps onto it.
...
\section{The RSpace denotation: parallel composition as keyed multiset union}\label{sec:denotation}

The commitment reading of the previous functor invites a sharper denotation. Modulo
$\scong$ the parallel fragment $(\Par,\Zero)$ is the free commutative monoid, and
the free commutative monoid is the finite-multiset construction. Interpreting
$\Par$ as multiset union therefore makes $\sem{-}$ a monoid homomorphism
\emph{on the nose}, and structural congruence of the parallel skeleton becomes
literal equality of multisets --- the tightest fit syntax and semantics can have.
Bucketing the multiset by channel turns it into a hashtable; this is exactly the
RSpace store, and the denotation below is, term for term, the compilation of rho
into RSpace states.

\subsection{The table functor}

\begin{definition}[RSpace tables]\label{def:table}
A \emph{table} is a finite partial map from names to \emph{polarity-homogeneous}
multisets of payloads,
\[
  \Tab(R) \;=\; \Big(\Name \rightharpoonup
     \big(\,\Mfin(R)\ \uplus\ \Mfin(\nset R)\,\big)\Big),
\]
where each key lands in exactly one summand: a key holds either a multiset of
\emph{output payloads} $x\mapsto\bag{Q_1,\dots,Q_m}^{+}$ (produces, data) or a
multiset of \emph{input continuations} $x\mapsto\bag{\abs{y_1}{P_1},\dots,
\abs{y_n}{P_n}}^{-}$ (consumes, abstractions), never a mixture. $\Mfin$ is the
finite-multiset (free commutative monoid) functor; $\nset R$ is the
name-abstraction of \cite{knot}, \S6.2. $\Tab$ acts on the knotted nominal
universe $\mathcal U$, the keys being names (red atoms) and hence, by
Section~\ref{sec:swap}, quoted processes --- content-addressed channels.
\end{definition}

\begin{definition}[The RSpace denotation]\label{def:rspace}
Define $\sem{-}\colon\Proc\to\Tab(\Proc)$ by
\[
  \sem{\Zero}=\varnothing,\qquad
  \sem{\out x Q}=\big\{\,\sem{x}\mapsto\bag{\sem Q}^{+}\,\big\},\qquad
  \sem{\inp y x P}=\big\{\,\sem{x}\mapsto\bag{\abs y{\sem P}}^{-}\,\big\},
\]
\[
  \sem{P\Par Q}=\sem P \uplusk \sem Q,\qquad
  \sem{\drp x}=\drp{\sem x}=\swp(\kt^{-1}\sem x),\qquad
  \sem{\quo P}=\quo{\sem P}=\kt(\swp\,\sem P),
\]
where $\uplusk$ is \emph{key-wise} multiset union: $(\sem P\uplusk\sem Q)(x)$ is the
multiset union of the bags $P$ and $Q$ install at $x$. Each prefix installs a
singleton bag; $\Par$ co-locates produces and consumes in the store. A key whose
two operands carry the same polarity stays homogeneous; a key receiving both
polarities is a \emph{redex} (Definition~\ref{def:eager}).
\end{definition}

\begin{proposition}[Exact homomorphism]\label{prop:hom}
$\sem{-}$ is a homomorphism of commutative monoids: $\sem\Zero=\varnothing$ and
$\sem{P\Par Q}=\sem P\uplusk\sem Q$, with $\uplusk$ associative and commutative and
$\varnothing$ its unit. Hence the parallel laws of $\scong$ ---
associativity, commutativity, unit --- hold as literal equalities of tables, and
$P\scong Q\Rightarrow\sem P=\sem Q$. With the reflection law of
Proposition~\ref{prop:reflection}, every generator of $\scong$ is validated.
\end{proposition}
\begin{proof}[Proof sketch]
$\uplusk$ inherits the commutative-monoid laws of multiset union key-wise; the empty
table is the unit. The prefix clauses are singletons, so reassociating or
commuting a parallel composition reassociates or commutes the corresponding bag
unions, which multiset equality absorbs. This is the gain over the corecursive
$\rpar$ of the set model: multiset union has no idempotence to defeat, so
$\out a 0\Par\out a 0$ installs $\bag{\sem{0},\sem{0}}^{+}$ at $a$, a two-element
bag, distinct from the one-element bag of $\out a 0$ --- counts are kept, and the
operation is union outright rather than a context-carrying expansion.
\end{proof}

\subsection{Eager reaction is the zip, and the swap fires}

\begin{definition}[Eager reaction]\label{def:eager}
A key $x$ carrying both a produce-bag $B^{+}=\bag{Q_1,\dots,Q_m}^{+}$ and a
consume-bag $B^{-}=\bag{\abs{y_1}{P_1},\dots,\abs{y_n}{P_n}}^{-}$ is a redex.
Eager reaction fires it: choose underlying lists of $B^{+}$ and $B^{-}$ and zip
them; each matched pair reacts by application,
\[
  \abs{y_i}{P_i}\cdot Q_i \;=\; P_i\{\,\quo{Q_i}\,/\,y_i\,\}
                          \;=\; P_i\{\,\kt(\swp\,Q_i)\,/\,y_i\,\},
\]
and the resulting process is merged, by $\uplusk$, back into the table; the
$\lvert m-n\rvert$ unmatched payloads of the longer bag remain at $x$, now a
homogeneous surplus (and $x$ is dropped from the table when $m=n$). Eager reaction
fires every redex, including those created by merged reaction products; its
\emph{normal forms} are the tables with no mixed-polarity key. The seal
$\quo{\,}=\kt\circ\swp$ acts in each application: \emph{communication is where the
colour-swap of Section~\ref{sec:swap} fires}, delivering the name $\quo{Q_i}$ of
the sent process, not the process.
\end{definition}

\begin{remark}[The pairing is COMM's nondeterminism, located]\label{rem:pairing}
Which $Q_i$ meets which $\abs{y_j}{P_j}$, and which payloads survive as surplus,
depend on the chosen list orderings: this is precisely rho's choice of which send
meets which receive, now localised at the key. Eager reaction is therefore a
\emph{relation} on tables, confluent only up to the equivalence that forgets the
pairing --- resource bisimilarity (Section~\ref{sec:fa}). The denotation $\sem{-}$
remains a function (plain $\uplusk$, no firing); reaction is the separate dynamics.
The minimal context $\hole\Par\out x Q$ of Definition~\ref{def:ctx-lts} is exactly
the minimal store-insertion --- one produce at key $x$ --- that turns a pending
consume into a redex; context labels are RSpace puts.
\end{remark}

\subsection{RSpace as a bialgebra, and the factoring}

\begin{definition}[The RSpace bialgebra]\label{def:bialg}
The table functor $\Tab$ carries a monad structure: $\Mfin$ is the free
commutative-monoid monad, and the key-wise lift $\Name\rightharpoonup\Mfin(-)$ is
the store, with multiplication $\uplusk$. RSpace is this monad \emph{together with}
the eager reaction of Definition~\ref{def:eager}, presented as a distributive law
of the reaction over the store --- a bialgebra in the sense of Turi--Plotkin
\cite{turiplotkin}. This makes precise the standing description of RSpace as
``strictly more than a monad'': the monad is the store ($\Mfin$-valued map,
for-comprehension as the consume bag), and the surplus is the distributive law ---
the zip, with send-and-substitute $P_i\{\quo{Q_i}/y_i\}$ as its witness.
\end{definition}

\begin{proposition}[The factoring onto $\nu\B$]\label{prop:factor}
Let $\supmap\colon\Mfin\Rightarrow\mathcal P$ be the support transformation
(forget multiplicities) and let $\mathsf{flat}$ flatten a table into its set of
context-labelled transitions (a consume at $x$ is an offer-input transition, a
produce an offer-output, a mixed key an internal redex). The composite
$\pi=\mathsf{flat}\circ\supmap\colon\Tab(R)\to\B(R)$ is a natural transformation,
and it carries the RSpace dynamics to the context-labelled system of
Section~\ref{sec:lts}. Hence the RSpace table model sits \emph{above} the
behaviour model and projects onto it:
\[
  \text{Syntax} \xrightarrow{\ \sem{-}\ } \Tab\text{-tables (RSpace)}
                \xrightarrow{\ \pi\ } \nu\B ,
\]
and the composite is the context-behaviour denotation of
Section~\ref{sec:functor}. Resource bisimilarity (Definition~\ref{def:resbis})
refines context bisimilarity, with $\pi$ the quotient that forgets multiplicity and
keys.
\end{proposition}

The reflection law is undisturbed by the refinement --- it concerns names, not the
store monoid --- so Theorem~\ref{thm:adequacy} below holds verbatim, now with the
homomorphism into tables.
...
\begin{enumerate}[label=\textup{(\roman*)},nosep]
  \item $\sem{\quo P}=\quo{\sem P}$, $\sem{\drp x}=\drp x$, and
        $\sem{\drp{\quo P}}=\sem P$: the reflection law holds in the model, as
        $\swp\circ\swp=\mathrm{id}$ (Proposition~\ref{prop:reflection});
  \item $\sem{-}$ is a monoid homomorphism into tables
        (Proposition~\ref{prop:hom}); in particular $P\scong Q\Rightarrow\sem P=\sem Q$,
        the parallel-monoid laws holding as literal table equalities and the
        reflection law as the involutivity of $\swp$.
\end{enumerate}
\end{theorem}

\section{Full abstraction}\label{sec:fa}

The factoring of Proposition~\ref{prop:factor} gives two full-abstraction
statements, one at each layer. At the \emph{resource} layer the model counts
parallel copies, so the equivalence it realises is resource bisimilarity; at the
\emph{behavioural} layer, reached by the projection $\pi$ that forgets multiplicity
and keys, it is the context bisimilarity of Section~\ref{sec:lts}. We give the
resource statement first, then recover the behavioural one as its image under
$\pi$.

\subsection{The resource layer}

\begin{definition}[Resource bisimilarity]\label{def:resbis}
A symmetric relation $\mathcal R$ on tables is a \emph{resource bisimulation} if
related tables carry, key by key, multisets matched \emph{with multiplicity}: equal
counts of produces and of consumes at each channel, with $\mathcal R$-related
payloads and (after the eager zip of Definition~\ref{def:eager}) $\mathcal
R$-related residuals. Resource bisimilarity $\resbis$ is the largest such relation;
on processes, $P\resbis Q$ means $\sem P\mathrel{\resbis}\sem Q$. It counts copies:
$\out a0\Par\out a0\not\resbis\out a0$.
\end{definition}

\begin{lemma}[The store functor is well behaved]\label{lem:bag-wp}
The finite-multiset functor $\Mfin$ preserves weak pullbacks, because $(\mathbb N,+)$
is a refinement monoid (the Riesz interpolation property) \cite{gummschroder};
key-wise lifting, products, coproducts and the abstraction functor preserve weak
pullbacks too, so the table functor $\Tab$ preserves them. Hence on any
$\Tab$-coalgebra, resource bisimilarity is the kernel of the unique morphism into
the final table-coalgebra $\nu\Tab$.
\end{lemma}

\begin{theorem}[Resource full abstraction]\label{thm:res-fa}
For all rho processes $P,Q$,
\[
  \sem P \resbis \sem Q \quad\Longleftrightarrow\quad P\resbis Q .
\]
Equivalently, $\sem{-}$ followed by the unique morphism into $\nu\Tab$ identifies
exactly the resource-bisimilar processes. Replication forces a multiplicity of
$\omega$ ($!P\scong{!P}\Par P$ installs $\omega$ copies), so $\nu\Tab$ is taken with
multiplicities in $\mathbb N\cup\{\omega\}$ --- and that $\omega$ is the same one
the risk ledger (Section~\ref{sec:ledger}) charges as black infinity.
\end{theorem}
\begin{proof}[Proof sketch]
$\sem{-}$ is a homomorphism into tables (Proposition~\ref{prop:hom}); equipping
$\mathcal T/{\scong}$ with the eager dynamics makes it a $\Tab$-coalgebra, and the
unique morphism into $\nu\Tab$ exists by Lemma~\ref{lem:bag-wp}. Its kernel is
resource bisimilarity by the same lemma; that the syntactic resource bisimilarity
$\resbis$ matches the coalgebraic one is the multiset analogue of
Lemma~\ref{lem:oc}, the offer-input and offer-output families now compared with
multiplicity. The $\omega$-multiplicity refinement is the only point where
infinitude enters, and it enters exactly once.
\end{proof}

\subsection{The behavioural layer, by projection}

Applying the projection $\pi$ of Proposition~\ref{prop:factor} --- forget
multiplicity ($\Mfin\Rightarrow\mathcal P$) and keys --- carries the resource model
onto the context-behaviour model, and resource bisimilarity onto context
bisimilarity. The remaining lemmas establish the behavioural statement directly,
over the commitment refinement $\B^{\flat}=\pi\circ\Tab$.
...
For all rho processes $P,Q$, writing $\sem{-}_{\B}=\pi\circ\sem{-}$ for the
behavioural denotation (the table projected to $\nu{\B^{\flat}}$),
\[
  \sem P_{\B}=\sem Q_{\B} \quad\Longleftrightarrow\quad P\bis Q .
\]
Moreover, since $\bis$ is a congruence (Proposition~\ref{prop:cong}), this is full
abstraction with respect to a congruence: the behavioural denotation identifies $P$
and $Q$ exactly when no context can tell them apart. It is the image under $\pi$ of
the resource statement (Theorem~\ref{thm:res-fa}): $P\resbis Q$ implies $P\bis Q$,
the converse failing exactly where multiplicities differ.
\end{theorem}
\begin{proof}
By Proposition~\ref{prop:factor}, $\sem{-}_{\B}=\pi\circ\sem{-}$ is the unique
$\B^{\flat}$-coalgebra morphism $(\mathcal T/{\scong},\partial)\to\nu{\B^{\flat}}$,
so $\sem P_{\B}=\sem Q_{\B}$ iff $P,Q$ have the same image. By Lemma~\ref{lem:wp}
that image-equality is coalgebraic bisimilarity, and by Lemma~\ref{lem:oc} that is
context bisimilarity $\bis$. Chaining, $\sem P_{\B}=\sem Q_{\B}\iff P\bis Q$.
Congruence is Proposition~\ref{prop:cong}; the projection $\pi$ collapsing
multiplicity gives $P\resbis Q\Rightarrow P\bis Q$.
\end{proof}
...
\]
Behavioural equality in the model, context bisimilarity, and modal
indistinguishability are one relation seen three ways --- the Firing, Placing, and
Witnessing readings of the single label set $\DK$ --- with resource bisimilarity
$\resbis$ the finer relation living one layer up.
\end{corollary}
...
sets built from $\emptyset$. The RSpace layer sharpens the accounting: replication
$!P\scong{!P}\Par P$ installs an $\omega$-fold bag at a key, so the same single
$\omega$ appears as a multiplicity in $\mathbb N\cup\{\omega\}$
(Theorem~\ref{thm:res-fa}). Infinitude shows up once, in the count, and it is black
infinity.
...
\begin{obligation}[The RSpace bialgebra: eager confluence and store coherence]\label{ob:rspace}
Make the RSpace layer of Section~\ref{sec:denotation} rigorous. (i) Prove eager
reaction (Definition~\ref{def:eager}) confluent up to resource bisimilarity: the
pairing nondeterminism of Remark~\ref{rem:pairing} affects which payloads meet, but
all maximal reaction sequences reach $\resbis$-equal normal forms. (ii) Establish
the distributive law of Definition~\ref{def:bialg} and its bialgebra laws, so
``RSpace $=$ store monad $+$ zip'' is a theorem in the Turi--Plotkin format
\cite{turiplotkin}, with $\sem{-}$ the induced bialgebra morphism. (iii) Confirm
$\Tab$ preserves weak pullbacks in the \emph{nominal} and non-well-founded setting
(Lemma~\ref{lem:bag-wp} gives it over $\mathbb N$ by refinement \cite{gummschroder};
the $\mathbb N\cup\{\omega\}$ completion forced by replication must be checked to
retain the refinement property).
\end{obligation}

\begin{obligation}[Non-well-founded keyed multisets in the knotted universe]\label{ob:nwfbag}
Encode tables inside the knotted universe so that recursion is accommodated: a
table is a red set of triples $\langle$key, count, payload$\rangle$ with black-set
keys (red atoms) and red-numeral counts, so a non-well-founded bag is a
non-well-founded set of triples --- the non-well-foundedness living in the payload,
the multiplicity remaining a numeral. Verify that the swap $\swp$ and the knots
$\kt,\ktb$ respect the count arithmetic, and that multiplicity-$\omega$ is exactly
the black infinity of Section~\ref{sec:ledger}.
\end{obligation}
...
Definition~\ref{def:eager} computes it, with the seal $\kt\circ\swp$ applied to the
message. This is the lemma underwriting the eager zip and clause (ii) of
...
law is the involutivity $\swp\circ\swp=\mathrm{id}$, true by construction. Parallel
composition is interpreted as keyed multiset union, which makes the denotation a
monoid homomorphism on the nose --- the parallel laws of structural congruence
become literal equalities of tables --- and the table \emph{is} the RSpace store:
a hashtable from channels to polarity-homogeneous bags of produces and consumes.
Communication is eager reaction, the zip of dual bags at a key, with the
colour-swap firing to seal each sent process into the received name; RSpace is the
store monad together with this zip, a bialgebra, which is the precise content of
``strictly more than a monad''. Following the GSLT discipline we labelled
transitions by contexts --- the minimal store-insertions that enable a reaction,
which are at once the program/environment cuts, the OSLF modality indices, and the
locations in term structure --- and expressed bisimulation over them. The model
factors: the resource layer counts copies and realises resource bisimilarity; the
projection that forgets multiplicity and keys lands on the context-behaviour layer,
where we proved full abstraction, $\sem P_{\B}=\sem Q_{\B}\iff P\bis Q$, for a
congruence with no closure step. The risk ledger is the source paper's: finite
processes cost nothing beyond the empty set, recursion costs one $\omega$ --- now
visible as an $\omega$-fold multiplicity --- and not one primitive name is ever
spent. Aczel's model sits inside this one as its equivariant shadow; the surplus is
reflection, and reflection is the swap.

\paragraph{Future work.} Three threads we have deliberately set aside. The reaction
strategy here is \emph{eager}, with tables reduced to polarity-homogeneous normal
forms; the \emph{lazy} reading, treating a mixed-polarity key as a standing redex
and reacting on schedule, is closer to the coalgebraic machinery and deserves its
own treatment. The polarity-homogeneity invariant of reduced tables --- a key holds
produces or consumes, never both --- carries a proof-theoretic reading we leave for
a separate study. And the two involutions now in play, the colour-swap on names and
the polarity duality on channels, may or may not coincide; we have not pursued the
question here.
...
\bibitem{gummschroder} H. Peter Gumm and Tobias Schr\"oder. Monoid-labeled
transition systems. \emph{Electron.\ Notes Theor.\ Comput.\ Sci.}, 44(1):185--204,
2001.
