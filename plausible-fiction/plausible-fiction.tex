...
% MeTTaIL Theory / Module definitions (syntax of 2026-08-19)
\lstdefinelanguage{mettail}{
  morekeywords={Module,Theory,theory,import,from,as,Types,Exports,Replacements,Terms,Equations,Rewrites,Empty,free,let,in,if,then,subst,fold,relation},
  morekeywords=[2]{Term,HoleId,MetaId,Tag,Lvl,VariantAlt,CaseBranch,Price,Obsv,Proc,Name,HashBag,HashSet,List,Map,Ctx},
  sensitive=true,
  morecomment=[l]{//},
  morestring=[b]",
...
\lstinline[language=mettail,basicstyle=\ttfamily\small]|Theory| declaration, whose terms ride on
...
\textbf{Revision note (26 August 2026).} This revision re-expresses the object language in the
MeTTaIL surface frozen by the DDL and module decisions of 19 August 2026. The
\lstinline[language=mettail,basicstyle=\ttfamily\small]|language!| form is withdrawn; every language
is a \texttt{Theory}. Substantively: \texttt{PFLam} is now an escape-free theory (\S\ref{sec:theorydecl});
\texttt{grounded}, \texttt{holes} and \texttt{plug} are host operations at the bridge rather than term
formers carrying native folds, and the bidirectional checker is stated beside the theory
(\S\ref{sec:whatmoved}); the eta side condition and variant-arm selection are now expressible in the
theory itself, so the \texttt{selectArm} helper is gone; and the \texttt{case} motive acquires
concrete syntax. The design is unchanged. Two open questions the rewrite generated for MeTTaIL are
recorded in \S\ref{sec:open}.
\end{minipage}
\end{center}
\vspace{0.6em}
...
object language is given as a \lstinline[language=mettail,basicstyle=\ttfamily\small]|Theory|
declaration, and every coordination behaviour is given as embedded rholang. It is deliberately
...
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
  ...
  $\lambda$-calculus with typed holes, declared as a MeTTaIL
  \lstinline[language=mettail,basicstyle=\ttfamily\small]|Theory|. Pure and total: the syntax,
  the conversion theory, and directed reduction. Bidirectional typing, $\Holes(\cdot)$,
  $\grounded(\cdot)$, and $\plug$ are \emph{host} operations over that syntax and are stated
  with the bridge (\S\ref{sec:bridge}), not inside the theory.
...
\begin{tikzpicture}[font=\small,node distance=6mm]
  ...
  \node[layer,below=of coord] (bridge) {\textbf{Bridge — rho-callable folds (host operations)}\\[2pt]
    \texttt{pf |= ty}\quad\texttt{grounded(pf)}\quad\texttt{plug(pf,h,q)}\quad\texttt{holes(pf)}\quad\texttt{resolve(pf,s)}\\[1pt]
    {\scriptsize bidirectional checker (ascent) sits here, beside the theory}};
  \node[layer,below=of bridge] (obj) {\textbf{Object language \texttt{PFLam} — \texttt{Theory PFLam()}}\\[2pt]
    dependent $\Pi/\Sigma/\mathsf{Type}$ $\cdot$ finite variants + \texttt{case} $\cdot$ typed holes
    $\cdot$ conversion ($\beta\eta$, projections, variant-$\beta$) $\cdot$ directed reduction};
...
\subsection{The \texttt{Theory} declaration}\label{sec:theorydecl}
Below is \texttt{PFLam} in the MeTTaIL surface frozen on 2026-08-19: one declaration form
(\texttt{Theory}), capitalised builders, judgement-form term rules
(\lstinline[language=mettail,basicstyle=\ttfamily\small]!Label . ctx |- syntax : Cat!), \texttt{==}
for equations, and mandatory labels on rewrites. Three things that the withdrawn block carried
\emph{inside} the language definition are no longer there, and \S\ref{sec:whatmoved} says where they
went and why the move is an improvement rather than a concession.
...
Theory PFLam() {
    ...
    Types {
        ...
        HoleId;        // agent-hole identifiers (market-visible)
        MetaId;        // computation-hole / elaboration metavariables
        Tag;           // variant labels (outcome constructors)
        Lvl;           // universe levels
        VariantAlt;    // one  (Tag : Term)  alternative
        CaseBranch;    // one  (Tag x => body)  arm
    }

    Terms {
        // ---- core dependent calculus -------------------------------------
        // No `Var` rule: variables arise only from binders, which are the
        // abstraction sort `^x.body:[Term -> Term]` (G4).
        ...
        // A finite labelled sum  < l_1 : A_1 | ... | l_n : A_n >  (Spivak's
        // O_venue). The alternatives are a collection sort (G2), rendered
        // inline with a separator projection rather than a grammar pragma.
        Variant . alts:HashBag(VariantAlt)
                |- "<" alts.*sep("|") ">"                        : Term;
        Alt     . lbl:Tag, ty:Term        |- lbl ":" ty           : VariantAlt;
        Inj     . lbl:Tag, val:Term       |- "inj" lbl val        : Term;

        // Dependent eliminator: the motive is now WRITTEN, not merely passed.
        // Every argument in the context must appear exactly once in the
        // concrete syntax (G6), which the old positional form let us skip.
        Case    . scrut:Term, mot:Term, brs:HashBag(CaseBranch)
                |- "case" scrut "return" mot "of" "{" brs.*sep(";") "}"
                                                                 : Term;
        ...
        // ty is the Pi-closed interface  Pi Gamma_h . T_h  (sec. 4.1).
        Hole . h:HoleId, ty:Term          |- "?" h ":" ty         : Term;

        // Observation hole (outcome-resolved): an agent hole whose value is
        // revealed by RUNNING a step (attestation), not by a market plug. It
        // is a legitimate case-scrutinee and a CDT branch point (sec. 4.4).
        Obs  . h:HoleId, ty:Term          |- "observe" h ":" ty   : Term;

        // Computation hole: an elaboration metavariable, solved by
        // unification. NEVER advertised to the market.
        Meta . m:MetaId, ty:Term          |- "%" m ":" ty         : Term;
    }

    Equations {
        // beta / eta / projections / variant-beta are CONVERSION: the
        // equational theory used by type conversion and by the matching
        // test |=. Equations are unlabelled; rewrites below carry labels.
        (App (Lam a ^x.b) c) == (subst ^x.b c);

        // eta, with its side condition now EXPRESSIBLE. The withdrawn block
        // could only write "x not free in f" as a comment.
        if x # f then (Lam a ^x.(App f x)) == f;

        (Fst (Pair a b)) == a;
        (Snd (Pair a b)) == b;

        // variant beta. Selecting the matching arm out of the branch bag is
        // a remainder pattern (G3) with a repeated label variable `l` -- the
        // same shape as rho's COMM rule, which repeats the channel `n`.
        // No `selectArm` helper is needed: the pattern IS the selection.
        (Case (Inj l a) mot { (CaseBr l ^x.body), ...rest })
            == (subst ^x.body a);
    }

    Rewrites {
        // Directed reduction = "filling computation holes by standard
        // computation". Labels are mandatory: an anonymous rule cannot be
        // subtracted by `\` or retargeted by `Replacements`.
        RBeta : (App (Lam a ^x.b) c) ~> (subst ^x.b c);
        RFst  : (Fst (Pair a b)) ~> a;
        RSnd  : (Snd (Pair a b)) ~> b;

        // CASE on a KNOWN injection reduces (computation). On a NEUTRAL
        // scrutinee -- one stuck on an unresolved Hole/Obs -- no rule
        // applies: that stuck term is a branch point of the demand tree
        // (sec. 4.4). Stuckness is now a property of the rule set rather
        // than a comment on the rule.
        RCase : (Case (Inj l a) mot { (CaseBr l ^x.body), ...rest })
                  ~> (subst ^x.body a);

        // congruences, as conditional rewrites
        RAppFun : if S ~> T then (App S c) ~> (App T c);
        RAppArg : if S ~> T then (App c S) ~> (App c T);
        RScrut  : if S ~> T then (Case S mot brs) ~> (Case T mot brs);
        RArm    : if S ~> T then (Case c mot { (CaseBr l ^x.S), ...rest })
                                   ~> (Case c mot { (CaseBr l ^x.T), ...rest });
        // RPiDom, RPiCod, RLamDom, RLamBody, RSigFst, RSigSnd,
        // RPairL, RPairR, RFstArg, RSndArg: elided, all of this shape.
    }
}
\end{lstlisting}

\subsection{What moved out of the language definition, and why}\label{sec:whatmoved}
Three elements of the withdrawn \lstinline[language=mettail,basicstyle=\ttfamily\small]|language!|
block have no counterpart in the \texttt{Theory} form. In each case the relocation is the one the
architecture (\S\ref{sec:arch}) already asked for.

\begin{description}[leftmargin=0pt,style=nextline,itemsep=4pt]
  \item[\texttt{grounded}, \texttt{holes}, \texttt{plug} were term formers carrying native folds.]
  They are now \emph{host operations} and appear only at the bridge (\S\ref{sec:bridge}). This is a
  correction, not a workaround. None of the three is a constructor of the calculus of meaning:
  $\grounded$ is a predicate \emph{about} a term, $\Holes(\cdot)$ is an analysis \emph{of} a term
  returning an object (the CDT) that is not a \texttt{Term} at all, and $\plug$ substitutes for a
  \emph{hole identifier} rather than for a bound variable --- it is what the market does to a term,
  not a reduction inside it. Having them as \texttt{Term}-sorted formers with folds \emph{and}
  again as \texttt{Proc}-sorted folds at the bridge was a duplication the old surface concealed.
  With them gone, \texttt{PFLam} is a pure, escape-free theory: syntax, conversion, reduction. The
  \rul{PlugRw} rule of the withdrawn block disappears with it.
  \item[The \texttt{logic \{ relation \dots \}} block.] The \texttt{Theory} form has builders for
  \texttt{Types}, \texttt{Exports}, \texttt{Replacements}, \texttt{Terms}, \texttt{Equations} and
  \texttt{Rewrites}, and no builder for a relational program. The bidirectional checker therefore
  rides \emph{beside} the theory rather than inside it (\S\ref{sec:typing}). Whether \texttt{Theory}
  should grow a \texttt{Logic} builder is an open question we do not prejudge here
  (\S\ref{sec:open}); the checker is a consumer of the elaborated presentation either way.
  \item[Ground categories written as native-type aliases.] The withdrawn block wrote
  \lstinline[language=mettail,basicstyle=\ttfamily\small]|![String] as HoleId| and
  \lstinline[language=mettail,basicstyle=\ttfamily\small]|![u64] as Lvl|. \texttt{Types} declares
  categories; how a category is pinned to a host ground type is exactly the question the DDL plan
  leaves open under the heading of BNFC's \texttt{token} pragma. We declare \texttt{HoleId},
  \texttt{MetaId}, \texttt{Tag} and \texttt{Lvl} as ordinary categories and flag their groundness in
  \S\ref{sec:open}; nothing in this design depends on which spelling wins.
\end{description}

\paragraph{Two requirements this refresh generates for the DDL.} Writing \texttt{PFLam} in the
\texttt{Theory} form is not a translation exercise; it is a load test, and it returns two findings.
\emph{(i)} The alternatives of a variant and the arms of a \texttt{case} are keyed by a
\texttt{Tag} and must be label-unique. \texttt{HashBag} is the only collection sort the frozen
surface exercises, and it is a bag: it admits duplicates. Either the sort vocabulary gains a keyed
sort (a set, or a map \texttt{Tag $\to$ Term}), or label-uniqueness is a side condition the
elaborator must check. \emph{(ii)} The motive of \texttt{Case} had no concrete syntax in the
withdrawn block, which the positional BNFC form tolerated. Under the rule that every argument in
the context is referenced exactly once, it must be written, so we write it
(\texttt{case $s$ return $M$ of \{\dots\}}). Both are consequences of the \texttt{Theory} form
catching what the record form let pass, which is the argument for the \texttt{Theory} form.
...
\paragraph{The checker, stated beside the theory.} Since \texttt{Theory} has no relational builder
(\S\ref{sec:whatmoved}), the judgments above are given as an \texttt{ascent} program over
\texttt{PFLam}'s elaborated presentation rather than as a block inside the declaration. The
interface is unchanged; only its residence is.

\begin{lstlisting}[language=mettail]
// Bidirectional checker for PFLam. Consumes the elaborated presentation;
// lives beside `Theory PFLam()`, not inside it.
relation check(Ctx, Term, Term);      // ctx |- term <= ty
relation synth(Ctx, Term, Term);      // ctx |- term => ty
relation sub(Term, Term);             // conversion + universe cumulativity
relation models(Term, Term);          // the matching test  q |= ty
...
telescope $\tele_h$ per \S\ref{sec:teleholes}). This is a host operation over \texttt{PFLam} syntax,
not a \texttt{PFLam} reduction: it substitutes for a \emph{hole identifier}, which the theory's
\lstinline[language=mettail,basicstyle=\ttfamily\small]|subst| --- substitution into an abstraction
$\hat{x}.p$ for a \emph{bound variable} --- cannot express (\S\ref{sec:whatmoved}). \emph{Crucially, $q$ may itself contain agent holes.}
...
if $h$ was a \emph{scrutinee} its plug fires \rul{RCase}, realizing a branch and pruning the others
      ...
      for( @pf <- @[u, "pf", pfId] ) {                          // read current term
        new pf2 in {
          ...
          for( @next <- pf2 ) {
            ...
            // if h was a scrutinee, plug fired RCase: the branch is now realized,
...
Grounding is now \emph{path-realized}, not set-empty. As branches resolve, \rul{RCase} prunes the
...
(\rul{RCase}) and any holes the filler itself introduces.}
...
//    Plugging hVenue with an Outdoor filler fires RCase -> the Outdoor branch is realized:
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \emph{The keyed-collection sort.} \texttt{VariantAlt} and \texttt{CaseBranch} collections
  must be label-unique, and the frozen surface offers only \texttt{HashBag}, which is not. Either
  the sort vocabulary grows a keyed sort or the elaborator carries the side condition. This is a
  MeTTaIL question, raised by \texttt{PFLam}, not a \texttt{PFLam} question.
  \item \emph{Ground categories.} \texttt{HoleId}, \texttt{MetaId}, \texttt{Tag} and \texttt{Lvl}
  are pinned to host ground types. How a \texttt{Types} declaration expresses that pinning is open
  (\S\ref{sec:whatmoved}).
  \item \emph{Whether \texttt{Theory} should carry a \texttt{Logic} builder.} The checker is stated
  beside the theory today. If generated logics are to be derived from a presentation uniformly, the
  relational program arguably belongs inside it; we take no position here.
...
vs.\ caveats) they consume. (7)~Settle the two surface questions this revision surfaced: the keyed
collection sort for label-indexed alternatives and arms, and the spelling by which a \texttt{Types}
declaration pins a category to a host ground type.
