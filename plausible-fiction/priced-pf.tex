...
\lstdefinestyle{rho}{
  ...
  morekeywords={contract,for,new,in,match,where,if,else,true,false,Nil,
    ...
    Module,Theory,theory,Types,Exports,Replacements,Terms,Equations,Rewrites,
    Empty,then,subst,fold,import,from,as},
  morecomment=[l]{//},
...
\textbf{Revision note (26 August 2026).} This revision re-expresses the object-language delta in the
MeTTaIL surface frozen by the DDL and module decisions of 19 August 2026. The
\texttt{language!} form is withdrawn; every language is a \texttt{Theory}. The delta of
\S\ref{sec:lang} is now a \emph{parametric} theory, \texttt{Theory PricedPFLam(pf: PFLam)}, so the
extension is checked against \PFLam{} rather than described alongside it; and \texttt{val} is stated
once, at the bridge, rather than twice. The design is unchanged.
\end{minipage}
\end{center}
\vspace{0.6em}
...
\node[box,below=of bridge] (lang) {\textbf{Object language} --- \texttt{Theory PricedPFLam(pf: PFLam)}\\
 dependent \(\Pi/\Sigma/\textsf{Type}\) \(\cdot\) finite variants \(+\) \texttt{case} \(\cdot\)
 typed holes \(\cdot\) \(\textbf{price annotations}\) \emph{(new)}\\
 {\scriptsize a parametric extension of \texttt{Theory PFLam()}; \(\holes\to\CDT\) and \(\val\) are host operations at the bridge}};
...
Under the MeTTaIL surface frozen on 2026-08-19 this delta is no longer a prose annotation on a
listing --- ``only additions shown'' --- but a declaration in its own right. The priced language is a
\emph{parametric theory} taking \PFLam{} as its parameter, and the additions are literally the
builders chained onto it. Everything proved about \PFLam{} arrives with the parameter; nothing is
restated, and nothing can silently diverge from the note it extends.

\begin{lstlisting}[style=rho]
...
\noindent Three points. First, \texttt{Priced} attaches a reservation price to a hole without
changing its \emph{type}: admissibility ($q \modelsT T$) is untouched, so all of \PF{}'s decidability
and termination results carry over verbatim. Second, \texttt{val} --- the PJES denotation of a term
as a demand price over its CDT --- is \emph{not} a constructor of this theory. It was written as a
term former carrying a native fold in the previous revision; it is a pure analysis over the term,
delivering a \texttt{Price} rather than a \texttt{Term}, so it belongs with the other host operations
at the bridge (\S\ref{sec:bridge}), where it was already stated. Removing the duplicate leaves
\texttt{PricedPFLam} escape-free, exactly as \PFLam{} now is. Third, the parametric form makes the
extension \emph{checkable}: if a future revision of \PFLam{} renames \texttt{Hole} or changes its
arity, the elaborator reports the break here rather than letting two listings drift apart.

\paragraph{A note on sharing.} \texttt{PricedPFLam} takes \PFLam{} as a parameter rather than
redeclaring any of its categories. That matters when a venue composes theories: a market that also
imports, say, a reputation theory over the same \texttt{HoleId} category joins the two over their
shared \PFLam{} instance, and the join is a pushout that identifies \texttt{HoleId} once. Passing
two independently instantiated copies of \PFLam{} would instead be rejected, and correctly so ---
it would assert that the priced holes and the reputed holes are different holes.
...
and where economic penalties attach on failed attestation. (7) Settle whether \texttt{Price} and
\texttt{Obsv} are ground categories pinned to host types or theory-defined ones --- the same
question the \PF{} note leaves open for \texttt{HoleId} and \texttt{Tag}, and the answer should be
uniform across both.
