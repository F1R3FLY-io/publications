...
\flang{} takes the desideratum LINQ identified and goes the rest of the way.
...
Here is the forager's query again, in \flang{}.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{SQL} & \textbf{\flang{}} & \textbf{What changed} \\
\midrule
...
A \texttt{SELECT} list picks columns. A \flang{} pattern matches structure: it
...
A query language has one privileged verb. \flang{} has two, and the second is the
...
from SQL. \flang{} distinguishes three ways of reading --- inheriting the
distinction, and the notation, from rholang --- and it is exactly the
distinction a database developer already makes between a destructive read, a
...
the main design decision in a \flang{} program and the choice is invisible in the
...
already there, and the join is a computation over them. A \flang{} join is over
...
When a reader and a writer meet on a channel, one rule fires. In the \flang{}
