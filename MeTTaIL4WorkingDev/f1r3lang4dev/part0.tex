...
The reason is that the control flow language of \flang{} has a familiar door and
...
answers. Taught the other way round, the \texttt{Theory} block looks like an
...
\flang{} adds; what that addition forces us to have; and then, twice, what
...
\subsection{Notation, and three conventions worth flagging}

The language is \flang{}, and it has two halves. Its \emph{control flow
sublanguage} --- CFL for short --- is the part that makes things happen: sends,
receipts, the comprehension of \S\ref{sec:cfl}. Its \emph{data definition
sublanguage} --- DDL --- is the part that says what there is: the
\texttt{Module} and \texttt{Theory} declarations of \S\ref{sec:ddl}. Both
acronyms are older than either language and are used here in their ordinary
sense. Neither half has a separate name. There is one language.

\flang{} inherits a good part of its CFL from \emph{rholang}, and this note
names the debt where it is useful --- the sends and receipts of \S\ref{sec:cfl}
will be familiar to anyone who has written rholang, and the translation table
of \S\ref{sec:table} would read much the same for either. The inheritance is
not an equivalence, and it is worth being precise about where it stops. The
\texttt{where} clause of \S\ref{sec:graded} answers with a value rather than a
verdict, and a program whose guards return values is not a program with better
guards --- it is a different kind of object, a simulation of a population
rather than a transition of a state. That single change puts \flang{} somewhere
rholang does not reach, and most of the second half of this note is an account
of where.

The calculus underneath both is the \rhoc{} calculus~\cite{MR05}. The name abbreviates
...
\texttt{f1r3node-rust} repository~\cite{f1r3node,modulesyntax}, where they
compile and are covered by tests.
