...
\item LINQ exists to embed it in a host language so that a developer can finally
  express the \texttt{DO}. \flang{} goes the rest of the way and builds the
...
\item That leaves the pattern and the condition unaccounted for, which forces a
  data definition sublanguage. \flang{}'s \texttt{Theory} declaration supplies
  it in four rungs: grammar, equations, rewrites, parameters. The second rung
  holds all the invariants your type system cannot read; the third makes the
  data wiggle; the fourth makes a language definition something you can ship,
  import by address, and extend without copying.

\item Because the fourth rung exists, a language definition composes like a
  module rather than like a file. Theories combine over shared ancestors, and
  the elaborator holds you to the claim: two things that arrived by different
  routes are two things, and if they share a name that is an error rather than
  an override.
...
\item Run the reference elaborator on its own corpus, on the branch that
  carries it~\cite{modulesyntax}. It is dependency-free and builds in
  seconds. The instructive half of that corpus is the
  \texttt{bad/} directory: six modules that are wrong on purpose, each with the
  diagnostic it earns. Reading those is the fastest way to acquire the
  sharing-is-a-claim reflex of \S\ref{sec:sharing}.
\item Write a \texttt{Theory} declaration for a data format you already own ---
...
\item Then, if the shape of the thing appeals, add a \texttt{Rewrites} block and
  discover that you have written a domain-specific language --- and then take
  the part of it that was not specific to your domain, give it a parameter, and
  discover you have written two.
\end{itemize}
...
cost-accounted computation \\[3pt]
\emph{Plausible Fiction on \flang{}}~\cite{PF} and its priced
companion~\cite{PricedPF} &
the fourth rung under load: a dependently typed object language written as a
\texttt{Theory}, and a second one that extends it by taking it as a parameter
\\[3pt]
\emph{\flang{} DDL and module system}~\cite{DDLPlan} &
the normative surface --- the decisions, the grammar of a theory expression,
and the list of what is struck \\
\bottomrule
\end{tabular}
\end{center}
