...
\begin{abstract}
    We derive contextual transition systems for programming languages specified as \emph{lambda theories} --- equational logics with binding and rewriting --- using Milner's framework of relative pushouts. Our central move is to make the class of admissible observers a \emph{parameter} of the construction. Where Leifer and Milner restrict which contexts may \emph{react}, we additionally restrict which contexts may \emph{observe}, and compute relative pushouts inside that restricted class rather than in the ambient theory. The resulting notion of $\D$-relative bisimilarity is a congruence for $\D$, which is the statement one actually wants: for a reflective calculus, congruence fails for arbitrary contexts and holds for the admissible ones; for an encoding, it fails for arbitrary target contexts and holds for the images of source contexts. We show that three apparently separate phenomena --- the failure of congruence under reflection in the $\rho$-calculus, the requirement that an encoding be probed only by encoded contexts, and the correctness of compiling away built-in data types --- are one theorem instantiated three times. The last of these is our practical motivation: a user of a language-definition platform should be able to specify a pure theory and know that extending it with built-in data types leaves their reasoning intact.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Three instantiations: reflection in the $\rho$-calculus (\S\ref{sec:rho}), encodings including $\pi$ into $\rho$ (\S\ref{sec:enc}), and extension by built-in data types (\S\ref{sec:data}), where we prove that the equations of a finitary algebraic theory become weak bisimulations under a Scott-style encoding, worked in full for $\mtt{Bool}$.
\end{enumerate}
...
Replication elimination $\rho^!\to\rho$ & Thm.~\ref{thm:repl} & \stProved\\
$\pi\to\rho$ preserves behavior & Thm.~\ref{thm:pirho} & \stOblig\\
...
\subsection{Metatheories and free extension}
\label{sec:meta}
...
\begin{mdframed}
\begin{center}
    \textbf{$\rho^!$: $\rho$ + replicated input}\\~\\
    \begin{tabular}{cl}
...
\begin{obligation}
\label{ob:decomp}
    Verify that $\map{C_\pi}\subseteq\rho$ is decomposition-closed up to $\approx$.
\end{obligation}
...
The smallest instance is the elimination of replicated input from $\rho^!$, using a duplicator stored at a name \citep{pi-rho}.

\begin{proposition}[\citealp{pi-rho}]
    There is an encoding of $\rho^!$ into $\rho$ given by
    \begin{center}
        \renewcommand{\arraystretch}{1.2}
        \begin{tabular}{rcl}
...
\begin{theorem}
\label{thm:repl}
    If $p\Dbis q$ in $\T_\rho^!$ then $\map{p}\Dbis \map{q}$ in $\T_\rho$.
\end{theorem}
\begin{proof}
    The only difference of $\T_\rho^!$ from $\T_\rho$ is the operation $!\of \{\mtt{in}(n,c)\}\to Pr$ and the rewrite $!(p)\rsq p\pr !(p)$, with no equations between $!$ and the other operations; so it has exactly one redex IPO not already in $\T_\rho$. It therefore suffices to consider $\lambda x.p\Dbis \lambda x.q$ and apply $\map{!\mtt{in}(n,-)}$. Because the result is an abstraction, the first transition must be an evaluation at a particular $n$.
...
The main encoding uses the same duplicator, plus a name server $!N$ that supplies fresh names to interpret $\nu$.

\begin{theorem}
\label{thm:pirho}
    \stOblig{} There is an encoding of the $\pi$ calculus with replicated input into the $\rho$ calculus, given as follows.
    \begin{center}
    \renewcommand{\arraystretch}{1.2}
    \begin{tabular}{rcl}
...
We state the proof obligations rather than the proof. The argument factors through the two lemmas below, of which the second is Theorem~\ref{thm:repl}.

\begin{obligation}[the $\nu$ lemma]
\label{ob:nu}
    The three $\nu$-equations of $\pi$ become bisimulations:
    \tbl{
        $\map{\nu(\lambda x.0)}(n,v)$ & $\approx$ & $0$\\
        $\map{p\pr \nu(\lambda x.q)}(n,v)$ & $\approx$ & $\map{\nu(\lambda x.(p\pr q))}(n,v)$\\
        $\map{\nu(\lambda x.\nu(\lambda y.p))}(n,v)$ & $\approx$ & $\map{\nu(\lambda y.\nu(\lambda x.p))}(n,v)$
    }
    equivalently, that $\map{-}$ is a functor into $\map{\D}/\!\approx$ on the $\nu$ fragment.
\end{obligation}

This is the whole difficulty and it is where the effort should go. The name-supply discipline is what carries it: $+n$, $n+$ and $n\cdot n$ generate a binary tree of names from a single root, and the second equation holds because relocating a $\nu$ across a parallel composition permutes which subtree a fresh name is drawn from, which no encoded context can detect. That last clause is exactly a $\map{\D}$-relative statement and is false for arbitrary $\rho$ observers, which can send on $+n$ directly.

\begin{remark}[staging]
    We suggest proving Obligation~\ref{ob:nu} against $\rho^!$ rather than $\rho$, and composing with Theorem~\ref{thm:repl}:
    \[\pi \longrightarrow \rho^! \longrightarrow \rho\]
    The second leg is done. The first leg then concerns only $\nu$ and the name supply, with replication handled natively. The factorization is not free, since $\map{-}$ into $\rho$ is parameterized by $(n,v)$ where the replication encoding needs only $n$; but it isolates name creation, which is the part nobody has written down.
\end{remark}

\begin{obligation}
\label{ob:pifull}
    Full abstraction: $\map{p}\Dbis\map{q}$ in $\map{\D}$ implies $p\Dbis q$ in $\pi$.
\end{obligation}
...
\end{lemma}
\begin{proof}
    The maps $n\mapsto +n$ and $n\mapsto n+$ are injective with disjoint images, since $\mtt{o}(n,0)$ and $\mtt{i}(n,\lambda x.0)$ are distinct terms and $@$ is injective. So roots form a binary tree and the derived names of a node lie in its own subtree.
\end{proof}

\begin{lemma}[root independence]
\label{lem:root}
...
\textbf{Encodings.} \citet{gorla} surveys the criteria by which an encoding between process calculi is judged --- compositionality, name invariance, operational correspondence, divergence reflection, success sensitiveness. Definition~\ref{def:encoding} is more parsimonious and, we think, better factored: the criteria are consequences of being a functor into $\map{\D}/\!\approx$ rather than a list. \citet{funproc} is the origin of the question for $\lambda$ into $\pi$; \citet{boudol} gives the name-passing variant that makes the translation tractable; \citet{pi-rho} gives the $\pi$-into-$\rho$ translation we take up in \S\ref{sec:enc}. Scott-style encodings of data are folklore; \citet{mogensen} is the usual reference.
...
\begin{enumerate}
    \item \textbf{Decomposition-closure} (Obligation~\ref{ob:decomp}). Whether $\map{C_\pi}$ is closed under decomposition up to $\approx$ inside $\rho$. Everything else depends on this and it is cheap to test: one worked factorization of an encoded context will settle it.
    \item \textbf{The $\nu$ lemma} (Obligation~\ref{ob:nu}). The single largest piece of new mathematics, and the one that has been outstanding longest. We recommend proving it against $\rho^!$ and composing with Theorem~\ref{thm:repl}.
