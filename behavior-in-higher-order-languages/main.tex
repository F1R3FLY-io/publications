...
$\pi\to\rhoc$ preserves behavior & Thm.~\ref{thm:pirho} & \stOblig\\
...
The smallest instance is the elimination of replicated input from $\rhoc^{!}$, using a duplicator stored at a name \citep{pi-rho}.
...
\begin{remark}[staging]
    We suggest proving Obligation~\ref{ob:nu} against $\rhoc^{!}$ rather than rho, and composing with Theorem~\ref{thm:repl}:
    \[\pi \longrightarrow \rhoc^{!} \longrightarrow \rhoc\]
...
\begin{enumerate}
    ...
    \item \textbf{The $\nu$ lemma} (Obligation~\ref{ob:nu}). The single largest piece of new mathematics, and the one that has been outstanding longest. We recommend proving it against $\rhoc^{!}$ and composing with Theorem~\ref{thm:repl}.
