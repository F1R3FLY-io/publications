...
(``keep everything from both faces'') is aligned with \emph{unification}~\cite{robinson} --- the most general common
...
(structural congruence), and rewrites (reduction)~\cite{nslogic,ntt,gslt}, and the continuation in an
interaction redex \emph{carries binding structure}. The whole graded apparatus must be reread inside
the category of terms-with-binding --- nominal sets / a presheaf category over name-contexts, or
de~Bruijn families~\cite{gabbaypitts,pitts,fpt}. We list what each of the three ingredients becomes.
...
\emph{fusion}~\cite{parrowvictor,lanevevictor}: it computes a unifier $\sigma=\mgu(\text{face}_1,\text{face}_2)$ and applies $\sigma$
