...
\newcommand{\drop}[1]{{*}#1}
\newcommand{\quo}[1]{@#1}
\newcommand{\dr}[1]{\,\overline{#1}\,}   % dereference / unquote: written #x in the source, set as bar here
...
\begin{abstract}
We make the one-page fuzzy $\rho$-calculus precise: the comm rule transmits not $Q$ but
$(u\cmp x)\act Q$, where $u\cmp x$ is a quantale element measuring the match between the two
interaction faces and $a\act(-)$ is its action on terms. We compute the model in full, show
that the order axiom is a \emph{theorem} there (modulo one congruence side-condition isolated as
a lemma), and observe that the page commits to an \emph{accumulator} reading of $\cmp$, with a dual
\emph{attenuator} reading available. We then sketch the intended payoff: this is the binding-free,
asymmetric, single-face instance of a general construction --- a quantale-graded interaction in a
concurrent--interactive GSLT --- and identify what each ingredient must become once continuations
...
Recall the reflective syntax, with processes $P$ and names $x$ mutually defined, and the single
structural reduction (comm):
\[
...
$\dr{x}$ for the dereference (the source's $\#x$), so $\dr{\quo P}=P$ and $\quo{\dr x}=x$.
...
(-\cmp-)\colon \Names\times\Names\to A \quad(\text{face comparison}),
\]
...
\paragraph{The grade structure.} We demand that $A$ carries a quantale structure
...
The page's model takes grades to be names themselves and realises every piece through
...
\begin{remark}[The page commits to an accumulator]\label{rem:fork}
...
(``keep everything from both faces'') is aligned with \emph{unification} --- the most general common
\emph{instance}, $\mgu$ --- while the attenuator (``keep only what is shared'') is aligned with
\emph{anti-unification} --- the least general common \emph{generalization}, $\lgg$. These are dual
...
Flat $\rho$ is convenient because $\FN(P)$ is a bare set of names and the action
\eqref{eq:modeldefs} is just a name-substitution over that set. In a concurrent--interactive GSLT a
\emph{language} is presented by a grammar (sorts/operations, some of which bind), equations
(structural congruence), and rewrites (reduction), and the continuation in an interaction redex
\emph{carries binding structure}. The whole graded apparatus must be reread inside the category of
terms-with-binding --- nominal sets / a presheaf category over name-contexts, or de~Bruijn families.
We list what each of the three ingredients becomes.
...
quantale (functorial and monotone in $A$) and $\meas$ a lax monotone measure, whence \emph{fuzzy
reduction is grade-monotone} --- now internal to the presheaf category.

\subsection{(ii) The comparison $\cmp$: subject-matching vs.\ unification}

\paragraph{Asymmetric case (the usual suspects: $\lambda,\pi,\rho$).}
There is a privileged binding face. The faces are a subject pair (the input channel $x$, the output
...
There is no privileged binder. Both faces present terms with (meta)variables, and the reaction is a
\emph{fusion}: it computes a unifier $\sigma=\mgu(\text{face}_1,\text{face}_2)$ and applies $\sigma$
to \emph{all} continuations in scope, each application a graded action. The grade is read off the
unifier,
\[
\cmp(\text{face}_1,\text{face}_2)\;=\;\mathrm{grade}(\sigma)\in A,
\]
...
\emph{unification distance}: how much the $\mgu$ had to do to make the faces agree, or whether they
agree at all. Dually (Remark~\ref{rem:foretaste}), the attenuator/meet variant is realised by
anti-unification: $\cmp=\lgg$ keeps the shared skeleton of the two faces, the symmetric face of the
...
\noindent\textbf{Summary.} Fuzzy $\rho$ is the binding-free, asymmetric, single-face, par-quote
instance of one construction: a quantale-graded interaction in a ci\,GSLT, with the grade obtained by
comparing interaction faces (subject-matching when asymmetric, (anti-)unification when symmetric),
