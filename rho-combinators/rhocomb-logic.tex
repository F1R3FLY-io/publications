...
\date{September 2026 --- draft 2, revision 2, 23 September 2026}
...
\paragraph{Revision 2.} The companion's signature is now generated: one
constructor per atom shape, erected at run time by a second translation phase
\cite[Def.~3.8, \S6.3]{rhocomb3}. The grammar here has one structural
production per shape and extends schematically, so $\Atm_{\mathrm{full}}$ is
restated as the shape set of a program's signature, and the lattice of
\S\ref{sec:occ} is read, as the companion's is, over the four
static-apparatus constructors. Two consequences are recorded where they
arise: every phase-two image contains $\ev$, so the decidability of
Proposition~\ref{prop:decmodal} covers no compiled program with an input; and
occupancy is informative for phase-one images, while phase-two images all sit
near the top of both axes. No result is changed. Numbering is preserved.
\end{abstract}
...
Write $\Atm_{\mathrm{full}}$ for the set of atom shapes of the signature ---
the thirteen displayed, for the static-apparatus signature, and in general the
base atoms, $\consp$, and one constructor for each shape a program erects
\cite[Def.~3.8]{rhocomb3}, every production of \S\ref{sec:logic} being a
schema over shapes --- and
...
implementation of~B. Both presentations have the same atom shapes, so
...
The hypothesis $\ev \notin \Atm$ excludes every compiled program with an
input: the gate of \cite[Lem.~6.2]{rhocomb3} contains $\ev$, and phase two
erects each allocated atom through $\ev$ \cite[Rem.~3.11]{rhocomb3}. For
compiled code the proposition applies to phase-one images of input-free
programs only, and model checking a compiled program is the undecidable case of
Proposition~\ref{prop:undec} unless a bound is supplied.
