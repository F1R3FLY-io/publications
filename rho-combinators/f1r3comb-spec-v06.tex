...
\begin{abstract}
\noindent
v0.5 made the erection scheme a seam and set the default to the curried,
path-indexed constructors, against \cite[Rem.~6.22]{draft3}'s recommendation
of the context-instantiation atom $\inst$. That default is reversed here.

The reversal is not a change of mind about the arguments but a correction of
the frame they were weighed in. v0.5 weighed $\inst$ against a store-based
machine and against \cite{logic}; it did not weigh it against where this
compiler is going. \cite{matrix} and \cite{matspec} make the target a sparse
linear-algebra representation executed data-parallel, and two of v0.5's three
arguments move under that frame. The substitution objection --- that
$C[v]$ reintroduces substitution under quotation as a machine primitive ---
largely dissolves, because $C$ is a compile-time constant and $C[v]$ is a
fixed linear map on the vector encoding, no more exotic than any other row of
the rule matrix. The depth argument reverses outright and decisively: erection
in two steps per instance regardless of unit size keeps the band of the
upper-triangular matrix shallow and constant, where the curried scheme costs
$n$ steps for an $n$-atom unit; \cite[Rem.~6.22]{draft3}'s observation that
$\inst$ inflates width rather than depth is worth more here than it is on a
store. A fourth argument appears that v0.5 did not have: $\inst$ is one rule
row for any program, while the curried family needs a row per shape and path
tuple, so the rule matrix is program-dependent and wider for no gain.

The one v0.5 argument that survives intact is the logic's.
\cite{logic} revision~2 restates $\Atm_{\mathrm{full}}$ in the curried style,
and $\inst$'s third argument is a term with holes, so it has no structural
production, no occupancy disjunct and no position on either axis
(Remark~\ref{rem:logiccost}). The curried scheme is therefore kept, behind
\code{curried-erection}, as the shape-table-conforming variant the logic
tooling can read. Everything else follows: the context node joins the term
type, the machine gains $C[v]$ as a first-class traversal, the allocation
gadget and the address fan-out leave the specification entirely, and the
figures become two steps and $O(|{\rm unit}|)$ work per instance.
Section~\ref{sec:v5delta} lists the changes.
...
\code{0x60} & $\inst(a,f,C)$\\
\code{0x61} & context node (Requirement~\ref{req:contextnode})\\
\code{0x62} & hole, carrying its canonical index\\
\bottomrule
\end{tabular}
\end{center}
...
Under Decision~\ref{dec:scheme} there is no fan-out: $\inst$ consumes the
address once and the unit is erected from it. Under
\code{curried-erection} the fan-out is of the \emph{address}, not of each
leaf: one copy of $\sigma$ per erecting constructor, delivered by a duplicator
tree on static channels, each $\dd$ having one premise and one address input. Either count \MUST{} be computed
...
\begin{decision}[$\inst$ is the target; curried behind a feature]
\label{dec:scheme}
\Comb{} implements $\inst$ by default, following
\cite[Rem.~6.22]{draft3}, and the curried scheme under the
\code{curried-erection} feature. The scheme \MUST{} be recorded in the
artefact header, since the two differ in every reduction sequence and every
step count, and a golden file \MUSTNOT{} be compared across schemes. The
curried variant is retained not as a hedge but because it is the one
\cite{logic} can read (Remark~\ref{rem:logiccost}); it is the logic tooling's
target, not the machine's.
\end{decision}
...
\cite[Rem.~6.22]{draft3} says as much. Decision~\ref{dec:scheme} therefore
does not rest on it: it rests on the matrix path
(Remark~\ref{rem:whyinst}). A third arm in \code{phase2.py} is still cheap and
would let the step and mixing figures be compared directly rather than argued.
\end{remark}
...
correction cuts against the default Decision~\ref{dec:scheme} takes, which is
why it is worth keeping: the engineering gap between the schemes is narrower
than the table suggests, so the case for $\inst$ has to be made on the grounds
of Remark~\ref{rem:whyinst} rather than on bookkeeping the curried scheme does
not in fact carry. The corrected comparison:
\end{remark}
...
\begin{remark}[Why the default is $\inst$]\label{rem:whyinst}
The comparison above is nearly even on a store-based machine, which is the
frame v0.5 weighed it in. It is not even on the path this compiler is on.
\cite{matrix} and \cite{matspec} make the eventual target a sparse
linear-algebra representation of the reduction graph, executed data-parallel;
three of the rows change sign under that frame and a fourth appears.

\textbf{Substitution stops being a primitive and becomes a constant.} The
objection to $C[v]$ is that \cite[\S3.6]{draft3} identifies substitution under
quotation as the operation the constructor family exists to simulate, so
taking it as primitive gives up the property that a rule's right-hand side is
built from names by fixed shape formers. On a store that costs
Decision~\ref{dec:machine}: filling holes beneath quotations is a traversal
with a rebuild, not a store operation. In the matrix representation $C$ is a
compile-time constant, so $C[v]$ is a fixed linear map on the vector encoding
and the rule is one row like any other. The objection is real for the
intermediate machine and does not survive to the destination, and a
specification that is building towards the destination should say which
weighs more.

\textbf{Depth.} Two steps per instance regardless of unit size keeps the band
of the upper-triangular matrix shallow and constant; the curried scheme costs
$n$ steps for an $n$-atom unit and deepens it by the size of the program's
largest unit. \cite[Rem.~6.22]{draft3} puts it as inflating width rather than
depth, and width is what a data-parallel execution is for. This is the
argument that decides it.

\textbf{One rule row, not a family of them.} $\inst$ is a single rule for any
program. The curried family needs a row per shape and path tuple, so the rule
matrix is program-dependent and wider, with no compensating gain: the factored
form \cite{matrix} wants --- a matrix representation of the rewrite rules,
applied repeatedly to a vector encoding of a term --- is cleanest when the
rule set is fixed. v0.5 did not have this argument because it was not looking
at the rule set as an object.

\textbf{What does not change.} The work figure. $\inst$ erects a unit in two
steps, but the second rebuilds the unit's term with leaves substituted and
recomputes cached encodings, $O(|{\rm unit}|)$ inside one rule application;
the curried scheme spends the same total across $n$ parallel steps. A
cost-accounted variant \MUST{} charge $\inst$ by size and not by step, and
Requirement~\ref{req:instwork} says so.
\end{remark}

\begin{remark}[What $\inst$ costs the logic, which is not nothing]
\label{rem:logiccost}
\cite{logic} revision~2 restates $\Atm_{\mathrm{full}}$ as the base atoms,
$\cons_{\mid}$, and one constructor per shape a program erects, with every
production of its grammar a schema over shapes. The curried family is exactly
that shape set with a path tuple as a parameter. $\inst$ is not: its third
argument is a term with holes, not a name, so it has no structural production,
no occupancy disjunct, and no position on either axis of
\cite[\S9]{draft3}'s lattice. Under $\inst$ the phase-two occupancy report
degenerates to a single shape.

Two things keep this from being decisive. Occupancy was already informative
mainly for phase-one images, which no erection scheme touches; and
\cite{logic} revision~2 already records that phase-two images sit near the top
of both axes, so the report was close to a constant function before $\inst$
made it one. What is lost is the ability to read a phase-two image in the
logic at all, and that is why \code{curried-erection} is kept
(Requirement~\ref{req:curriedfeature}) rather than deleted. The two notes
currently point in different directions on this and
Section~\ref{sec:paper} asks which is intended.
\end{remark}

\begin{requirement}[The context node]\label{req:contextnode}
\code{f1r3comb-term} \MUST{} carry a context node: a term with holes in name
positions, holes numbered canonically in encoding order, encodable and
hashable like any other node, with its own tag in the block of
Requirement~\ref{req:tags}. Holes \MUST{} be permitted beneath quotations,
since \cite[\S6.7]{draft3} requires it and the gate's payload is where they
occur. A context \MUSTNOT{} appear anywhere but as the third argument of an
$\inst$ atom, and the type system \SHOULD{} enforce that, so that
Requirement~\ref{req:novars} --- no binders in the target --- is not quietly
weakened by a node that looks like one.
\end{requirement}

\begin{requirement}[Filling a context]\label{req:instwork}
The machine \MUST{} implement $C[v]$ as a traversal that descends into
quotations, substitutes $v$ at every hole, and rebuilds cached encodings on
the way out, producing a term in normal form
(Requirement~\ref{req:dropelim}). Hole positions \MUST{} be precomputed at
compile time and stored with the context, so the traversal visits only the
spine from each hole to the root rather than the whole term. The cost \MUST{}
be reported to the observer seam as $O(|{\rm unit}|)$ for the single step, not
as unit cost: a cost-accounted variant that charges $\inst$ one step would
make erection free, which it is not.
\end{requirement}

\begin{requirement}[$\inst$ and the discipline]\label{req:instsai}
$\inst(r,f,C_{\mathrm{unit}}) \mid \ev(f)$ is the whole of a unit's phase-two
image. It satisfies Definition~\ref{def:sai} trivially: one premise, one
address input, at the static channel $r$. The conformance check of
Requirement~\ref{req:sai} \MUST{} still run --- it is cheap and it is what
catches a regression that reintroduces a join --- and \MUST{} not be skipped
on the grounds that the default scheme passes it by construction.
\end{requirement}

\begin{requirement}[The curried feature]\label{req:curriedfeature}
Under \code{curried-erection} the compiler \MUST{} emit
$\cons^{*}_{A}[\vec w]$ per erected atom with a duplicator tree delivering a
copy of $\sigma$ to each, \MUST{} omit the context node from the emitted term,
and \MUST{} produce an occupancy report over the shape set that
\cite{logic} revision~2 describes. The feature \MUST{} be exercised on the
whole corpus (Obligation~\ref{obl:schemes}), because a variant kept for one
consumer and never run is a variant that does not work.
\end{requirement}
...
$r_{j}$, the delivery channel $f$ of each unit's $\inst$, and, under
\code{curried-erection}, the $f_{A}$ and the fan-out channels of
...
Section~\ref{sec:schemes}, but by different routes and this \MUST{} be
reported separately: $\inst$ emits two atoms per unit whose context carries
the unit's text, so the atom count is $2u$ for $u$ units while the byte size
tracks $|P|$; \code{curried-erection} emits one constructor and one $\dd$ per
erected atom, so both track $|P|$. The logarithmic factor \cite[Prop.~8.1]{draft3} folds in is a
...
\begin{remark}[The one place $\inst$ dents this]\label{rem:machinedent}
Every rule but $\inst$ is a subject match and a rebuild from names. $\inst$'s
right-hand side is $C[v]$, a traversal with a rebuild
(Requirement~\ref{req:instwork}), so the machine is \Camp{} with a three-line
matcher \emph{plus} one term operation, dispatched from one row of the
generated table as \code{consume([a],[loud],Inst(f,C))}. That is the cost
Remark~\ref{rem:whyinst} weighs and accepts: it is bounded --- one operation,
on a compile-time-constant context, with precomputed hole positions --- and it
touches neither the store, the matcher, the joins nor replay. Under
\code{curried-erection} it is absent.
\end{remark}
...
while occupancy is informative for phase-one images. Under Decision~\ref{dec:scheme} the
phase-two point degenerates completely to a single shape
(Remark~\ref{rem:logiccost}); \code{curried-erection} is what restores it, and
is the build the logic tooling \MUST{} use. Reporting only the target would make the
...
\cite[Rem.~2.5]{logic} turns. Under the default scheme the API \MUST{} report
plainly that a phase-two multiset is a single $\inst$ shape rather than
returning a degenerate multiset that looks like a measurement; the report
\MUST{} name \code{curried-erection} as the build that gives a shape set. It
\SHOULDNOT{} expose more.
\end{requirement}
...
\begin{obligation}[Both schemes, on the whole corpus]\label{obl:schemes}
The corpus \MUST{} be compiled and run under $\inst$ and under
\code{curried-erection}, and the observations compared, exactly as
Obligation~\ref{obl:AB} compares presentations. Encodings, hashes, step counts
and traces \emph{will} differ and \MUST{} be expected to; only observations are
compared. For $\inst$ the step count per erection \MUST{} be two and the work
\MUST{} be measured, not assumed, so that
Requirement~\ref{req:instwork}'s claim about where the cost went is tested
rather than asserted.
\end{obligation}

\begin{obligation}[Contexts round-trip]\label{obl:context}
For generated units, $C[v]$ \MUST{} agree with the term obtained by compiling
the same unit under \code{curried-erection} and running its erection to
completion: same atoms, same names, same encoding. This is the one place a
context node could silently diverge from the calculus, because it is the one
node whose meaning is given by an operation rather than by its children, and
it is worth a property test of its own.
\end{obligation}
...
\code{comb-context-misplaced} & 4 & A context node outside the third argument
of an $\inst$ (Requirement~\ref{req:contextnode})\\
...
node. A machine step \MUST{} be $O(1)$ expected in the size of the store, with the
single exception of $\inst$, which is $O(|{\rm unit}|)$ by
Requirement~\ref{req:instwork} and \MUST{} be excluded from the per-step
figure rather than averaged into it,
...
\item \textbf{The two-step figure is not a work figure}
(Requirement~\ref{req:instwork}). This specification now takes $\inst$
(Decision~\ref{dec:scheme}), so the point is no longer an objection but a
statement the recommendation should carry: the second step rebuilds the unit's
term with leaves substituted and recomputes cached encodings,
$O(|{\rm unit}|)$ inside one rule application. A cost-accounted variant must
charge $\inst$ by size and not by step, or erection becomes free. Proposed
amendment: say so in \S6.7, next to the ``two steps'' row, since that row is
what the recommendation is read off.

\item \textbf{$C[v]$ and \S3.6.} \cite[\S3.6]{draft3} identifies substitution
under quotation as the operation the constructor family exists to simulate;
$\inst$ takes it as primitive. That is a real change in what the calculus
claims about itself, and the note should say which claim survives --- on our
reading, that the \emph{base} atoms are name-free and the erection layer is
explicitly not, which is a defensible position but a different one from
\S3.6's. On the matrix path the cost largely disappears, because $C$ is a
compile-time constant and $C[v]$ is a fixed linear map
(Remark~\ref{rem:whyinst}); that is the frame in which the recommendation is
right, and naming the frame would strengthen it.

\item \textbf{$\inst$ has no home in \cite{logic}} (Remark~\ref{rem:logiccost}).
Revision~2 restates
...
occupancy disjunct and no position on either axis. With
Decision~\ref{dec:scheme} the divergence is now between the implementation's
default and the logic's grammar, which is the sharpest form of the question:
either the logic gains a production for a context-parameterised atom, or it
declares itself a logic for the curried presentation and the compiler keeps
\code{curried-erection} as the build it reads. Proposed amendment: say which,
in \cite{logic}, since the two notes currently point in different directions
and the implementation has now picked a side.
...
\item \textbf{The matrix encoding.} Now the reason for
Decision~\ref{dec:scheme} rather than an aside. Two things still want
settling. A reduction graph as an upper-triangular matrix wants a bounded name
universe, and phase two makes the universe grow one address level per
unfolding; \cite{matspec} is where that is answered, and this specification's
Requirement~\ref{req:intern} (prefix sharing) is the half of the answer that
lives here. And the rule matrix under $\inst$ has one row whose right-hand
side is a compile-time constant map --- one per context, hence one per unit,
hence program-dependent after all, though bounded by the program rather than
by the shape-and-path product. Whether that is better than the curried rule
set as a sparse object is a measurement \cite{matspec} can make and this
...
M2 & Phase two & The context node and $C[v]$
(Requirements~\ref{req:contextnode}, \ref{req:instwork}), $\inst$ erection,
the marking analysis (Requirement~\ref{req:marking}), spares; the
...
\code{curried-erection} and Obligations~\ref{obl:schemes}
and~\ref{obl:context}\\
...
%======================================================================
\section{Changes from v0.5}\label{sec:v5delta}
...
v0.5 & v0.6\\
\midrule
Decision 5.19: curried by default, $\inst$ behind \code{inst-erection} &
\textbf{Reversed.} $\inst$ is the target; curried behind
\code{curried-erection} (Decision~\ref{dec:scheme})\\
Remark 5.22, three reasons for curried & Remark~\ref{rem:whyinst}: the
substitution objection is a cost on a store machine and dissolves on the
matrix path, where $C$ is a compile-time constant; the depth argument reverses
and decides it; and a fourth argument appears, that $\inst$ is one rule row
where the curried family is a row per shape and path tuple\\
The logic objection, one of three & Remark~\ref{rem:logiccost}, the only one
that survives intact, and the reason \code{curried-erection} is kept as the
logic tooling's build rather than deleted\\
Requirement 5.23, what \code{inst-erection} would turn on & Split into
Requirements~\ref{req:contextnode} (the context node in the term type and the
tag table), \ref{req:instwork} ($C[v]$, precomputed hole positions, cost
charged by size), \ref{req:instsai} (conformance still checked) and
\ref{req:curriedfeature} (the curried build)\\
Fan-out of the address, one $\dd$ per erected atom & Gone under the default:
$\inst$ consumes the address once. Retained under
\code{curried-erection}\\
Machine is \Camp{} with a three-line matcher & Plus exactly one term
operation, from one row of the generated table
(Remark~\ref{rem:machinedent})\\
--- & New: Obligation~\ref{obl:schemes}, both schemes on the whole corpus,
and Obligation~\ref{obl:context}, $C[v]$ agreeing with the curried erection
run to completion --- the one node whose meaning is an operation rather than
its children\\
Matrix encoding as an open-question aside & The rationale for the decision,
with the two things it still leaves open\\
Seven items asked of the papers & Seven, with two reframed: the work figure
and \S3.6 are no longer objections but statements the recommendation should
carry\\
\bottomrule
\end{tabularx}
\end{center}
...
\bibitem{matrix} L.\,G. Meredith.
\emph{Rho combinators as sparse linear algebra: a classical route from
reflective concurrency to data-parallel execution.} Version~1.2,
\code{publications/rho-combinators}, 23 September 2026.

\bibitem{matspec} F1R3FLY.io.
\emph{The rho combinators as sparse linear algebra: a Rust implementation of
the matrix representation, the bulk-synchronous machine, and the transfer
operator.} Specification v0.2, \code{publications/rho-combinators},
23 September 2026.
