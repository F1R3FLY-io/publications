...
that enforcement never paints a voice into a corner. This is the dance between freedom
and constraint that jazz makes its own: a score contains non-determinism, an
improvised performance is a trace through it, and the crisp layer fixes which
traces are admissible while the grading says which the idiom favours.
...
\paragraph{Freedom and constraint.} Much of the appeal of jazz lies in the
dance between freedom and constraint: the player is free, but free within a
tune, a set of changes, a style, and what the band is doing. Conventional
notation has no place for that dance. A page either fixes the notes, leaving
nothing to the player, or writes chord symbols and leaves the constraint to
convention. A score in this calculus can say both things at once. It contains
non-determinism, and an improvised performance is a \emph{trace} through it:
one path among the paths the score allows. The score does not merely permit
improvisation; it says which traces are admissible and which are not, and, among
the admissible ones, which the idiom favours. Constrained non-determinism is the
native form of such a score, not an approximation to it
(\S\ref{ssec:improv}).
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item The \emph{score calculus} (\S\ref{sec:calculus}), illustrated on
  \emph{Twinkle, Twinkle, Little Star} (\S\ref{ssec:twinkle}), in which every reduction
...
\item \emph{Motifs and idioms} (\S\ref{sec:idioms}): recognising and completing
  ...
  (Proposition~\ref{prop:enforce}); improvisation as a trace through a score
  whose crisp layer fixes which traces are admissible and whose grading fixes
  how likely they are (Proposition~\ref{prop:support}); message-passing idioms as score structure
  (\S\ref{ssec:exchange}).
...
\subsection{Example: \emph{Twinkle, Twinkle, Little Star}}\label{ssec:twinkle}

Before any grading, the calculus can already write down a fixed piece. Take the
first phrase of the tune --- ``Twinkle, twinkle, little star, how I wonder what
you are'' --- with a plain bass line under it:
\begin{center}
\begin{tabular}{@{}lcccc@{}}
\toprule
& bar 1 & bar 2 & bar 3 & bar 4 \\
\midrule
melody & C4 C4 G4 G4 & A4 A4 G4\,(half) & F4 F4 E4 E4 & D4 D4 C4\,(half) \\
bass   & C3\,(half) C3\,(half) & F3\,(half) C3\,(half) & F3\,(half) C3\,(half) & G3\,(half) C3\,(half) \\
\bottomrule
\end{tabular}
\end{center}
Unmarked melody notes are quarters. The pitch alphabet contains the nine pitches
used and the rest $\rest$; the duration alphabet contains the quarter
$\mathsf{q}$ with $\len{\mathsf{q}} = \tfrac14$, the half $\mathsf{h}$ with
$\len{\mathsf{h}} = \tfrac12$, and the null duration.

\paragraph{One written note.} A note of pitch $p$ and duration $d$, played at a
location $L$ in timbre $\tau$ and followed by $P$, is a hand on the channel for
$p$ together with a message on the co-channel for $d$:
\[
\mathsf{N}_{L,\tau}(p, d;\, P) \;:=\;
  \rcv{y}{\nm{L}{\tau}{p}}{P} \;\mid\; \snd{\nm{L}{\tau}{d}}{0}{\tau}{\rest}
\qquad (y \text{ not free in } P).
\]
The two meet by $\textsc{Comm}_1$ and emit $(\tau, p, d)$. The name passed to
the continuation is not used, so the payload and the carried datum do not
matter.

\paragraph{The score.} A part is a right-nested chain of written notes, and the
piece is the parallel composition of its parts. With $L_m$ and $L_b$ two
inequivalent locations,
\begin{align*}
\mathsf{Melody} &:= \mathsf{N}_{L_m,\mathsf{piano}}(\mathrm{C4},\mathsf{q};\;
  \mathsf{N}_{L_m,\mathsf{piano}}(\mathrm{C4},\mathsf{q};\;
  \mathsf{N}_{L_m,\mathsf{piano}}(\mathrm{G4},\mathsf{q};\; \cdots\;
  \mathsf{N}_{L_m,\mathsf{piano}}(\mathrm{C4},\mathsf{h};\; 0)\cdots))) \\
\mathsf{Bass} &:= \mathsf{N}_{L_b,\mathsf{bass}}(\mathrm{C3},\mathsf{h};\;
  \mathsf{N}_{L_b,\mathsf{bass}}(\mathrm{C3},\mathsf{h};\;
  \mathsf{N}_{L_b,\mathsf{bass}}(\mathrm{F3},\mathsf{h};\; \cdots\;
  \mathsf{N}_{L_b,\mathsf{bass}}(\mathrm{C3},\mathsf{h};\; 0)\cdots))) \\
\mathsf{Twinkle} &:= \mathsf{Melody} \;\mid\; \mathsf{Bass}
\end{align*}
with fourteen written notes in the melody and eight in the bass, in the order of
the table. It is a finite term: no recursion, no where-clauses, no grading.

\paragraph{Melody is interaction.} At the start, the only top-level parts of
$\mathsf{Melody}$ are the hand holding C4 and the message holding a quarter,
both at $L_m$ and both stamped $0$. They communicate, emitting
$(\mathsf{piano},\mathrm{C4},\mathsf{q})$ at onset $0$, and the continuation
brings the next hand and message to top level stamped $\tfrac14$. Each note
sets up the next. Because only one hand and one message of the melody are ever
at top level, every contention set has exactly one candidate, and the melody can
use a single location for all fourteen notes.

\paragraph{Harmony is independence.} The bass lives at another location in
another timbre, so the two parts share no active location and are independent.
By Theorem~\ref{thm:interleave} every onset-ordered run has the same
performance:
\begin{center}
\small
\begin{tabular}{@{}r@{\quad}ll@{\qquad}r@{\quad}ll@{}}
\toprule
onset & melody & bass & onset & melody & bass \\
\midrule
$0$            & C4 q & C3 h & $2$              & F4 q & F3 h \\
$\tfrac14$     & C4 q &      & $\tfrac94$       & F4 q &      \\
$\tfrac12$     & G4 q & C3 h & $\tfrac52$       & E4 q & C3 h \\
$\tfrac34$     & G4 q &      & $\tfrac{11}4$    & E4 q &      \\
$1$            & A4 q & F3 h & $3$              & D4 q & G3 h \\
$\tfrac54$     & A4 q &      & $\tfrac{13}4$    & D4 q &      \\
$\tfrac32$     & G4 h & C3 h & $\tfrac72$       & C4 h & C3 h \\
\bottomrule
\end{tabular}
\end{center}
The eight rows in which both parts strike are the chords of the phrase, I I IV I
IV I V I. Nothing in the term coordinates them. Each part keeps its own clock in
its own durations, and the notes line up because the durations add up. The
reference simulator plays this term under both tie-breaking orders and obtains
the same twenty-two timed notes, ending at $4$ whole notes.

\paragraph{What would go wrong.} Write the bass at the melody's location and in
the melody's timbre, and the parts are no longer independent. At onset $0$ the
melody's hand holding C4 could then meet the bass's half-note message as well as
its own quarter, and the two parts would trade durations. Independence is a
property of $(\text{location},\text{timbre})$ pairs, and a composer who wants
independent parts keeps them apart. Proposition~\ref{prop:twosided} makes
precise what happens when they are not.

\paragraph{From a tune to a distribution of tunes.} Every fitting in
$\mathsf{Twinkle}$ has exactly one candidate, so the piece is the degenerate case
of everything that follows. Replace each written message by a fan and each hand
by a graded hand whose clause weights the tune's own transitions heavily and
others lightly, and the same score becomes a distribution over variations of the
tune (\S\ref{sec:voice} and \S\ref{sec:dist}); give its clause the phrase as a
motif, and the variations can be required to return to it
(\S\ref{sec:idioms}).
...
generator, a physical source, or the digits of a constant. It may equally be a
person: a player choosing, at each fitting, among the candidates of positive
weight (\S\ref{ssec:improv}). A score together with a fixed source is a single
determined piece.
...
\subsection{Improvisation: freedom within constraint}\label{ssec:improv}

The two layers of a clause, crisp and graded, divide the composer's authority
over an improviser cleanly.

\begin{definition}[Admissible traces]\label{def:admissible}
A trace of a score is \emph{admissible} if every note in it was emitted by a
candidate of positive weight. The \emph{support} of a score is the set of its
admissible traces.
\end{definition}

\begin{proposition}[Constraint and preference are separate]\label{prop:support}
\begin{enumerate}[label=(\roman*),itemsep=1pt]
\item The support of a score depends only on which candidates have weight zero,
  and not on the values of the positive weights. Changing the grading within the
  positive values changes how likely each admissible trace is, and never which
  traces are admissible.
\item Multiplying a clause by a crisp factor $\crisp{\beta}$ can only shrink the
  support, and every admissible trace of the result satisfies $\beta$ at every
  note.
\item With the viability factor of Proposition~\ref{prop:enforce}, every
  admissible prefix extends to an admissible continuation: a performer who stays
  within the support can never be left with nothing admissible to play.
\end{enumerate}
\end{proposition}
\begin{proof}
(i) Definition~\ref{def:wcomm} makes a candidate available exactly when its
weight is positive, and the positive values enter only the probabilities of
Definition~\ref{def:resolve}. (ii) A crisp factor sends to zero the candidates
where $\beta$ fails and leaves the others' zero-or-positive status unchanged.
(iii) is Proposition~\ref{prop:enforce}(ii).
\end{proof}

So a composer writes \emph{constraint} with crisp formulae and \emph{tendency}
with weights, and the two do not interfere. The crisp layer is the tune, the
changes and the rules of the idiom: what an improviser may play. The graded
layer is the style: what the idiom would rather play. Because the formulae
include spatial and context-labelled modalities, the constraints can be about
more than single notes. ``Return to the head within eight bars'' is a formula
over the recorded past; ``do not step on the soloist'' is a formula over what a
choice leads to in the company of the other players.

The same score then serves three kinds of performer. An algorithmic performer
samples a trace with the probabilities the grading gives. A human improviser
ignores the probabilities and chooses freely among the candidates of positive
weight, and the score guarantees that whatever is chosen is admissible and, with
the viability factor, that the choice never runs out. And a performer can mix
the two, as an accompanist driven by the score might follow a human soloist. In
each case the performance is a trace through the composition, and the
composition is the set of traces it allows together with the weight it gives
them.
