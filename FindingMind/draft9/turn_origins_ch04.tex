...
The prediction can be sharpened from a direction into a rate.
Proposition~\ref{scm:prop:frontier} shows that the copies are not
merely a by-product of efficient replication but are \emph{forced} by
depth: an ecology using a tower of height $h$ on media of reliability
$p$ must hold at least $m p^{-h}$ learners at that depth, so the two
coordinates lie on a frontier whose slope is $\log(1/p)$.
The joint rise is therefore not two findings that happen to
