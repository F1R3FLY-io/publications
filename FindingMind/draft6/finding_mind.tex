...
\input{TheTurn/turn_causality_ch01}  % Introduction and Motivation
\input{TheTurn/turn_causality_ch02}  % Graph-Structured Lambda Theories
...
\input{TheTurn/turn_causality_ch05}  % The Labeled Transition System and Context-Decorated HML
\input{TheTurn/turn_causality_ch05a} % What Goes Into a Logic
\input{TheTurn/turn_causality_ch06}  % The Weight Map and Lagrangian Structure
\input{TheTurn/turn_causality_ch07}  % The Cost Map and Hamiltonian Structure
\input{TheTurn/turn_causality_ch08}  % Extended Modal Operators
\input{TheTurn/turn_causality_ch09}  % Complex Amplitudes and Quantum Dynamics
...
\input{TheTurn/turn_sciencebot_ch04}  % Interactive GSLTs and Behavioral Primes
...
%% ----------------------------------------------------------------
%%  The Turn: Ecology as Cognitive Architecture
%% ----------------------------------------------------------------
...
\begin{mdframed}[backgroundcolor=remarkgray, linecolor=gray!60,
                 ...
                 bottomline=false]
\small\itshape
Every part so far has needed an agent and has helped itself to one.
This part builds the agent out of the same material as everything else,
and then watches what happens when a learner has to pay for what it
learns.
...
\noindent
The preceding parts have each required a knower and each declined to say
what one is.
Causality gives a physics, and someone must be there to do the physics.
The argument about how scientists are misled needs an agent with a finite
budget running the scientific method against a population --- and the whole
force of the conclusion depends on that budget being real --- yet the agent
itself stays a placeholder, a thing that has theories and can afford some
...
computation is other computations, and a learner has no privileged vantage
from which to observe: it is a term sitting in parallel with the term it
wants to know about, and the only way to find out anything is to interact.
Interaction costs.
