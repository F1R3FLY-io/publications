...
\begin{enumerate}[label=\textbf{Gap \arabic*.}, leftmargin=*, itemsep=8pt]
  ...
  \item \textbf{The oracle as a term --- superseded, and replaced
        by a sharper question.}
        This gap originally asked for a construction of the oracle
        as a persistent receive in the Rho calculus.
        Chapters~\ref{ch:apr} and~\ref{ch:agy} argue the gap was
        the wrong shape: the oracle need not be constructed at all,
        because the resolution of a contended channel is already a
        choice function and the scheduler that supplies it already
        spends a graded choice resource.
        What is now owed instead is the metatheorem.
        Does the term assignment of Chapter~\ref{ch:chs} satisfy
        subject reduction --- can a scheduler, merely by running,
        come to realise a stronger choice principle than it was
        typed for?
        If it can, the graded factorisation is not a reduction
        invariant, an island can crack while it runs, and that
        cracking is what one agent perturbing another amounts to.
        This is Question~\ref{ndo:q:crux} and
        Question~\ref{ndo:q:sr}, which are the same question, and
        it is the load-bearing gap of this part.
  ...
  \item \textbf{Approximate oracle access and graded consciousness.}
        ...
        Chapter~\ref{ch:weight} makes the second half of that
        available --- the quantum-jump unravelling is well posed over
        a finite configuration space --- and makes the first half
        harder, since by
        Proposition~\ref{wt:prop:interference} an amplitude
        attached to a rewrite carries no coherence, and the
        quantity a graded definition would want is therefore a rate
        and not an amplitude unless the presentation supplies a
        Hamiltonian.  The definition needs restating on that basis
        and has not been.
