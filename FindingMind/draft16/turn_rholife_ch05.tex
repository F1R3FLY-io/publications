...
engine that individuates one from another. Each was argued at the scale where the
...
\label{spk:prop:mcp}
The threshold presentation of Definition~\ref{spk:def:neuron} realises the monotone
...
presentation is the one at $\theta = 1$, where threshold behaviour is trivial and the
...
law of the chain rather than a modelling choice.
\end{remark}
...
The ceiling is not cosmetic. With a quantised increment it is what makes the reachable
weight-map set finite, which is the hypothesis Definition~\ref{wt:def:hilbert} requires,
and it is a case where a modelling convenience and a mathematical necessity happen to be
...
The point generalises well beyond bookkeeping, and it is the reason this chapter sits in
...
makes the reachable term set finite and the ceiling $w_{\max}$ with a quantised increment
...
classical network transmits at $m$ times. Under the normalisation of
