...
Recall from Part~\ref{part:physics} that the reversibility construction
$\GSLT \to \GSLT^\dagger$ of Chapter~\ref{sec:reversibility} makes the laws
time-symmetric.  The argument of this section strengthens that to a condition on
the \emph{decoration}: $\bwd_r(\phi) = \overline{\fwd_r(\phi)}$ for every rule
$r$ --- the backward amplitude is the complex conjugate of the forward
amplitude.
