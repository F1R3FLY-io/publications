...
it closely.
The Turn took it apart in front of you.
What is left is to bring it back, and the first thing that has to
come back is the borrowed word.
...
second reading is what this part of the book is for.
...
There is a third instance of the same move, and it goes deeper than
either of these, though i cannot get to it until
Chapter~\ref{ch:notation} has set up the vocabulary.
Modal depth is about which \emph{questions} an agent can afford.
Counting is about how many \emph{things} it can afford to
distinguish.
The third is about which marks count as the same mark --- that is,
about whether the agent has a symbol system at all in any exact
sense, or only a smear.
Goodman's conditions on notation include a requirement of finite
differentiation that behavioural equivalence, being a greatest
fixed point, is systematically silent about; and the conjecture of
Section~\ref{sec:budget} is that a budget supplies it, with the
budget itself as the separation constant.
...
line between the digital and the analogue --- which has been
treated as a fact about kinds of inscription since Goodman drew it
--- turns out to be a fact about what an observer can pay for.

That is the pivot.
Kant's conclusion survives the translation into computation --- the
thing in itself really is out of reach --- but it survives as an
economic fact rather than a logical one, and economic facts have
the property that you can do something about them, at a price, up
...
Testing a hypothesis costs --- the source programme for this book
...
result: interference cancels, and bisimulation refuses to pool the
runs that would have to be pooled.
...
One more thing has to happen before any of that, though, and it is
not a construction.
...
available.
It is also a condition we arranged.
The next chapter says how, and says it in detail, because a
metaphysics that will not show its own method is exactly the thing
the \emph{Prolegomena} was written against.
