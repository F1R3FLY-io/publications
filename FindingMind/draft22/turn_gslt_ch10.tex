...
logic~\cite{HennessyMilner1985}, Stirling's modal characterizations, and, for the
...
generated modal logic --- \emph{adequacy}, the characterization of the theory's
behavioral equivalence by satisfaction. It does not, and the reason is worth stating
...
fails --- without negation one characterizes a simulation preorder rather than
...
that its typing judgement characterize observational equivalence; one asks that it be
...
generating a behavioral characterization.
\end{remark}
...
general strictly more discriminating than the behavioral equivalence one would
...
The reader will recognize $K_j$. In a theory presented as an interaction cut, the contact
...
The modal fragment is intentionally behavioral, and this has a consequence that should
be stated plainly rather than discovered: \emph{terms with no available one-step behavior
...
can then be combined with the modal layer to express shape and one-step behavior in the
...
\section{Sort slots and the equational center}
...
\emph{equational center} $\Zc$, and the vertices of the resulting hypercube are the
elements of $\Zc$.
...
\begin{remark}[Why Barendregt needed no equational center]
...
admissible, $\Zc$ is everything, and the distinction between the raw cube and the center
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \item \textbf{The cube vertex.} Choose an element of the equational center $\Zc$. This
...
has, in the typical case, dialed out of the adequate region on the way.
