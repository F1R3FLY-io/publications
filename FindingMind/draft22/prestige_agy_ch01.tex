...
strength of choice it demands turns the factorization into a \emph{filtration}: not one
...
whether the graded factorization survives reduction --- whether an island can crack
...
We can now turn the instrument on agency. If nondeterminism is localized at cuts
...
the boundary is a modeling decision. Second, because of this, a seam where choice is
...
So the relationship between Smith's factorization and ours is not that we fill in his
...
statement is that the factorizations \emph{cross} rather than refine one within the
...
the tempting one and its failure is instructive. Read quickly, Smith's factorization
...
\section{Decorating the cuts: the factorization, graded by choice}\label{ndo:sec:graded}
...
\S\ref{ndo:sec:macro} collapse into a single labeling,
\[
...
macro-component factorization of \S\ref{ndo:sec:macro} is read straight off the decoration: the islands
are the maximal subterms whose internal cuts all carry $\bot$, the seas the cuts that carry more.

Once the cuts are labeled, the binary distinction --- $\bot$ against more-than-$\bot$ --- is
...
\begin{proposition}[The factorization is a filtration]\label{ndo:prop:filtration}
...
partition is the confluent-islands / non-confluent-seas factorization of \S\ref{ndo:sec:macro}; at
$C = \top$ the whole term is a single island. A term therefore carries not one factorization but a
\emph{filtration by choice strength} --- a family of factorizations, monotone in the threshold,
...
one term carrying one labeling, of which the strata are the level-sets: different observers are
...
$C^{*}$-island, so that Smith's thermodynamic whole nests inside our graded factorization at
...
is an island cracking as it runs} --- the $C$-factorization refining dynamically because a boundary
decision manufactured strength inside. The crux of \S\ref{ndo:sec:crux} is therefore exactly the
question whether the graded factorization is a reduction invariant. Second, $\chi$ is a semantic
...
effective approximation of the decoration, which is why a factorization defined semantically can
...
conjecture rather than a theorem, the fit that motivates the formalization:
...
angelic, fair-merge, relativized, transfinite --- is the adjoint structure of one
fibration rather than a list of cases.
\end{conjecture}

\noindent
The load-bearing check is whether the fibers carry the limits and colimits to
support both adjoints; that is where a formalization should begin.
...
universal channel homogenizes the field to the top degree --- the jumble dies.
...
a cost discipline (the phlogiston of the source program) and reach becomes finite
...
channel homogenizes the field upward; keep it and interference must be \emph{cut-triggered}.
...
formalization should resolve first. Everything above treats ``determinate interior,
...
computation are jointly asking. If rigidity is stable, the factorization is robust: a
...
developmental cascade absorbing an upstream choice into one canalized outcome. If rigidity
...
\noindent
If subject reduction holds, the graded factorization is stable. Determinate interiors
...
reindexing --- the decisive sub-question being whether the GSLT fibers carry the limits and
...
rigidity under descent --- proving the structure theorem if it holds, or characterizing
...
macro-components \emph{cross} rather than nest, and characterize each as the carrier of a
...
Proposition~\ref{ndo:prop:filtration}, establish that the $C$-factorization is a reduction invariant
...
factorization effectively certifiable.
...
apparatus, already in our bibliography --- characterizes exactly the obstruction to a global,
...
that \emph{correlated choice is a real, mathematically characterized, experimentally confirmed
...
program \cite{OSLF}, and on Smith and Morowitz's reading of the biosphere \cite{SmithMorowitz}; its
