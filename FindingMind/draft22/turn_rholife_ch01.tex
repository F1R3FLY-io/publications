...
\noindent
Part~\ref{part:machinery} built the apparatus: a category of theories, a monad
that charges for a step, a logic generated from the presentation. What it did not
build is anybody to use it. The parts that follow ask what a physics is and why a
scientist inside one would be misled; both take the scientist for granted. This
chapter builds one.
...
and sex is a defense against a predator's hypothesis language rather than a
...
--- context-labeled bisimulation \cite{Probing}. That is the ceiling R1 demands, and it is derived
...
canonical answer: the language must be adequate for context-labeled bisimulation, which is exactly
...
survey is bounded by its wealth --- together with a catalog of strategies compatible with those
...
sees things context-labeled bisimulation in the base theory does not. Chapter~\ref{ch:obsext} settles
...
for the idealized calculus, is what defines the invariants; \emph{fragments}, restricted so that model
...
factorization is a choice of which side one is standing on, and in \S\ref{sci:sec:mixed} we will take
...
$C[E]$ can then take. This is the only route to behavior. It is worth being careful here, because
...
the distinction the note turns on: \textbf{observation versus disclosure}. Behavior is what one
...
\paragraph{Behavioral modalities.} One primitive modality $\langle K_j \rangle$ per rewrite rule and
choice of redex position, labeled by the one-hole context in which the step fires. The index is not
...
modal layer its force: an action-labeled logic sees one label where this one sees as many as there
...
subject to one constraint: it must be \emph{decentralized}, manufacturing names from material a
...
depends on branching and on evaluation strategy besides. The schedule remains a modeling choice,
...
realized on a probe channel $p$ as
\begin{lstlisting}
...
A \emph{revision} is a move on the relaxation lattice: \emph{generalization} forgets a conjunct and
moves down, \emph{specialization} restores one and moves up. A scientist's trajectory is a walk
...
Let $E$ be an environment whose behavior is not eventually structural in the sense of
...
overlapping. Every point of a ball is a center of it, so no point in a neighborhood is distinguished.
...
neighboring points, and an ultrametric space supplies neither. What it supplies instead is branching,
...
We catalog rather than recommend, since which policy is best is an ecological question and depends on
...
the most classically scientific of the options: it maximizes the information a single assay returns.
...
internalizing encoding $\enc{-} : \Crho \to \rhoc$ that represents the located token stacks in the
...
So eating is a receive, death is starvation, and defense is restriction. All three are already in the
calculus.
...
\subsection{Predation is the failure of internalization to be faithful}\label{sci:S7.3}
...
Let $\enc{-} : \Crho \to \rhoc$ be the internalizing encoding and let $C[-]$ be a \rhoc\ context. If
...
that the modeling is not merely decorative.
\end{remark}
...
The prey's optimal defense in a world of predators looks, at first, unbeatable. Restrict everything.
A fully closed computation is a live $\tau$-network with no free names; no context can communicate
with it; and by the pessimism of \cite[\S1]{ClassifierNote} its observable behavior carries almost no
...
instructive way. Because name manufacture is decentralized (Remark~\ref{sci:rem:sugar}), two computations
...
needs no normalization by stack size or burn rate.
\end{proposition}
...
Smith's organization of the biosphere \cite{SmithMorowitz} distinguishes organisms that consume other
...
think that reading survives inspection, and we record why, because the temptation is to apologize for
...
corroboration of the modeling rather than as its source.
...
specialization is an attractor.
\end{enumerate}
...
On the Earth, dual capability is realized almost entirely by symbiosis: corals with their
...
rather than atoms. The restriction permits each component to specialize its access profile --- one
...
prospect and not a modeling artefact; it is the correct statement of the situation at the scope at
...
mechanism is reflection, and the interest of it is that the genetic apparatus is not modeled but
...
formed, whereas variation confined to continuations perturbs only behavior under a hypothesis that
...
metabolize, does not age, and burns no tokens while held.
...
\S\ref{sci:sec:logic} are labeled by, so genetics requires no notion of address the note did not
...
locus, and no rewrite fires there, so no $K_j$ equals it. The characterization is then immediate
...
a behavioral sequence of length $d$. Hence the perturbation induced by a swap is non-increasing in
...
hole is at a communicating pair, which is the characterization of the labels.
\end{proof}
...
\]
and the swap removes the pair $(\,c!(P),\ \texttt{for}(x \leftarrow c)Q\,)$ in favor of a father's
...
context-labeled modality can exercise, and what Lemma~\ref{sci:lem:exposure} was about: a computation all
...
It also repairs Proposition~\ref{sci:prop:preferredprey}, whose original statement had to normalize by
...
There is therefore no behaviorally silent variation that is also metabolically silent, and selection
...
Internal variation accumulates without behavioral consequence and becomes consequential exactly when
...
observable variation is later drawn, so the defense is two-staged: accumulate where it cannot be seen,
...
across every decentralized scheme --- which is why we may remain agnostic about which one is taken.
...
\begin{proposition}[Decentralization makes recombination generative]
...
soma and germ turns out to be, the weight must be renormalized or lineages accumulate without bound.
\end{remark}
...
spatial--behavioral logic supplying the keys, and a weight map, to a weighted coalgebra
...
the desugaring of \texttt{new} & free, subject to decentralization
...
We used to regard the smallness of that list as the principal evidence that the modeling choices were
...
does not determine a rate of five, nor $1/5$, nor $e^{-5}$; every one of those is a further modeling
...
context-labeled one and is a congruence relative to a class of admissible contexts
...
generated logic --- structural layer included --- exactly when it preserves context-labeled
...
one. This is the proximate/ultimate distinction of behavioral ecology: viability is the ultimate
...
\subsection{A division of labor}\label{sci:S17.1}
...
wrong in a way worth recording rather than quietly repairing. If the front end is modeled as adjoining
...
that the structural layer strictly refines context-labeled bisimulation. An argument cannot rest on an
...
environment; it is adequacy for interaction \emph{as such}, since context-labeled bisimulation is the
...
domains which have historically resisted mathematization have tended to be of just this kind is not
...
hypothesis language must be adequate for context-labeled bisimulation, on pain of being blind or
...
What the note does claim is narrower and, we hope, more durable: that once learning is modeled as a
...
relaxation lattice that is optimal in the ecological sense --- maximizing expected residual life
...
isolated. Delete the \texttt{where} clause from \texttt{Mate} and they hybridize; the offspring
