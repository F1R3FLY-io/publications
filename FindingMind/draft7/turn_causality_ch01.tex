...
rewrites carry a semiring value
  & a decoration: nondeterminism, cost, probability or amplitude,
...
Part~\ref{part:choice} will describe a different ladder --- the
hypercomputational fibration, in which each stage extends the one below
with an oracle, and the physics at each stage is strictly richer than the
one below.
...
consideration.
The ladder of Part~\ref{part:choice} moves \emph{up}, adding computational
...
Chapter~\ref{sec:weighted} introduces the decoration --- one map into a
semiring --- and locates the least-action principle in its tropical
instance.
...
connects to the quantum analogue of the Gillespie algorithm, and states
