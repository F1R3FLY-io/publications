% ==================================================================
% ch04 -- from the research note "Enzymes and Individuals"
% ==================================================================
\chapter{Enzymes and Individuals}
\label{ch:eng}
...
work. Drop it --- let each community's tokens be its own flavor, as the
...
single token flavor, so its conservation law is the flat statement that a
...
Give each community its own token flavor. Adjoin enzymes between flavors
and weight each enzyme by its dissipation $\theta$. The resulting weighted
...
We claim: that the enzyme is derivable as a fourth access profile rather
...
enzymes make arbitrage a property of a marking (\S\ref{eng:sec:fork}); that
...
composition; the factorization theorem, that namespace disjointness is
...
From \cite{vtok}: conversion systems, enzymes, rates with
...
%% ===================================================================
\section{Flavors, enzymes, and a fork in the conservation law}\label{eng:S3}
\label{eng:sec:fork}

\subsection{Communities are flavors}\label{eng:S3.1}

\begin{definition}[The flavor of a learner]\label{eng:coloured-learner}
A \emph{flavored learner} is a learner $\Lrn$ in the sense of
\S\ref{com:S3} of \chComp together with a distinguished signature
$c(\Lrn) \in \Sig$, its \emph{flavor}: the signature at the head of every
stack held at a name in $\Met(\Lrn)$.
\end{definition}

The flavor is not new apparatus. \cite{CARho} already replaces one
...
Taking a community's flavor to be its own signature is the natural
...
\begin{definition}[Enzyme]\label{eng:def:engine}
An \emph{enzyme} between flavors $a$ and $b$ is a persistent process
holding two located reserves, one of each flavor, and offering a linear
...
\begin{remark}[On the word, and the one we did not use]
\label{eng:rem:nomenclature}
We flirted with \emph{engine}, and it nearly won. An engine converts between two
reservoirs at a rate, dissipates as it goes, and has an efficiency; the vocabulary
arrives pre-loaded with the quantities this chapter needs. The payoff was real and it
was specific --- Remark~\ref{eng:ecological-pyramid}'s observation that a trophic ratio
of one in ten is a toll of $\theta = \ln 10$ is a thermodynamic statement, and
\emph{engine} is the word that makes it audible.

We dropped it because that payoff is the nineteenth century's, and so is everything
else that comes with it. Engines are Carnot's. Reservoirs, efficiency and heat death are
the furniture of the physics that produced them, and the furniture arrives whether or
not it was sent for. This book's move is that the ledger is informational before it is
thermal: erasure is what costs, and $\theta$ is a rendezvous toll rather than a
temperature. To name the object after a steam engine is to import a settled picture at
precisely the point where the reader should be unsettled. The interlude that opens
The Turn has its narrator distrust the word \emph{inspiration} for smelling of the
nineteenth century, of laudanum and s\'eance. \emph{Engine} smells of the same decade,
and is worth distrusting for the same reason.

\emph{Enzyme} is the twentieth century's word for the same job, and the better fit
besides. An enzyme is specific to a substrate pair; it persists while what it binds does
not, which is exactly Remark~\ref{eng:persistence-load-bearing}; and its one
distinguishing degree of freedom is its rate, which is the only property separating the
fourth row of Table~\ref{eng:tab:profiles} from a source and from prey. It also settles
a debt. In a reaction network the no-arbitrage condition around a cycle is the
Wegscheider condition for detailed balance, and the potential whose existence it is
equivalent to is a chemical potential. The virtual token has a second name, and chemists
have been using it since 1901. That identification is ours and we have not checked it
against the chemical-kinetics literature; a reader who knows that literature should
treat it as a conjecture with a citation attached rather than a result.

What is surrendered is the market register --- Coase, mycorrhizal trade, and the
financial combinators the reaction language is built from. Those are kept as analogies
and the noun is left biochemical, on the grounds that a reader who has just been told
tokens have flavors is not helped by being told they are burned.
\end{remark}
...
things. \chComp conserves tokens \emph{per flavor}: $\Theta$ is
fixed and never minted. \cite{vtok} conserves the scalar $\nu$: an enzyme
consumes one $a$ and releases $r$ units of $b$, which is not per-flavor
conserving at all. One must give.

\begin{itemize}[itemsep=3pt]
\item[(i)] \textbf{Enzymes hold inventory.} A swap is against stock, so
per-flavor totals \emph{are} conserved and $r$ is the ratio at which
inventory is exchanged. The cost: enzymes deplete, $r$ moves with the
marking, and $\ell$ becomes a function of state.
\item[(ii)] \textbf{Conserve only $\nu$.} Flavors interconvert at fixed
rates and $\Theta_\nu$ is the invariant. This matches \cite{vtok} exactly
but abandons the per-signature ledger the scientist note runs on.
\end{itemize}

We take (i). It preserves the more specific law, it keeps enzymes mortal
(an enzyme that runs out of one flavor stops being an enzyme, which is what
...
of the enzyme reserves. Consequently the Hodge splitting
...
block~(4), exhibits a triangle of constant-product enzymes whose cycle
yield is exactly $1$ at the balanced marking and departs monotonically as
one enzyme is drained; six trades of ten units carry
...
\cite[Prop.~3.4]{vtok} is realized rather than postulated.
\end{remark}
...
Under (i) the observable that sets an enzyme's rate reads the enzyme's own
...
%% ===================================================================
\section{The enzyme is an organism}\label{eng:S4}
...
The enzyme is a third way, and it is obtained by taking the same four
...
\textbf{Enzyme} & \textbf{yes} & \textbf{no} & \textbf{per flavor}
                & \textbf{in rate} \\
\bottomrule
\end{tabular}}
\caption{The enzyme as a fourth access profile. It is source-like in
persistence and non-exclusivity, prey-like in exhaustibility, and unique
in that what adapts is not its behavior under attack but its
\emph{terms}.}
\label{eng:tab:profiles}
\end{table}

\begin{proposition}[The enzyme is derived, not imported]\label{eng:engine-derived-imported}
...
per-flavor exhaustibility; and computing the rate from those reserves
...
\begin{remark}[What the enzyme eats]\label{eng:rem:eats}
\cite[\S6]{vtok} introduces dissipation $\theta(a{\to}b) \geq 0$ as a
modeling parameter and reads the resulting inequality as the second law.
...
enzyme's own stack, because something has to fund the enzyme's continued
existence and nothing else is available. The second law and the enzyme's
metabolism are the same fact. An enzyme whose traffic falls below its burn
rate starves, exactly as a scientist does.
\end{remark}

\begin{remark}[Not a decomposer]\label{eng:decomposer}
It is tempting to call the enzyme a decomposer, on the grounds that it
lives on what others cannot metabolize. The better analogue is the
...
level down: they name the enzyme's reactions, not the enzyme.
...
settlement optimization is, in this reading, a reduction of the second-law
loss along every edge of the enzyme graph, and hence --- by
...
loss of generality, because imperfection can be modeled as an additional
...
Proposition~\ref{eng:prop:wlog} is a statement about behavior, not about
...
attributes to the enzyme. This is the third instance in the series of one
...
reading    & forwards and retains           & grade-one access to all
                                              traffic \\
converting & swaps flavor against inventory & the enzyme of
                                              Definition~\ref{eng:def:engine} \\
\bottomrule
\end{tabular}}
\caption{Media classified by behavior. The enzyme is the top rung, so the
fourth access profile of Table~\ref{eng:tab:profiles} is not a separate kind of
thing but the most behaviorally elaborate interposition.}
...
Under fork~(i) the per-flavor totals are conserved by construction. A
...
quote path of enzyme $e$. The price code is not detecting an abstract
...
carrier of names. Second, a rich $C$ is itself realized by interposition,
...
Table~\ref{eng:tab:ladder} classifies media by \emph{behavior}; Definition~
\ref{eng:def:three} classifies them by \emph{ownership}. Individuation lives on
the second axis only. An enzyme may be internal or boundary, and which it
...
%% ===================================================================
\section{The enzyme graph}\label{eng:S6}
\label{eng:sec:code}
...
\S\ref{com:S5} of \chComp reads ecological relations off it. The \emph{enzyme
graph} $\Gamma$ has flavors as vertices and enzymes as edges; its edge data
is the log-rate $\ell$ and the dissipation $\theta$.

They are related but not identical. An enzyme is a shared name, hence a
...
Two learners linked by an enzyme have $k_{\mathrm{met}} \geq 1$. Hence by
the factorization theorem of \S\ref{com:S4.2} of \chComp they are not
probabilistically independent, and by
Proposition~\ref{eng:prop:nomet2} below their overlap cannot be positive-sum in
tokens.
\end{proposition}

\begin{remark}[The vertices are the atomic flavors]\label{eng:vertices-atomic-colours}
By \cite[Prop.~4.4]{vtok} a valuation is a homomorphism out of $\Sig$ and
induces no enzyme; in particular no enzyme converts a compound authority
...
are vertices of no enzyme. Throughout, $\Gamma$ is taken on the atomic
flavors, and $\bt$ is computed there.
\end{remark}
...
$\Pi_{\Cyc}$ for the two projections, and for an enzyme $e$ let $\Reff(e)$
...
Let the true rates be exact and let a single enzyme $e$ be corrupted by
...
equivalently $\Reff(e) = 1$, which for unit conductances characterizes
bridges.
\end{proof}

\begin{theorem}[The code's total strength is $\bt$]\label{eng:thm:foster}
For any connected enzyme graph,
\[
...
of independent arbitrage cycles, and the geometry of the enzyme graph
decides how that budget is distributed across enzymes. Dense regions get
...
enzyme, $\bt = 12$ and $\sum_e(1-\Reff) = 12.000000$, with the bridge
contributing $0$ and each cluster edge contributing $0.6$.

\begin{proposition}[Series enzymes are confusable]\label{eng:prop:series}
Two corrupted enzymes $e, f$ produce the same syndrome direction iff
$\Pi_{\Cyc}\mathbf{1}_e \parallel \Pi_{\Cyc}\mathbf{1}_f$, which holds iff
$e$ and $f$ lie in exactly the same cycles. Hence enzymes in series --- a
chain through vertices of degree two --- cannot be told apart by the code,
while enzymes in parallel can.
\end{proposition}

In $C_6$, where $\bt = 1$, all $15$ edge pairs are confusable: a
one-dimensional cycle space detects that \emph{something} is wrong and
never what. In the barbell, no pair is. The code localizes exactly when the
ecology is dense.

\begin{corollary}[Density buys detection]\label{eng:cor:density}
In a $d$-regular enzyme graph on $n$ flavors the mean detection strength is
...
\end{tikzpicture}}
\caption{The price code on a barbell enzyme graph. Twenty intra-cluster
enzymes each carry $0.6$ of detection strength; the bridge carries none.
The total is $\bt$ exactly (Theorem~\ref{eng:thm:foster}). A misquoted rate
inside a cluster produces a syndrome and is localizable; a misquoted rate
...
enzyme operations. Along a path of length $d$ in the cost metric the
...
Predation is an enzyme: it converts prey-flavor into predator-flavor, and
...
$r = 1/10$, i.e.\ $\theta = \ln 10 = 2.303$ per enzyme. The energy pyramid
is then exactly the exponential decay of $\nu$ along a path of enzymes,
...
\paragraph{Factorization survives and is sharpened.} An enzyme is a
...
Under (i) the total token supply of each flavor is conserved by every
enzyme operation, and by \cite[Prop.~5.1]{vtok} the total $\nu$ is
...
raises it. In one flavor the asymmetry is easy to miss because the
transfers that raise the aggregate are exactly the ones a predator has no
reason to make. Flavors make it conspicuous, because dead stock is worth
...
and that the dilution is regressive. Enzymes reinstate that competition
across boundaries that rooting had made clean: an enzyme is a shared name,
...
modularity; an enzyme sells it back at the price of the toll. Modularity
...
Merger is now sharply distinguished from composition-with-enzymes: merger
identifies namespaces, an enzyme prices them, and \S\ref{eng:sec:indiv} says
when each is available. Succession --- a change of the grading vector
forced by source exhaustion --- acquires a second mechanism: an enzyme's
...
Removing an enzyme from a cycle reduces $\bt$ by one and redistributes
$\Reff$; removing the last enzyme on a cut disconnects $\Gamma$ and
...
channel \cite{compaut} is parameterized on --- error rate on transmission
...
A region $B$ of the enzyme graph may close over its namespace --- may be
...
an individual of $5$ flavors, the grid $85$, the $3$-regular graph $160$,
...
The intuition that centralization buys compression and loses error
...
A merged region has one flavor, hence no enzymes internal to it, hence no
cycles in $\Gamma$ restricted to it. By Theorem~\ref{eng:thm:foster} its total
detection strength is zero. Centralization does not degrade the price code;
it deletes it.
\end{proposition}

\begin{proposition}[Merger destroys independence]\label{eng:merger-destroys-independence}
By the factorization theorem of \S\ref{com:S4.2} of \chComp, disjointness is
...
no enzymes to pay; no arbitrage, since there are no cycles to carry it; and
...
\begin{remark}[A second criterion, and it is not this one]
\label{eng:rmk:two-criteria}
Everything in \S\ref{eng:S8} draws the boundary \emph{economically}:
close where repair is cheaper than exposure.
That is a criterion about who pays for what, and it is the only one this
chapter supplies.
There is a second, and Chapter~\ref{ch:apr} --- in the Prestige, and a
long way ahead --- supplies it: close where no internal cut carries
observable choice, drawing the line around the regions that are
determinate in the sense of Definition~\ref{ndo:def:aperture}.
Chapter~\ref{ch:agy} then shows that neither criterion implies the
other.
Being worth owning and being determinate are independent properties, and
most organisms have the first without the second: an animal is an
economic individual whose interior is full of live races.

Read forward, this says that the individuation fixed point of
\S\ref{eng:S8} has a companion it does not know about, and that the
fixed point's several solutions are not the only slack in the account.
Read backward from Chapter~\ref{ch:agy}, it says that the semantic
criterion, which that chapter cannot cash on its own, has an economic
partner already worked out here.
Neither chapter closes the gap.
It is named in both so that a reader does not have to notice it alone.
\end{remark}
...
by replication of $r^\ast$-sized units composed with enzymes. Each
...
organism / organism & $\mid$ + enzyme & $\Met$ (transfer), $\Src$, $\Prb$, $\Mat$
& kin, trade, culture \\
...
\emph{arena} as the candidate fifth type; \chGame realizes the arena
...
The skills share one budget, so within the organism there is one flavor, no
enzymes, and $\bt = 0$: \emph{there is no market inside an individual}. This
...
claim about flavor change: no rung-five medium, no conversion, allocation
...
individual is where the token flavor changes \emph{and} where the medium
...
rates, hence composition with enzymes, and the framework predicts what is
...
any contact between them}, which is automatization and the critical period,
with a closed form.
...
Life elsewhere may not be organized around the same chemistry, and in this
framework that is not a matter of expense but of topology.

\begin{proposition}[No enzyme, no numeraire]\label{eng:no-engine-no-numeraire}
If no enzyme links the flavors of two learners then $\Gamma$ is
...
enzyme between their flavors. Then no token can pass between them. But by
...
An enzyme requires a rendezvous, hence a shared name. Two ecologies rooted
...
construction. So the establishment of a first enzyme between ecologies is a
...
catalog can be re-read as a portfolio problem. We flag this and do not
pursue it.
\end{remark}
...
flavors is the ratio of marginal inquiry-yield per metered unit, which
...
opposed to the most natural; that inventory enzymes are the right reading
...
Proposition~\ref{eng:prop:marking}; any treatment of strategic behavior by
enzymes, which is where biological market theory \cite{noe} would enter.
...
Make $r^\ast = \min_i r^\ast_i$ precise and determine the minimizing $i$ at
...
\item \textbf{Dynamics on the enzyme graph.} Under (i) the marking drifts,
so $\bt$ is fixed but $\Reff$ moves and enzymes can die. Run the composite
of \S\ref{com:S10} of \chComp with enzymes under a Gillespie scheme and ask
whether the ecology self-organizes toward an expander.
...
$\Merge$ identifies flavors; $\Pi$ projects rates. Both take a conversion
...
\section{Appendix: An enzyme, in rholang}\label{eng:S15}
\label{eng:app:rho}
...
\begin{quote}
\small
\emph{Listing withheld from this printing.} The source of \texttt{engine.rho} was not committed alongside the other artefacts of this part and is not reproduced here; the construction it realizes is given in full in \S\ref{eng:S4} and \S\ref{eng:S5} above.
\end{quote}
...
enzyme's own slot on every swap, which is Remark~\ref{eng:rem:eats}: $\theta$ is
...
enzyme: $|V| = 10$, $|E| = 21$, $\bt = 12$. An exact cochain has syndrome
...
\paragraph{(4) Inventory enzymes.} A triangle of constant-product enzymes,
each with reserves $(100,100)$, has cycle yield exactly $1$. Draining one
enzyme in steps of ten carries the yield through
\[
