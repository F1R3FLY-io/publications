...
factorization of a term into determinate islands and open seas stands or falls on one
...
by normalization. Here the declarative side is the mathematical theory of choice --- a
...
a scheduler is a realizer of a choice principle; running the schedule is
normalization.}
\end{quote}
...
reduction preserve typing --- says here that running a schedule does not come to realize
...
The program is stated to be discharged, not completed. What follows lays out the two
...
and when the family is \emph{presented} --- each fiber given as a list, as a finite bag of
...
whose later fibers depend on earlier selections. $\DC$ is the type of a scheduler with memory and
...
\cite{martinlof}); its realizer is a pure term. The set-theoretic, non-constructive $\AC$ is a
different type, realized only by programs with classical control. The two are not rival accounts of
...
for decades, and in every case the realizer of a choice principle is a program whose extra power
...
\item \emph{Bar recursion (Spector \cite{spector}).} Spector's bar recursion realizes the
double-negation shift and hence countable choice and classical analysis, as a recursion schema in an
extension of G\"odel's System~T. A choice principle is realized by a \emph{specific recursion operator}
--- a term with a definite computational shape, a scheduler with backward-inductive lookahead.
\item \emph{Computational content of $\AC$ (Berardi--Bezem--Coquand \cite{bbc}).} A realizer of the
...
interprets $\DC$ using a ``quote'' instruction and a clock --- the realizer is a program with a
...
contention is a race at a channel and the realizer is a scheduler over a store. Assembling them there
is the program.
...
half of that analogy --- the proofs-as-processes program of Abramsky and of Bellin--Scott
\cite{abramskyPaP,bellinscott}, and the propositions-as-sessions of Caires--Pfenning and Wadler
\cite{cairespfenning,wadler} --- is exactly where the present opening lies. In that program a
...
We sketch the term assignment. As in \cite{consensus} this is a program, stated to be discharged, not
a completed system.
...
of list orderings is the \emph{term} inhabiting the aperture's choice-type, and normalization --- running
...
is \emph{proof-irrelevant}: several pairings inhabit it but all normalize to $\rbisim$-equal results, so the
...
type (the $f$-zone) says no resolution exists. This is the fairness guarantee, internalized as a typing property.

\emph{Subject reduction}: running the schedule preserves the type --- the scheduler does not, by reducing,
come to realize a \emph{stronger} choice principle than it was typed for. Operationally: \emph{scheduling
...
\noindent
If subject reduction holds, the graded factorization of \chAgy is stable: determinate interiors stay determinate
...
\emph{Stage 3 --- optimization.} As in \cite{consensus}, extraction yields a naive scheduler; bisimulation-preserving
transformations reduce overhead while preserving the realized choice-type, and the phlogiston metering of the
rholang runtime serves as the objective, now with the added invariant that optimization must not raise the
...
strength order made into types; interference is a realizer supplied from a stronger type than the lower system's
...
extraction by normalization. The notes compose rather than merely resemble: Coalition Logic over the communication
...
nondeterminism. A mobile process is fully accounted for only by both, and the present program is the missing half.

\emph{To realizability and selection functions \cite{martinlof,spector,bbc,krivine,escardo}.} The technical
grounding: choice principles already have realizers whose power tracks their strength. The contribution sought here
is to carry that into a concurrent, store-based setting where the realizers are schedulers and the contention is a
...
Weihrauch/Turing degree equals its altitude in the tower of \S\ref{sec:fibration}: type, degree, and altitude are one.
\item \textbf{The hinge.} Resolve Question~\ref{ndo:q:sr} --- whether scheduling can climb the stalk --- proving stability
of the factorization if subject reduction holds, or characterizing perturbation as its definite failure mode if it does
not.
...
\item \textbf{Mechanisation.} Formalize the logic of choice, the scheduler calculus, and the term assignment in a proof
