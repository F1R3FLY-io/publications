...
\begin{description}[leftmargin=2.2em,itemsep=2pt]
  ...
  \item[two introductions $\Kp(I,C_{\mathrm p})$ and
    ...
    environment). The factoring is Milner's~\cite{MilnerPolyadic1993}: in
...
introduction $\Ke$ has no surface and the environment continuation
carries no residual, so that the argument is its own continuation and
pseudo-application is ordinary application (Section~\ref{sec:examples}).
...
$C_{\mathrm p} = \sgn{M}{s_{\mathrm p}}$. The argument has no subject and
no residual, so $\Ke$ is \emph{degenerate}: no interaction surface, the
argument being its own wrapped environment continuation
...
explicit gate. The interaction surfaces $I$ (of the receiving
introduction) and $J$ (of the eliminand) are the parts that must match,
and we demand a nearness predicate
\[
  \near(I, J)
\]
...
rho and $\pi$, and interface agreement in the interaction categories. In
the rigid case $\near(I,J)$ is discharged for free by the
context $\Kctx{S}$ --- being in the hole \emph{is} being near --- so the
predicate unifies the regimes: nearness is always the capability
