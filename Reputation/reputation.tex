...
Fix a population $\mathcal{A}$ of agents, each associated with a verification key for a digital signature scheme. We write $\Sig(A)$ for the identity of $A \in \mathcal{A}$ in the term position of a judgment, understood as a commitment to a verification key together with the right to author signed messages.
...
\begin{align*}
\modD{a}^{B}\phi &\quad\text{``some attestor $B$ directly attests an $a$-transition into a state satisfying $\phi$''} \\
\modW{a}^{B}\phi &\quad\text{``some attestor $B$ witnessed an $a$-transition into a state satisfying $\phi$''} \\
\modH{a}^{\chi}\phi &\quad\text{``the hearsay chain $\chi$ supports an $a$-transition into a state satisfying $\phi$''}
\end{align*}

Modal strength is partially ordered:
\[
\modD{a} \;\succeq\; \modW{a} \;\succeq\; \modH{a}^{\chi}
\]
