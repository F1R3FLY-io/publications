...
\begin{lstlisting}
// A source (Def. 30): rate-limited by being a single contract, non-exclusive
...
def Source(s, q, reserve) = {
  for (ret <- s) {
    match reserve >= q {
      ...
      false => { ret!(0) }                  // exhausted; cf. Prop. 36
    }
  }
}

// Reclaim - debit - reissue (Def. 12).  Between the receipt and the send the
// stack is a message in flight: metabolism is a race condition (Rem. 13), and
// the metabolic channel m is exactly the vulnerability of Thm. 16.
...
// was born knowing.  By Prop. 32 its environment is a source, so its science
...
// -- it does not reduce, does not age, burns nothing (Def. 38).  Dropping it
// with * is what makes it a phenotype, and what starts the meter.
//
// The guard enforces Prop. 50: a parent may not endow below the outlay its
...
// three loci; crossover proper (Sec. 11.4) splits a guard from its body
// inside an allele and carries the reading-frame condition of Def. 42.
...
// recombination to compute.  Strictly cheaper -- and by Prop. 37 it presents
...
new sun1, sun2, mate in {
  ...
  // of Sec. 12.2 instantiated as two loci rather than as a distribution.
    Source(sun1, 1,  60000)
