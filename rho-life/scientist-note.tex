...
\begin{abstract}
We model a learner as a computation. Since computations are ontologically isolated, the environment
of a computation is other computations, and what a learner can come to know of its environment is
bounded above by the context-labelled bisimulation class of that environment. We therefore fix the
model of computation as the \rhoc\ calculus and take the hypothesis language to be the logic that the
OSLF family of algorithms generates from the \rhoc\ calculus' own presentation --- namespace logic --- so
that hypotheses are grounded in the ontology the computational model generates rather than imported
from outside it. The structural predicates of that logic are admitted, and they are what equip the
learner with senses. Hypotheses are installed operationally, as \texttt{where}-clause guards on
receipts, so that an assay is a receipt and a confirmation is a rendezvous. To motivate the learner we
gamify: tokens are conserved and never minted, the learner has a bounded internal supply, further
stacks lie distributed through the environment including inside other computations, and breaking a
computation open to harvest its stack is the analogue of eating. Three results organise the note.
First, because \rhoc\ is Turing complete we may work in the image of the translation of cost-accounted
\rhoc\ back into \rhoc, and in that image predation is nothing more than a receipt --- so the ecology
lives precisely in the gap between the cost-accounted calculus and its image, where the erasure fails
to be faithful against contexts outside the image. Second, because harvest ends the prey's
computational life, each specimen affords exactly one measurement, and hypotheses about individuals
are therefore worthless: the destructive character of the assay \emph{forces} the hypothesis language
to be a logic of namespaces. Third, a closed computation cannot forage, so metabolic viability
lower-bounds observability, and perfect concealment is available only to the starving. We close with
distributions over ecologies --- a microcanonical ensemble in which science turns out to be a phase
rather than a universal advantage --- an audit separating what the assumptions force from what they
leave free, and an analysis of whether the assignment of ecologies to cost-accounted graph-structured
lambda theories is functorial. It is not, quite; the failure is instructive.
\end{abstract}
...
We are claiming that a modest list of commitments --- the \rhoc\ calculus, the logic it generates,
cost accounting as a monad, conservation of tokens, and harvest as the means of replenishment ---
determines almost all of the apparatus one would otherwise have to invent in order to model a learner
that does science. The interesting sections below are the ones in which we discover we had no
choices left. Section~\ref{sec:audit} is an explicit audit of which parts of the construction are
forced and which are free parameters, and we recommend the reader who suspects us of building a
machine to fit a story turn there first.

We are \emph{not} claiming to have built a learning algorithm, to have proved a convergence theorem,
or to have said anything about the statistical efficiency of the resulting learner. The revision
policy --- how the scientist walks the lattice of hypotheses --- is left open on purpose, and it is
one of only three genuinely free parameters we find. Nor do we treat reproduction: harvest here ends
a computational life and nothing is born. That is a deliberate scoping decision and the subject of a
companion note.
...
calculus, the OSLF family of algorithms \cite{OSLF} emits a logic, and it is the same logic whether
one is classifying knot diagrams or hunting for tokens. What is new here is the reading, the game,
and three consequences of the game that we did not anticipate.

\subsection{Why an adequate logic, and why this one}

The choice of hypothesis language is not a matter of convenience, and it is worth saying why before
any technical development.

A learner separated from its environment by an interaction cut has exactly one channel of access:
what interaction reveals. It follows that the finest thing it could ever learn is the environment's
behaviour up to the equivalence induced by the contexts it can build --- context-labelled
bisimulation in the sense of \cite{Probing}. A hypothesis language chosen from outside this setting
will be wrong in one of two ways. If it is too coarse, there are distinctions the learner can observe
but cannot state, and the learner is blind in a way its own faculties do not require. If it is too
fine, the learner will spend its life chasing distinctions that do not exist --- differences between
environments that no experiment could ever separate. Adequacy is exactly the condition ruling out
both failures at once: the formulae separate precisely what interaction separates, because formulae
denote bisimulation equivalence classes.

This is why the logic must be generated from the same presentation that generates the calculus, and
not designed. A designed logic has no reason to be adequate. A generated one is adequate whenever its
