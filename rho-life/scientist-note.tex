...
$\langle K_j \rangle$ of \S\ref{sec:logic} is a context around the \emph{redex} --- everything in
...
become enabled is indistinguishable from one that never will. This is Proposition~\ref{prop:trichotomy}'s
...
\S\ref{sec:repro}. The second is what makes the complementary guard pair of
Definition~\ref{def:assay} necessary rather than fussy: without the negated guard, clause 2 says a
...
\S\ref{sec:logic} are labelled by, so genetics requires no notion of address the note did not
...
We flag it here and repair it in \S\ref{sec:pairs}, where the repair turns out to be a discipline
...
It also obtains something we owed. Proposition~\ref{prop:locivlabels} observed that loci properly
...
logic can state, and the transport of \S\ref{sec:transport} is well defined.
\end{proposition}
\begin{proof}[Proof sketch]
Immediate from Proposition~\ref{prop:locivlabels}: the discipline admits exactly the loci whose
...
\qquad j, j'' \text{ at out-type positions},\ j' \text{ at an in-type position},
\]
with all three positions on channels the classification of Definition~\ref{def:chantypes} marks as
...
\S\ref{sec:lattice} carries term structure to formula structure, so a swap at a term-context $K_j$
induces a revision at the formula-locus $K_j$ corresponds to --- and by
Proposition~\ref{prop:alignment} the swap is at a $K_j$ in the first place only because the
