...
\lstdefinelanguage{rholang}{
  morekeywords={new,in,for,contract,match,where,or,and,not,true,false,Nil,nu,H,select,if,else},
  sensitive=true,
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]",
...
\title{\textbf{Learning to Play}\\[2mm]
\large Noughts-and-crosses as an ecology of mortal scientists:\\
...
\begin{abstract}
A companion note \cite{scientist} models learning as the loop the scientific method
describes, run by a computation with a finite and conserved token supply in an
environment made of other computations. Its worked ecosystem exhibits every
construction of the framework as running code, and demonstrates none of the
framework's reasoning: its hypotheses are arithmetic predicates on quanta, so
nothing in it ever learns anything a reader would recognise as content.

This note supplies the missing demonstration by mapping a problem space whose
correct answers the reader already holds. We take noughts-and-crosses,
adversarial and solved, and build it as an energy landscape: an arena that
holds no stack and merely routes, a stationary practice partner that is a
source in the trophic sense, and a population of scientists whose hypotheses
are formulae in the spatial--behavioural logic that the OSLF family generates
from the presentation of the \rh{} calculus \cite{classifier}. Three things fall
out of the mapping rather than being put in. A move is an assay, so playing and
experimenting are one act. Minimax is an alternating modal formula, so the
companion note's asymmetry between existential and universal modalities predicts
that attack is learned before defence. And a fork---two threats at distinct
squares---is a graded separating conjunction at $k=0$, so the concept that
game-playing programs normally hand-code is a term the generated logic already
contains. That last demand dictates the board encoding: the squares, not the
lines, must be the parands.

We then measure. A seven-rung ladder of hypotheses is priced by the modal depth
of the formula each installs, and both the outcome distribution and the expected
metering are computed exactly over the game's $255{,}168$ plays. Three results
are worth the price of the exercise. The spatial rung---the one a purely
behavioural logic could not state---returns $+0.0102$ of win probability per
metered unit, the best marginal return anywhere on the ladder and some five
hundred times that of the modal rung below it. The exact characteristic formula
$\chi$ is \emph{dominated}: against fallible prey it wins less often than the
heuristic rung beneath it while costing half again as much, which is the
companion note's ``yield, not truth'' remark arriving as a number rather than a
caveat. And a lineage whose theory is deep but whose opening is fixed cannot
repair the opening by deepening; a crossover at the opening locus invades at
identical cost, which is confinement, and its escape, measured on a real
hypothesis space. We close on a prediction the conservation law forces: a
population that solves its game stops eating. A solved science is a dead
ecology.
\end{abstract}
...
\subsection{Why a worked example, and why this one}

The companion note \cite{scientist} asks what it would take for a computation to
...
\S\ref{sec:measure} reports the numbers and Appendix \ref{app:repro} says how to
regenerate them.

\begin{remark}[The magic square comes free]
...
\subsection{What the mapping is supposed to show}
...
\item[A move is an assay.] One cannot observe an opponent without playing, and
  the position afterwards is not the position before. \S1.1 of \cite{scientist}
  argues that observation is action; here it is not an argument but the rules of
  the game.
\item[Minimax is an alternating modal formula.] $\langle K\rangle[K']\langle
  K''\rangle\cdots$ reads ``I have a move such that for every reply I have a
  move such that\ldots'', which is minimax written out. Proposition 6 of
  \cite{scientist} then has something to say about which half of the game is
  cheap to learn.
\item[A fork is a separating conjunction.] Two threats whose completing squares
  ...
  separation at $k=0$ is exactly disjointness of interface. \S\ref{sec:board}
...
a choice, defended in \S\ref{sec:price} and varied in \S\ref{sec:measure}, and
...
We recapitulate only enough to orient a reader who has not just read
\cite{scientist,classifier}, and refer to those notes for everything else.
...
namespace logic \cite{nslogic}. The classifier note \cite{classifier} adds two
features we use: a greatest fixed point $\nu X.\varphi$, and \emph{graded
separation}
\[
P \models \varphi \mid_k \psi \iff P \equiv \Hq \vec{a}.(Q\mid R),\quad Q\models
\varphi,\ R \models \psi,\quad |\,\mathrm{fn}(Q)\cap \mathrm{fn}(R) \cap \vec a\,|
= k ,
\]
which splits a term under restriction while counting the names the two halves
share. Everything this note does that a modal logic cannot do is done with
$\mid_k$.

\paragraph{Hypotheses are guards.} The \texttt{where} clause of the \rh{}
calculus variant \cite{gslt} puts a formula in operational position:
...
Proposition 12 of \cite{scientist}---\emph{confinement}---says a walk whose
...
the image they are messages. The ecology lives in that gap, and Theorem 24 of
\cite{scientist} identifies the predatory contexts as exactly the non-image
contexts holding a metabolic name. A \emph{game}, in the technical sense of
Definition 25 there, is a choice of admitted context class $\mathcal{A}$ with
$\llbracket \Cost(\rh)\text{-Ctx}\rrbracket \subseteq \mathcal{A} \subseteq
\rh\text{-Ctx}$.

\section{The world}\label{sec:world}

\subsection{Three kinds of thing, and only one of them holds tokens}
...
That distinction is what keeps \S16 of \cite{scientist} intact. The objection to
...
\begin{observation}[The arena is the admitted context class]
Definition 25 of \cite{scientist} leaves $\mathcal{A}$ as a parameter and
...
completed a line against $P$, and by Theorem 24 those are non-image contexts. So
...
\begin{remark}[Capability gating, concretely]
\cite{scientist} distinguishes the capability-gated game---contexts that
...
\subsection{The wall is a source, not a prey}

A world in which the only food is other players is a world of pure
heterotrophs, and by \S11 of \cite{scientist} such worlds pay an unbounded
...
\cite{scientist}, \S11.2: it is a persistent emitter, rate-limited by being a
...
\subsection{Conservation, stated for this world}
...
\ref{app:code} can be audited for this by inspection.

\section{The board: the logic dictates the encoding}\label{sec:board}
...
\paragraph{The encoding.} The board is a parallel composition of nine
\emph{cell} processes, one per square, each holding a resting send on that
square's channel carrying its mark; and eight \emph{line} processes, one per
line, each persistently reading its three cells and, when the line holds two
marks of one player and one empty square, reinstating a standing \emph{offer} on
the empty square's channel. A threat is therefore not something computed on
demand. It is a piece of the term.

\begin{definition}[Threat, fork]\label{def:fork}
For a player $p$,
\[
\Thr(p) \;:=\; \Hq s.\ \lout(s)@(\text{\texttt{claim}},p) \mid \top,
\qquad
\Frk(p) \;:=\; \Thr(p) \mid_0 \Thr(p) .
\]
\end{definition}

$\Thr(p)$ says some square is offering a completion for $p$; it is a structural
formula of depth one. $\Frk(p)$ says the term splits into two parts, each
carrying such an offer, \emph{sharing no restricted square name}. Two threats at
the same square would share that square's name and are excluded by $k=0$. This
is the graded separation of \cite{classifier}, \S2.3.2, used at the bottom of its
grading, and it was not introduced for this purpose.
...
(Proposition~\ref{prop:parands}).}
\label{fig:fork}
\end{figure}

\begin{proposition}[The squares must be the parands]\label{prop:parands}
Let $\llbracket B \rrbracket$ be an encoding of a position under which
$\Frk(p)$ as given in Definition \ref{def:fork} is satisfied exactly by the
positions in which $p$ has a fork. Then the restricted names over which
$\mid_0$ counts sharing must be the square names, not the line names.
\end{proposition}
\begin{proof}[Proof sketch]
Two lines of a three-by-three grid meet in at most one square. Suppose the
parands are line processes and the shared names are line names. Two distinct
lines share no line name, so $\mid_0$ is satisfied by any two distinct threats,
including a pair whose gaps coincide---for instance the two diagonals when only
the centre is empty. That is not a fork, since one move blocks both. Conversely,
under the square-indexed encoding an offer is a send on the gap's own channel,
so two threats share a restricted name iff they share their gap, and $k=0$ holds
iff the gaps are distinct, which is the definition of a fork.
\end{proof}
...
\subsection{Six rungs and a limit}
...
\begin{tabular}{@{}clll@{}}
\toprule
rung & formula added & kind & depth \\
\midrule
0 & $\top$ & --- & 0\\
1 & $\langle K\rangle \Wn(\text{me})$ & modal, existential & 1\\
2 & $\langle K\rangle \neg\Thr(\text{them})$ & modal & 2\\
3 & $\langle K\rangle \Frk(\text{me})$ & \textbf{spatial} ($\mid_0$) & 3\\
4 & $\langle K\rangle[K]\,\neg\Frk(\text{them})$ & modal, alternating & 4\\
5 & $\Ctr$ & \textbf{spatial}, no lookahead & 0\\
6 & $\chi$ & $\nu$-formula & --- \\
\bottomrule
\end{tabular}
\end{center}

Here $\Wn(p) := \langle K\rangle \lout(\mathit{over})@(\texttt{won},p)$;
$\Ctr$ is a purely structural preference over squares (centre, then the corner
opposite an opponent's, then any corner, then a side), containing no modality at
all; and $\chi$ is the characteristic formula of the arena running against this
opponent, a $\nu$-formula in the sense of \cite{classifier}, \S3.

\begin{remark}[Why $\chi$ is reachable here, and what that costs later]
Proposition 9 of \cite{scientist} says the complete theory of an environment is
unaffordable when the environment's recursion follows the reduction rather than a
finite structural cycle. Noughts-and-crosses is the converse case of Remark 10
there: the game tree is finite, the recursion follows the structure, and $\nu$ is
exact at a finite unfolding. So this is a \emph{closed} science---one of the
subject matters that admit a complete finite theory. That is what makes the
domain a good demonstration and, as \S\ref{sec:death} shows, what kills the
ecology.
\end{remark}
...
\begin{proposition}[The ladder is a geodesic-free chain]
Under the stratified distance of \cite{scientist}, Definition 8,
$d(\varphi_r,\varphi_{r+1}) = 2^{-r}$ for $r=1,\dots,4$: the rungs agree on all
approximants below stage $r$ and differ at stage $r$, because rung $r$'s added
disjunct is a formula of modal depth exactly $r$.
\end{proposition}

Two consequences follow immediately and they are the ones that structure
\S\ref{sec:confine}. First, moving from rung $r$ to rung $r+1$ is a \emph{short}
...
placed it in. Second, by Proposition 14 of \cite{scientist}, cost is monotone in
distance, so the cheap moves are exactly the confined ones.

\subsection{The modal asymmetry, and the order of learning}

Proposition 6 of \cite{scientist} says a diamond formula has a finite
confirming witness and no finite refuting one, and dually for a box. Read on
this ladder:
\[
\underbrace{\langle K\rangle \Wn(\text{me})}_{\text{cheap to confirm}}
\qquad\text{versus}\qquad
\underbrace{[K]\,\neg\Wn(\text{them})}_{\text{cheap to refute}} .
\]
Establishing that one \emph{has} a winning move requires exhibiting it.
Establishing that a move is \emph{safe} requires quantifying over every reply.
The framework therefore predicts, from the cost structure alone, that a mortal
learner acquires attack before defence---which is what every human learner of
the game in fact does, and what our rung ordering was independently obliged to
encode, since rung 1 is offensive and rung 2 defensive.

We can put a number on the asymmetry. Restricting the top heuristic rung to
each colour separately: as the first player it achieves the game-theoretic value
against a perfect opponent, drawing every game; as the second player it loses
$6.3\%$ of games against a perfect opponent. The same policy is complete on
offence and incomplete on defence.

\begin{remark}[A reading, offered as such]
We resist over-claiming here. The first player has the initiative, so
existential reasoning is more nearly sufficient for it, and one could attribute
the asymmetry to that alone. What the framework adds is a reason the two are not
symmetric \emph{in price}: the universal formula is the one whose confirmation
has no finite witness, so a budget-bounded learner confirms attacks and can only
ever refute claims of safety. We take the measurement as consistent with
Proposition 6 rather than as a demonstration of it.
\end{remark}

\section{The assay}\label{sec:assay}

A move is an experiment, and in this domain that is not a metaphor to be
maintained but the only thing a move can be.

\begin{definition}[The move-assay]
...
from patience. The three outcomes of Proposition 5 of \cite{scientist} are
...
\begin{remark}[Where the checker is]
...
\subsection{Pricing}\label{sec:price}

We charge each rule the modal depth of the formula it installs, times the number
of candidate moves it must examine:
\[
\kappa(d) = 2^{d}, \qquad
\text{cost of a decision at rung } r \;=\; |M| \cdot \!\!\sum_{\text{rules } 1..r}\!\! \kappa(d_{\text{rule}}) ,
\]
with $|M|$ the number of legal moves. So rung 1 costs $2$ per candidate, rung 2
adds $4$, rung 3 adds $8$, rung 4 adds $16$, and $\chi$ costs $32$; the
positional rung, being depth zero, adds $1$.

Three comments. The schedule is a \emph{choice}, and \S\ref{sec:measure} varies
it. The exponential shape is not arbitrary: it is what makes distance, depth,
and cost one graded structure, which is the reason \cite{scientist} prefers the
...
\section{Destructive assay, again}\label{sec:destructive}

Section 8 of \cite{scientist} argues that because harvest ends the prey's
...
Every rung of \S\ref{sec:ladder} is of this form: none of them mentions an
...
\begin{proposition}[Learning requires a population]
...
first and \S\ref{sec:confine} supplies the second.
\end{proposition}

This is the same conclusion \S17.2 of \cite{scientist} reaches for continual
...
\subsection{The ladder pays, and where}
...
1 & $\langle K\rangle\Wn$                   & 0.668 & 0.075 & 0.258 & 40.9 & $+0.231$ & $+0.0057$ \\
2 & $\langle K\rangle\neg\Thr$              & 0.798 & 0.165 & 0.037 & 113.8 & $+0.130$ & $+0.0018$ \\
3 & $\langle K\rangle\Frk$                  & 0.831 & 0.135 & 0.035 & 228.1 & $+0.033$ & $+0.0003$ \\
4 & $\langle K\rangle[K]\neg\Frk$           & 0.836 & 0.138 & 0.027 & 449.8 & $+0.005$ & $+0.00002$ \\
5 & $\Ctr$                                  & \textbf{0.918} & 0.080 & 0.002 & 457.9 & $+0.083$ & $\mathbf{+0.0102}$ \\
6 & $\chi$                                  & 0.873 & 0.127 & \textbf{0.000} & 679.2 & $-0.046$ & $-0.0002$ \\
\bottomrule
\end{tabular}
...
\draw[->] (0,0.40) -- (0,0.96) node[above left,font=\small,rotate=0] {win rate};
...
% ladder path
\draw[thick,gray!70] (0,0.437) -- (40.9,0.668) -- (113.8,0.798) -- (228.1,0.831) -- (449.8,0.836) -- (457.9,0.918);
\draw[thick,gray!70,dashed] (457.9,0.918) -- (679.2,0.873);
% dominated region for rung 5
\fill[gray!18] (457.9,0.40) rectangle (740,0.918);
\draw[gray!55] (457.9,0.918) -- (740,0.918);
\draw[gray!55] (457.9,0.40) -- (457.9,0.918);
% points
\foreach \x/\y/\l in {0/0.437/0,40.9/0.668/1,113.8/0.798/2,228.1/0.831/3,449.8/0.836/4}
  {\fill (\x,\y) circle (2.2pt); \node[font=\small,above left=-1pt] at (\x,\y) {\l};}
\fill (457.9,0.918) circle (2.6pt); \node[font=\small,above=1pt] at (457.9,0.918) {5};
...
Rungs 1--4 show sharply diminishing returns for a roughly doubling price. Rung
5---the spatial conjunct, of modal depth zero---is nearly free and delivers the
largest jump on the ladder. The complete theory $\chi$ lies inside the shaded
region: it costs more than rung 5 and wins less, so it is dominated on both
axes.}
\label{fig:pareto}
\end{figure}

Three features of Table \ref{tab:ladder} deserve separate statements.

\begin{observation}[Diminishing returns, until they don't]
Marginal return falls by more than two orders of magnitude from rung 1 to rung
4---$+0.0057$, $+0.0018$, $+0.0003$, $+0.00002$---while cost roughly doubles at
each step. Rung 4 in particular buys $+0.005$ of win probability for $222$
metered units. A budget-bounded learner would stop there, and the foraging
inequality of \cite{scientist}, Proposition 27, says exactly when: harvest
credits the scientist only if the prey's stack exceeds what finding and opening
it cost.
\end{observation}

\begin{observation}[The spatial rung is the profitable one]\label{obs:spatial}
Rung 5 returns $+0.0102$ per metered unit: the best marginal return anywhere on
the ladder, nearly twice that of the first rung and some five hundred times that
of rung 4 beneath it. It achieves this because $\Ctr$ has modal depth zero---it
is a structural predicate on the current position, requiring no lookahead---and
because what it encodes, the differential value of squares by their line
membership, is a fact about the \emph{shape} of the board rather than about the
game's dynamics.
\end{observation}

This is the result we would lead with. The classifier note argues, in the knot
setting, that the spatial layer is where the generated logic earns its name and
that a purely behavioural logic simply cannot state the properties that matter
(\cite{classifier}, Remark 5). That argument is made there on expressiveness
grounds. Here the same claim is cashed in an ecological currency: the spatial
conjunct is not merely expressible-and-useful, it is the \emph{cheapest useful
thing on the ladder}, and a mortal learner that priced its own hypotheses would
find it first among the deep options and last among the cheap ones. A modal
logic could not have offered it.

\begin{observation}[The true theory is dominated]\label{obs:chi}
...
$0.918$ at cost $457.9$ while $\chi$ wins $0.873$ at cost $679.2$; against a
rung-4 opponent, $0.448$ at $503.2$ versus $0.418$ at $708.8$. Fewer wins, half
again the price.
\end{observation}
...
measured it. Remark 16 of \cite{scientist} says that a mortal scientist is not
...
\begin{corollary}[Truth buys safety, not food]
...
\subsection{Invasion, and the price of being right}
...
against the resident's $585.0$. The mutant is favoured when $\sigma \cdot
0.0317 > \text{price} \cdot 135.0$, that is when
\[
\sigma \;>\; 4252 \cdot \text{price},
\]
...
where the stakes are some four thousand times the unit cost of thought.

\begin{remark}[Basic research, priced]
Remark 80 of \cite{scientist} concedes the pragmatist's objection---this
...
known to be wrong and known to be more profitable.
\end{remark}

\subsection{The phase band}

Table \ref{tab:phase} reports, for each opponent depth and each prey stack, which
...
400  & 3 & 3 & 5 & 5 & 5 & 2 & 0 \\
800  & 5 & 5 & 5 & 5 & 5 & 2 & 0 \\
1600 & \textbf{6} & \textbf{6} & 5 & 5 & 5 & 2 & 0 \\
\bottomrule
\end{tabular}
...
The shape is the phase diagram of \cite{scientist}, \S13.2, in a domain where
...
Raising the price to $0.2$ and to $1.0$ shifts the band toward the cheap rungs
without changing its shape: at price $1.0$ the optimal rung is $0$ or $1$ over
most of the table and never $6$. The qualitative structure---interior optimum,
degenerate at both ends---is robust; the boundaries are not.

\section{Confinement, measured}\label{sec:confine}

This section is the one we would keep if we could keep only one, because it
turns Proposition 12 of \cite{scientist} from a fact about ultrametric spaces
into a fact about a hypothesis space someone might actually hold.

\subsection{Two loci, and only one of them is reachable}

A policy on this ladder has exactly two parameters, and they are the two that
Definition 13 of \cite{scientist} says a revision move must choose: a
...
\ref{app:code} this is visible as a fact about the code: \texttt{Revise} only
...
\begin{observation}[The best opening is not a property of the game]
...
\begin{corollary}[Deepening within a ball can make things worse]
...
This is Proposition 12 and Proposition 14 acting together and it is worth being
...
\subsection{The escape, and what it costs whom}

\S6.5 of \cite{scientist} says an individual has exactly one route out and it is
...
\begin{proposition}[The invading move is the one no individual can make]
The corner-opening mutant differs from the resident by a revision of distance
$2^{-1}$. By Proposition 12 no walk of deepenings reaches it; by Proposition 14
...
That is the whole argument of \S12 of \cite{scientist} for why reproduction is
...
\begin{remark}[Opening theory changes by catastrophe]
...
\section{Succession, and the death of a solved world}\label{sec:death}

Noughts-and-crosses is a draw under optimal play. That fact, harmless in game
theory, is fatal here.

\begin{proposition}[A solved science is a dead ecology]
...
\begin{corollary}[Imperfection is load-bearing]
...
Predation is the purchase of somebody else's remaining time (\cite{scientist},
Remark 19). A population that has solved its environment has nothing left to
...
\begin{remark}[What would keep it alive]
Three exits, none of which this note takes. Enlarge the game so that $\chi$ is
unaffordable, putting the domain back on the Proposition 9 side of Remark 10---
...
conserved system. The third is the one biology took, and \cite{scientist},
...
constrains the encoding of the world (\S\ref{sec:board}). Pricing hypotheses by
...
the ladder and the complete theory is dominated (\S\ref{sec:measure}).
...
(\S\ref{sec:confine}). And conservation forces an ending (\S\ref{sec:death}).
...
structural and modal formulae of \S\ref{sec:ladder} are specifications, and the
running listing checks their arithmetic shadows. The pricing schedule is a
choice. And the ecology of Appendix \ref{app:code} is written but not run to a
fixed point; the measurements of \S\ref{sec:measure} are exact game-tree
...
\begin{enumerate}
\item \textbf{Run the ecology.} Appendix \ref{app:code} specifies a population;
\S\ref{sec:measure} computes payoffs. Closing the gap---sampling trajectories
via the Gillespie construction of \cite{stochastic}---would turn Table
\ref{tab:phase} from a payoff calculation into the ensemble test that is the
first open problem of \cite{scientist}.

\item \textbf{An unsolvable game.} Everything interesting about
\S\ref{sec:death} follows from the domain being closed. Rerunning the mapping on
...
\item \textbf{The reversal.} \S\ref{sec:confine} finds that the optimal opening
...
for the other and Table \ref{tab:phase} is a snapshot of a game whose payoffs
are themselves changing. This is problem 3 of \cite{scientist} and this domain
is small enough to attack it in.
\end{enumerate}

\appendix

\section{The world, in rholang}\label{app:code}
...
\lstinputlisting{noughts.rho}

\section{Reproducing the numbers}\label{app:repro}

All figures in \S\ref{sec:measure} and \S\ref{sec:confine} are exact
...
\S\ref{sec:price} and computes, by memoised recursion over all $255{,}168$
...
\bibitem{scientist} L. G. Meredith. \emph{The Mortal Scientist: learning as
cost-accounted computation in a reflective higher-order calculus.} F1R3FLY.io,
2026.

\bibitem{classifier} L. G. Meredith. \emph{Classifying knot diagrams with a
spatial--behavioural logic: a logic obtained for free from the encoding of knots
as processes.} F1R3FLY.io, 2026.

\bibitem{knots} L. G. Meredith. \emph{Knots as cost-accounted rholang processes:
a Dowker--Thistlethwaite-driven encoding and weak-bisimulation invariance.}
F1R3FLY.io, 2026.

\bibitem{gslt} \emph{Graph-structured lambda theories: a reference for the
working developer.} F1R3FLY.io, 2026.

\bibitem{probing} L. G. Meredith. \emph{Probing universality: context-labelled
bisimulation and a relative refinement of Turing completeness for interactive
GSLTs.} F1R3FLY.io, 2026.

\bibitem{stochastic} L. G. Meredith. \emph{Stochastic simulation for decorated
ciGSLTs.} F1R3FLY.io, 2026.

\bibitem{nslogic} L. G. Meredith and M. Radestock. Namespace logic: a logic for
a reflective higher-order calculus. In \emph{Trustworthy Global Computing},
LNCS 3705, Springer, 2005.

\bibitem{rho} L. G. Meredith and M. Radestock. A reflective higher-order
calculus. \emph{Electronic Notes in Theoretical Computer Science}
141(5):49--67, 2005.

\bibitem{newell} A. Newell and H. A. Simon. \emph{Human Problem Solving.}
Prentice-Hall, 1972.

\bibitem{caires} L. Caires and L. Cardelli. A spatial logic for concurrency
(part I). \emph{Information and Computation} 186(2):194--235, 2003.

\bibitem{cleaveland} R. Cleaveland. On automatically explaining bisimulation
inequivalence. In \emph{Computer Aided Verification (CAV)}, LNCS 531, Springer,
1990.
