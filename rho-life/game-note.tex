...
\item[Minimax is an alternating modal formula.] The modalities are labelled by
  redex positions, and \S\ref{sec:K} shows that in this domain a redex position
  ...
  over something rather than standing in for it. Proposition 6 of
...
built by reflection in the manner of the knot encoding's arcs: a \emph{move
channel} $\mathit{sq}_k = @[*b,k]$ and a \emph{mark cell} $\mathit{mk}_k =
@[*b,\texttt{"m"},k]$. An empty square is a \emph{receipt} offering to be
...
grades one and two of Table 1 of \cite{scientist}, enforced by the choice of
namespace.

\begin{definition}[Threat, fork]\label{def:fork}
For a player $p$,
\[
\Thr_s(p) := \lout(\mathit{th}_s)@p, \qquad
\Thr(p) := \Hq s.\ \Thr_s(p) \mid \top, \qquad
\Frk(p) := \Thr(p) \mid_0 \Thr(p) .
\]
\end{definition}

$\Frk(p)$ says the term splits into two parts, each carrying an offer,
\emph{sharing no restricted name}. Two threats at the same square---two lines
with a common gap---both deposit on $\mathit{th}_s$, so the two parands share
that name, $k \geq 1$, and $\mid_0$ excludes them. This is the graded separation
of \cite{classifier}, \S2.3.2, used at the bottom of its grading, and it was not
...
\begin{proposition}[The offers must be indexed by square]\label{prop:parands}
Let $\llbracket B \rrbracket$ be an encoding under which $\Frk(p)$ as given in
Definition \ref{def:fork} is satisfied exactly by the positions in which $p$ has
a fork. Then the names over which $\mid_0$ counts sharing must be indexed by the
completing square, not by the line.
\end{proposition}
...
\subsection{The redex context is the move}\label{sec:K}
...
(\cite{classifier}, \S2.1). The label is not an action name. It records the
...
\[
W \;\equiv\; \Hq \vec b.\ \Big(\ \prod_{k \in \mathrm{empty}} \mathsf{Empty}_k
...
\begin{definition}[The move context]\label{def:K}
...
\begin{proposition}[Moves are redex positions]\label{prop:moves}
...
\begin{remark}[Why an action label would not do]
...
reasoning about names, which is the manoeuvre \cite{classifier}, \S3.1 declines
...
\begin{lemma}[The board settles]\label{lem:settle}
...
\begin{definition}[The settled modality]
...
\ref{lem:settle}. This is the weak diamond, and Lemma \ref{lem:settle} makes it
deterministic in $s$.
\end{definition}
...
\ref{prop:moves} is to say over legal moves.

Two features of the shape are worth naming, and both were invisible while $K$
was an uninstantiated placeholder.

\begin{observation}[Winning and blocking are perception, not lookahead]
...
interaction---grades one and two of Table 1 of \cite{scientist}---and they are
...
\begin{observation}[The asymmetry has moved to the right place]\label{obs:asym}
...
$\exists s \forall t$, a box under a diamond. By Proposition 6 of
\cite{scientist} the diamond has a finite confirming witness and the box has
none, so rung 4 is the rung whose confirmation is unbounded and whose refutation
is cheap. Section \ref{sec:measure} finds that it is also, by a wide margin, the
...
structural cycle. Noughts-and-crosses is the converse case of Remark 10 there:
...
what makes the domain a good demonstration and, as \S\ref{sec:death} shows, what
kills the ecology.
\end{remark}
...
\begin{proposition}[The ladder is a chain of shrinking steps]
Under the stratified distance of \cite{scientist}, Definition 8, the rungs
...
\subsection{The modal asymmetry, and what it predicts}

Observation \ref{obs:asym} located the ladder's one alternating rung. It is
...
replies, and by Proposition 6 of \cite{scientist} no finite run witnesses that;
...
game in fact does, and which \S\ref{sec:measure} confirms in the currency that
...
measurement as consistent with Proposition 6; the rung-4 cost is the
demonstration.
\end{remark}
...
\]
with $|M|$ the number of legal moves---which by Proposition \ref{prop:moves} is
...
metered units, and it is the alternating rung of Observation \ref{obs:asym}: the
...
\begin{observation}[The non-modal rungs carry the ladder]\label{obs:spatial}
...
logic cannot state the properties that matter (\cite{classifier}, Remark 5).
...
\begin{remark}[Why this is not an artefact of the encoding]
...
paid: the line processes run on every settling, and \S\ref{sec:price} charges
