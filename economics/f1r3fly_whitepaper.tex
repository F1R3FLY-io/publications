...
\hfill
\headline{\$0.02--\$2.75}{acquired cost per head vs.\ \$19--\$50 retail D2C}
\hfill
\headline{99\%}{share of traffic the connector taxes once the network is dense}
...
These findings shape the go-to-market plan:
...
\begin{itemize}[leftmargin=1.4em, itemsep=3pt]
  ...
  \item \textbf{Acquisition is wholesale, not retail.} Court anchors --- artists, providers --- not individuals. CAC lands at \$0.02--\$2.75/head against \$19--\$50 D2C, and is self-liquidating because anchors pay fees from day one.
  \item \textbf{The connector is the profit centre.} A cross-shard interchange taxes value moving between shards; its tax base grows toward the entire platform as more shards connect, making it the apex revenue layer and the moat.
\end{itemize}
...
% =================== ACQUISITION ===================
\section{Acquisition: court populations, not people}

Incumbent direct-to-consumer acquisition runs ${\sim}\$50$/head (Facebook) and ${\sim}\$19$/head (WhatsApp) --- paid one head at a time. F1R3FLY's model is \emph{wholesale}: court one supply-side anchor and acquire its entire downstream population in a single deal, at $\mathrm{CAC} = A / (\text{population} \times \text{conversion})$.
...
\toprule
\rowcolor{headrow}
\textbf{Channel} & \textbf{Court} & \textbf{Acquire} & \textbf{CAC/head} & \textbf{vs.\ retail} \\
\midrule
Music     & the artist   & their audience & \$0.02--\$1.00 & up to ${\sim}950\times$ cheaper \\
Healthcare & the provider & the patient cohort & \$0.50--\$2.75 & up to ${\sim}100\times$ cheaper \\
\bottomrule
\end{tabular}
\end{center}

Two structural advantages compound. The anchor is a \textbf{paying customer, not a liability} --- its fees recover the acquisition cost, so CAC is self-liquidating. And patient data ownership (the private key to a personal data wallet) does double duty: it is a close-rate differentiator on the enterprise sale \emph{and} a downstream channel, because a user who owns their data carries it to every other F1R3FLY service at zero marginal CAC. That portability is also what makes a head acquired once a user in many markets --- multiplying lifetime value against a single acquisition.

% =================== CONNECTOR ===================
\section{The connector shard: where the network monetises itself}

Per-sector shards earn on work done \emph{inside} them. The \emph{connector} shard earns on value moving \emph{between} them --- the interchange position (Visa, SWIFT, an internet exchange point). A provider checks a box at deploy time to join, and F1R3FLY collects a fee on every cross-shard transaction.

\begin{center}
\begin{tikzpicture}
  \node[fill=f1r3mist, rounded corners=3pt, inner sep=12pt, text width=0.86\textwidth, align=center] (b) {%
    {\color{f1r3ink}\large As shards connect, the cross-shard share of traffic climbs toward \textbf{99\%}.}\\[5pt]
    {\color{f1r3red}\Large\bfseries The connector ends up taxing essentially all platform volume.}};
  \draw[line width=4pt, f1r3orange] ([xshift=2pt]b.south west) -- ([xshift=2pt]b.north west);
\end{tikzpicture}
\end{center}

This is the profit centre for three reasons. Its \textbf{tax base grows superlinearly}: each new connected shard makes more of every other shard's traffic cross-boundary, so the base asymptotes to total throughput. Its \textbf{cost stays tiny}, because it routes and clears rather than storing payloads. And the opt-in box is a \textbf{flywheel}: every provider that joins makes joining more valuable for the next, and it captures competitors too, since any rival that wants interoperability must connect and pay the toll. Travel and hospitality is the archetypal accelerant --- a single trip spans payment, identity, location, and booking, so it is cross-shard by nature and lights up the connector far beyond its standalone volume. The connector converts F1R3FLY from a sum of sector apps into a network whose value scales with connections: it monetises interoperability itself, which is the moat.
...
The original safe-to-critical risk progression is preserved; the economic ordering simply layers on top of it, putting the unbounded, self-funding sector first and the highest-assurance sector last. Above all of them sits the connector: the sectors bootstrap the volume, and the connector monetises the interoperability between them --- the apex revenue layer once the network reaches critical mass.
