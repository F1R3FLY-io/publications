...
of admissible observers --- is the right long-run discipline and is
developed elsewhere; nothing below needs it. Without minimality the
...
\ref{prop:criteria} and Theorem~\ref{thm:fullabs} are deferred to
Appendix~\ref{app:proofs}, which also sets out the induction and the
case analysis they require. The apparatus they use is
...
show Theorem~\ref{thm:fullabs} survives with a coarser relation.
...
Prop.~\ref{prop:complete}, \ref{prop:sound} & \textbf{deferred} & Appendix~\ref{app:proofs} \\
Thm.~\ref{thm:fullabs}, Prop.~\ref{prop:criteria} & \textbf{deferred} & Appendix~\ref{app:proofs} \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{This appendix is to be supplied. Its content is fixed by the
apparatus of \S\ref{sec:fullabs} and is set out here so that the shape of
the remaining obligation is visible.}

\paragraph{B.1 Substitution.}
The proposition that $\enc{P\{z/y\}}_\sigma = \enc{P}_\sigma\{z/y\}$ for
$y$ a $\pi$ variable, by induction on $P$, using
Lemma~\ref{lem:usage} to see that $y$ occurs only in positions
substitution reaches. This is where admissibility is used a second time.

\paragraph{B.2 Completeness.}
Induction on the derivation of $P \to P'$. Base case
$[\pi\text{-COM}]$: the images of the redex are
$\inp{y}{x}{\enc{P_1}_{\sigma_1}}$ and $\out{x}{\drp{z}}$, and one
reduction gives $\enc{P_1}_{\sigma_1}\{z/y\}$, which is
$\enc{P_1\{z/y\}}_{\sigma_1}$ by B.1. Congruence cases: for $\mid$, two
administrative steps precede; for $\nu$, a $\Frsh$; for structural
congruence, the case $P \mid \nil \equiv P$ requires
Lemmas~\ref{lem:admin} and \ref{lem:addrind}. Replication: one box copy,
one seed exchange, one $\Frsh$, then the source communication. Address
bookkeeping throughout by Lemma~\ref{lem:addrind}, which is what permits
$\sigma'$ to be any address satisfying
Theorem~\ref{thm:locality}.

\paragraph{B.3 Soundness.}
Induction on the length of the target reduction, with each step
classified as administrative or as the image of a source communication;
the classification is exhaustive because by
Theorem~\ref{thm:locality}(i) every subject is an address, a source
name, or a $\nu$-generated name. Administrative steps are absorbed by
Lemma~\ref{lem:admin}; the only non-confluence among them is the $\Frsh$
race, handled by Corollary~\ref{cor:race}. The case needing care is a
partially completed unfolding, in which a box has been copied but the
seed exchange has not occurred.

\paragraph{B.4 Full abstraction.}
From B.2, B.3 and Lemma~\ref{lem:compositional}. Forward: exhibit
$\{(\enc{P},\enc{Q}) : P \bisim_\pi Q\}$ closed under
$\bisim_{\enc{\Ctx_\pi}}$ as a $\enc{\Ctx_\pi}$-bisimulation. Backward:
contrapositively, a separating source context encodes to a separating
label, and the distinguishing barb is on a name of
$\varphi(\fn(P)\cup\fn(Q))$, preserved and reflected by
Lemma~\ref{lem:admin} and Theorem~\ref{thm:locality}.

\paragraph{B.5 Lybech's criteria.}
Compositionality is Lemma~\ref{lem:compositional}; substitution
invariance is B.1; operational correspondence is B.2 and B.3;
observational correspondence follows from
Theorem~\ref{thm:locality}(i); divergence reflection is
Lemma~\ref{lem:admin}; parameter independence is
Lemma~\ref{lem:addrind}.
