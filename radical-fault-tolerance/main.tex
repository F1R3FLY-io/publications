...
\section{What the calculus assumes about delivery}\label{sec:medium}

The caveat the sketch attaches to its arbiter --- that messages are neither lost nor indefinitely delayed --- bundles two assumptions that fail in different places, and the word \emph{asynchrony} is routinely made to carry a third that is not an assumption at all. It is worth separating them before drawing any conclusion about consensus.

\begin{description}
\item[(a) Asynchronous sending.] $x!(Q)$ has no continuation: a sender does not wait for a receiver. This is a presentational choice about the term language, made here and in the asynchronous $\pi$-calculus \cite{HondaTokoro,Boudol}, and it says nothing whatever about a network.
\item[(b) Unbounded delay.] The reduction relation imposes no schedule and no fairness condition, so a datum may remain unmatched for arbitrarily many steps while other reductions proceed. This is the distributed-systems sense of asynchrony, and the \rhoc{} already has it: it is a property of the \emph{resolver} of Section~\ref{sec:resolver}, which is free to keep choosing elsewhere, and not of the message discipline.
\item[(c) Reliable delivery.] A datum, once emitted, is delivered exactly once, to some receipt at its name, uncorrupted. This the \rhoc{} does assume, and it is the assumption that a network violates. Note that ordering is not among the assumptions: data at a name form a multiset, so reordering is native and costs nothing to model.
\end{description}

\begin{remark}
Section~\ref{sec:consensus} as first drafted, and the sketch before it, treated ``lost'' and ``indefinitely delayed'' as one caveat and called its removal asynchrony. They are not the same, they are not removed by the same move, and the calculus's position on them is not the same: (b) is already granted, (c) is already assumed. What follows separates them by making (c) a term rather than a side condition.
\end{remark}

\subsection{Channels as processes}

To speak about delivery one moves from a name to a \emph{channel}: a process interposed between two names, carrying the traffic from one to the other. Where the calculus writes $\In{y}{x}P \mid x!(Q)$, we write
\[
  \In{y}{x_1}P \;\mid\; \mathsf{Chan}(x_1, x_2) \;\mid\; x_2!(Q) ,
  \qquad
  \mathsf{Chan}(x_1,x_2) \;=\; \Inp{y}{x_2}\, x_1!({*}y) .
\]

\begin{proposition}[Transparency]\label{prop:chan}
The honest forwarder is transparent: for every $P$ and $Q$, the interposed term above is weakly barbed bisimilar to $\In{y}{x}P \mid x!(Q)$ up to the renaming of the interface, and the interposition costs one additional reduction per message.
\end{proposition}

This is the standard forwarder or link result for asynchronous name-passing calculi \cite{HondaTokoro,MerroSangiorgi}, and the qualification ``up to the renaming of the interface'' is the usual one: the composite has barbs at $x_1$ and $x_2$ where the original has one at $x$, which is again a statement relative to a class of observers rather than an absolute one.

What the reification buys is that $\mathsf{Chan}$ need not be honest.

\begin{definition}[Sinister channels]\label{def:sinister}
Writing $\mathsf{coin}$ for the local race of Definition~\ref{def:arbiter}:
\begin{align*}
  \mathsf{Drop}(x_1,x_2) &= \Inp{y}{x_2}\ \mathsf{if}\ \mathsf{coin}\ \mathsf{then}\ x_1!({*}y)\ \mathsf{else}\ 0 \\
  \mathsf{Dup}(x_1,x_2) &= \Inp{y}{x_2}\ \big(\,x_1!({*}y) \mid x_1!({*}y)\,\big) \\
  \mathsf{Corrupt}(x_1,x_2) &= \Inp{y}{x_2}\ x_1!(F({*}y)) \quad \text{for some } F \\
  \mathsf{Delay}(x_1,x_2) &= \Inp{y}{x_2}\ \mathsf{park}!({*}y) \ \mid\ \Inp{z}{\mathsf{park}}\ x_1!({*}z)
\end{align*}
\end{definition}

$\mathsf{Delay}$ is worth a word: it does not delay in any timing sense, because the calculus has no clock. It converts a datum into a datum at another name, which the resolver may then decline to choose for as long as it likes. That is the whole content of (b), and it confirms that unbounded delay needs no channel at all --- an honest medium already permits it. Loss, duplication and corruption are what genuinely need a sinister medium, and they are exactly what (c) rules out.

\begin{proposition}[The medium replicates for free]\label{prop:chanidem}
$\mathsf{Chan}$ is a persistent receipt whose continuation does not contend, so by Proposition~\ref{prop:idem} it is idempotent and is its own $n$-th root for every $n$.
\end{proposition}

Proposition~\ref{prop:chanidem} localises the difficulty of this note rather precisely. The transport layer needs no arbitration: replicating it is free, and the copies of the medium may be composed without coordination. Everything the arbiter is for lives in the \emph{linear} traffic that the medium carries.

\begin{remark}[The fault model becomes a perturbation of a term]\label{rem:faultmodel}
Section~\ref{sec:consensus} says that making the consensus correspondence a theorem needs a fault model, and that no fault model is given here. With the medium reified, the shape such a model would take is visible: it is the class of systems obtained from $\rt{S}$ by replacing the medium in some copies with a variant from Definition~\ref{def:sinister}. Omission faults are $\mathsf{Drop}$; a network-level Byzantine fault is $\mathsf{Corrupt}$; a node-level Byzantine fault is a copy whose \emph{arbiter}, rather than whose medium, departs from Definition~\ref{def:arbiter}. The point is that all of these are terms, so the fault model can be stated inside the calculus and the correctness of an arbiter against it becomes an ordinary equivalence question rather than a metalevel stipulation.
\end{remark}

\begin{principle}[The arbiter is a channel]\label{prin:arbchan}
Definition~\ref{def:arbiter} consumes data at the deposit names and emits at the delivered name. That is the shape of $\mathsf{Chan}$, with a policy inside: the arbiter drops nothing but defers some of what it receives (Remark~\ref{rem:loser}), and it chooses which. So the arbiter and the sinister channel are the same construction, and what distinguishes a consensus protocol from a faulty network is not the shape of the term but a guarantee \emph{across} copies. A consensus protocol is a family of media, one per copy, constrained to make the same choices.
\end{principle}

Principle~\ref{prin:arbchan} is Section~\ref{sec:resolver}'s slogan at the level of the medium, and it makes the internalisation pattern of this programme visible three times over in one construction: a graded where-clause moves the resolver's policy into the syntax, the arbiter moves the resolver's agreement into the syntax, and $\mathsf{Chan}$ moves delivery into the syntax. In each case a parameter of the interpretation becomes a term, and in each case what was an assumption one states about a model becomes a process one may reason about.

\begin{remark}[What existing protocols deliver]
Assumption (c) is not extravagant. Over a live connection, TCP \cite{RFC793} delivers exactly once, uncorrupted, and in order --- more than the calculus asks on ordering and precisely what it asks on reliability. What TCP does not supply is that the connection remains live; a partition is the failure of (b) and (c) together and is not detectable from inside. So the gap between the \rhoc{}'s message discipline and a real transport is smaller than the phrase ``the network is unreliable'' suggests, and the reified medium is there for the residue: partition, and adversarial peers.
\end{remark}
...
Reliable channels & Assumption (c) of Section~\ref{sec:medium}, reified as $\mathsf{Chan}$ and removable term-by-term \\
\hline
\end{tabular}
\end{center}

The sketch's own caveat names the right two things, and Section~\ref{sec:medium} says where each of them lives. The arbiter assumes reliable delivery --- assumption (c), which is the calculus's own and which a sinister medium removes --- and it assumes that the arbiter does not lie, which no medium can repair. It does \emph{not} assume anything about delay: assumption (b) is already absent, since the resolver is under no fairness obligation, and Lemmas~\ref{lem:progress} and \ref{lem:exh} are stated as reachability rather than as timing. So the two axes are unreliable delivery and dishonest participants, not ``asynchrony'' and Byzantine behaviour; and a protocol such as Casper exists to address both, together with the delay the calculus already permits, in a decentralised setting.

Two remarks about the shape a theorem would take, offered as orientation and not as results. First, the correspondence has an obvious direction: an arbiter that is correct in a model with crash faults would decide a binary value agreed by all copies, which is what the classical impossibility result for deterministic asynchronous consensus \cite{FLP} forbids --- so in such a model the coin of Lemma~\ref{lem:coin} would not be a stylistic choice but a necessity, and the neighbourhood to look in is randomised agreement \cite{BenOr}. The separation of Section~\ref{sec:medium} is what makes this remark say anything: the impossibility result assumes \emph{reliable} channels and drops only the timing assumption --- (c) held, (b) absent --- which is exactly the position the \rhoc{} is already in, and which is therefore the setting the arbiter of Section~\ref{sec:arb} would have to be judged in first. Second, the general site of Section~\ref{sec:every} asks for agreement on a serialisation rather than on a value, which is atomic broadcast, and hence the replicated state machine construction \cite{Schneider}. Making either remark precise requires a fault model, a definition of correctness for an arbiter under it, and a reduction; none of that is attempted here.
...
\paragraph{The arbiter against a sinister medium.} Remark~\ref{rem:faultmodel} says where a fault model would live but does not build one. The first question to ask of it is narrow and answerable: which of Lemmas~\ref{lem:progress}, \ref{lem:exh} and \ref{lem:agree} survive the replacement of one copy's medium by $\mathsf{Drop}$, and which by $\mathsf{Dup}$. Duplication looks the more dangerous of the two, since Lemma~\ref{lem:exh} counts.
...
The distance between the arbiter of Section~\ref{sec:arb} and a protocol that works on the open Internet is exactly the two assumptions the original sketch named: that messages arrive, and that the arbiter is honest. It is not, as one is tempted to say, a matter of asynchrony: the calculus imposes no schedule and no fairness, so unbounded delay is granted already, and what a network takes away is delivery. Once the medium is a process, that difference is a difference between terms, and the arbiter turns out to be a medium too --- one that $n$ copies are obliged to agree on. That the distance is only those two assumptions is the point of the exercise.
...
\bibitem{Boudol} G. Boudol. Asynchrony and the $\pi$-calculus. INRIA Research Report 1702, 1992.

\bibitem{HondaTokoro} K. Honda and M. Tokoro. An object calculus for asynchronous communication. \emph{ECOOP}, 1991.

\bibitem{MerroSangiorgi} M. Merro and D. Sangiorgi. On asynchrony in name-passing calculi. \emph{Mathematical Structures in Computer Science} 14(5):715--767, 2004.

\bibitem{RFC793} J. Postel (ed.). Transmission Control Protocol. RFC 793, 1981.
