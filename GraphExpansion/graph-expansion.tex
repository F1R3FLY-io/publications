...
% ---
\subsection{Worked Example: the Fano Plane}
\label{sec:fano}

The Fano plane $\mathrm{PG}(2,2)$---the smallest finite projective
plane---provides a transparent worked example of $\mathbf{expand}$: seven
nodes, seven lines, three nodes per line, three lines per node.
We label the nodes $0$--$6$, placing~$0$ at the origin, $1,2,3$ at the
vertices of the outer equilateral triangle, and $4,5,6$ at the midpoints of
sides $1$-$2$, $2$-$3$, and $1$-$3$ respectively.  Orienting each projective
line as a directed $3$-cycle (Table~\ref{tab:fano-lines}) yields a $3$-regular
directed graph $G_0$ shown in Figure~\ref{fig:fano-g0}.

\begin{table}[ht]
\centering\small
\caption{Directed Fano plane $G_0$: each projective line oriented as a
directed $3$-cycle.  Every node has in-degree~$3$ and out-degree~$3$.}
\label{tab:fano-lines}
\setlength{\tabcolsep}{9pt}
\begin{tabular}{@{}lll@{}}
\toprule
Line & Cycle & Directed edges \\
\midrule
$\{1,2,4\}$ & $1\!\to\!2\!\to\!4\!\to\!1$ & $1\to 2,\quad 2\to 4,\quad 4\to 1$ \\
$\{2,3,5\}$ & $2\!\to\!3\!\to\!5\!\to\!2$ & $2\to 3,\quad 3\to 5,\quad 5\to 2$ \\
$\{1,3,6\}$ & $3\!\to\!1\!\to\!6\!\to\!3$ & $3\to 1,\quad 1\to 6,\quad 6\to 3$ \\
$\{0,1,5\}$ & $1\!\to\!5\!\to\!0\!\to\!1$ & $1\to 5,\quad 5\to 0,\quad 0\to 1$ \\
$\{0,2,6\}$ & $2\!\to\!6\!\to\!0\!\to\!2$ & $2\to 6,\quad 6\to 0,\quad 0\to 2$ \\
$\{0,3,4\}$ & $3\!\to\!4\!\to\!0\!\to\!3$ & $3\to 4,\quad 4\to 0,\quad 0\to 3$ \\
$\{4,5,6\}$ & $4\!\to\!5\!\to\!6\!\to\!4$ & $4\to 5,\quad 5\to 6,\quad 6\to 4$ \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\begin{tikzpicture}[
    fnode/.style={circle, draw, thick, minimum size=7mm, inner sep=1pt,
                  font=\small\bfseries},
    mnode/.style={circle, draw, thick, fill=orange!30, minimum size=7mm,
                  inner sep=1pt, font=\small\bfseries},
    fedge/.style={-Stealth, semithick},
]
  \coordinate (v0) at (0,0);
  \coordinate (v1) at (90:2.2);
  \coordinate (v2) at (210:2.2);
  \coordinate (v3) at (330:2.2);
  \coordinate (v4) at (150:1.1);
  \coordinate (v5) at (270:1.1);
  \coordinate (v6) at (30:1.1);

  % Faint guide lines for the seven projective lines
  \draw[gray!30, thin] (v1)--(v2)  (v2)--(v3)  (v1)--(v3);
  \draw[gray!30, thin] (v1)--(v5)  (v2)--(v6)  (v3)--(v4);
  \draw[gray!30, thin] (v4)--(v5)--(v6)--cycle;

  % Outer triangle directed cycles
  \draw[fedge] (v1) to[bend right=10] (v2);
  \draw[fedge] (v2) to[bend right=10] (v4);
  \draw[fedge] (v4) to[bend right=10] (v1);
  \draw[fedge] (v2) to[bend right=10] (v3);
  \draw[fedge] (v3) to[bend right=10] (v5);
  \draw[fedge] (v5) to[bend right=10] (v2);
  \draw[fedge] (v3) to[bend right=10] (v1);
  \draw[fedge] (v1) to[bend right=10] (v6);
  \draw[fedge] (v6) to[bend right=10] (v3);
  % Inner triangle directed cycle
  \draw[fedge] (v4) to[bend right=10] (v5);
  \draw[fedge] (v5) to[bend right=10] (v6);
  \draw[fedge] (v6) to[bend right=10] (v4);
  % Lines through centre
  \draw[fedge] (v1) to[bend left=12] (v5);
  \draw[fedge] (v5) to[bend left=12] (v0);
  \draw[fedge] (v0) to[bend left=12] (v1);
  \draw[fedge] (v2) to[bend left=12] (v6);
  \draw[fedge] (v6) to[bend left=12] (v0);
  \draw[fedge] (v0) to[bend left=12] (v2);
  \draw[fedge] (v3) to[bend left=12] (v4);
  \draw[fedge] (v4) to[bend left=12] (v0);
  \draw[fedge] (v0) to[bend left=12] (v3);
  ...
  \node[fnode] at (v6) {$6$};
\end{tikzpicture}
\caption{The directed Fano plane $G_0$.  Faint lines show the seven undirected
projective lines; directed edges run as $3$-cycles per Table~\ref{tab:fano-lines}.
Nodes~$0$ (centre) and~$4$ (midpoint of side $1$--$2$) are shaded as the
parent pair $M, F$ for the expansion in Figure~\ref{fig:fano-expand1}.}
\label{fig:fano-g0}
\end{figure}

\paragraph{A single expansion: $\expand(G_0,\,0,\,4,\,D,\,S)$.}
Choose $M = 0$ (the central node) and $F = 4$ (the midpoint of side $1$--$2$).
Reading port sets from Table~\ref{tab:fano-lines}:
\begin{align*}
  \inSrc(0,G_0)  &= \{4,5,6\}, & \outTgt(0,G_0) &= \{1,2,3\},\\
  \inSrc(4,G_0)  &= \{2,3,6\}, & \outTgt(4,G_0) &= \{0,1,5\}.
\end{align*}
The expansion inserts two fresh blend nodes $D = {@}(0{\otimes}4)$ and
$S = {@}(4{\otimes}0)$ with edges
\[
  \{4,5,6\}\to D\to\{0,1,5\},
  \qquad
  \{2,3,6\}\to S\to\{1,2,3\}.
\]
Figure~\ref{fig:fano-expand1} shows the result.  Note that node~$6$ lies in
both $\inSrc(0)$ and $\inSrc(4)$, so it sends an in-edge to \emph{both} blend
nodes simultaneously, marking it as a shared conceptual source.  Duality
(\Cref{prop:duality}) is visible directly: swapping $0 \leftrightarrow 4$
exchanges $D \leftrightarrow S$ exactly.
...
\begin{tikzpicture}[
    ...
    mnode/.style={circle, draw, thick, fill=orange!30, minimum size=7mm,
                  inner sep=1pt, font=\small\bfseries},
    dblend/.style={circle, draw, thick, fill=blue!20, minimum size=8mm,
                   inner sep=1pt, font=\small\bfseries},
    sblend/.style={circle, draw, thick, fill=red!20, minimum size=8mm,
                   inner sep=1pt, font=\small\bfseries},
    bgedge/.style={-Stealth, thin, gray!45},
    kedge/.style={-Stealth, semithick},
    dedge/.style={-Stealth, semithick, dashed, blue!65!black},
    sedge/.style={-Stealth, semithick, dashed, red!65!black},
]
  \coordinate (v0) at (0,0);
  \coordinate (v1) at (90:2.2);
  \coordinate (v2) at (210:2.2);
  ...
  \coordinate (S) at  (3.7, 0.4);

  % Background Fano edges (not incident to 0 or 4)
  \draw[bgedge] (v1) to[bend right=10] (v2);
  \draw[bgedge] (v2) to[bend right=10] (v3);
  \draw[bgedge] (v3) to[bend right=10] (v5);
  \draw[bgedge] (v5) to[bend right=10] (v2);
  \draw[bgedge] (v3) to[bend right=10] (v1);
  \draw[bgedge] (v1) to[bend right=10] (v6);
  \draw[bgedge] (v6) to[bend right=10] (v3);
  \draw[bgedge] (v1) to[bend left=12]  (v5);
  \draw[bgedge] (v2) to[bend left=12]  (v6);
  \draw[bgedge] (v5) to[bend right=10] (v6);

  % Highlighted Fano edges (incident to 0 or 4) - port-set edges
  % inSrc(0)={4,5,6}: edges into 0
  \draw[kedge] (v4) to[bend left=12]  (v0);
  \draw[kedge] (v5) to[bend left=12]  (v0);
  \draw[kedge] (v6) to[bend left=12]  (v0);
  % outTgt(0)={1,2,3}: edges out of 0
  \draw[kedge] (v0) to[bend left=12]  (v1);
  \draw[kedge] (v0) to[bend left=12]  (v2);
  \draw[kedge] (v0) to[bend left=12]  (v3);
  % inSrc(4)={2,3,6}: edges into 4
  \draw[kedge] (v2) to[bend right=10] (v4);
  \draw[kedge] (v3) to[bend left=12]  (v4);
  \draw[kedge] (v6) to[bend right=10] (v4);
  % outTgt(4)={0,1,5}: 4->1 and 4->5 (4->0 already drawn above)
  \draw[kedge] (v4) to[bend right=10] (v1);
  \draw[kedge] (v4) to[bend right=10] (v5);

  % D edges: inSrc(0)->{4,5,6}->D and D->outTgt(4)->{0,1,5}
  \draw[dedge] (v4) to[bend right=18] (D);
  \draw[dedge] (v5) to[bend left=20]  (D);
  \draw[dedge] (v6) to[bend left=14]  (D);
  \draw[dedge] (D) to[bend left=12]   (v0);
  \draw[dedge] (D) to[bend right=14]  (v1);
  \draw[dedge] (D) to[bend right=20]  (v5);

  % S edges: inSrc(4)->{2,3,6}->S and S->outTgt(0)->{1,2,3}
  \draw[sedge] (v2) to[bend left=18]  (S);
  \draw[sedge] (v3) to[bend right=15] (S);
  \draw[sedge] (v6) to[bend right=12] (S);
  \draw[sedge] (S) to[bend left=14]   (v1);
  \draw[sedge] (S) to[bend right=18]  (v2);
  \draw[sedge] (S) to[bend left=12]   (v3);
  ...
  \node[dblend]  at (D)  {$D$};
  \node[sblend]  at (S)  {$S$};
  \node[font=\scriptsize, below=3pt of D] {${=}\,{@}(0{\otimes}4)$};
  \node[font=\scriptsize, below=3pt of S] {${=}\,{@}(4{\otimes}0)$};
\end{tikzpicture}
\caption{Result of $\expand(G_0,\,0,\,4,\,D,\,S)$.  Port-set edges (those
incident to the orange parent nodes $0$ and~$4$) are drawn in black; all
other Fano edges are greyed out.  Dashed blue arrows are the $6$ new edges
incident to $D$; dashed red arrows are the $6$ new edges incident to $S$.
Node~$6$ contributes an in-edge to \emph{both} blend nodes because it lies
in both $\inSrc(0)$ and $\inSrc(4)$.}
\label{fig:fano-expand1}
\end{figure}

\paragraph{The first generation $G_1$.}
Mating every unordered pair $\{i,j\} \subseteq \{0,\ldots,6\}$ yields
$\binom{7}{2} = 21$ expansions, each contributing two fresh blend nodes
$D_{ij}$ and $S_{ij}$.  The resulting graph
\[
  G_1 \;=\; G_0 \;\cup\; \bigl\{\,D_{ij},\,S_{ij} : 0 \le i < j \le 6\,\bigr\}
\]
has $7 + 42 = 49$ nodes.  Each expansion adds $12$ new directed edges ($6$ for
each blend node), so the total edge count is $21 + 21 \times 12 = 273$; every
node in $G_1$ has in-degree $3$ and out-degree $3$, so $G_1$ remains
$3$-regular.  Figure~\ref{fig:fano-g1} shows the node structure of $G_1$
schematically: the seven inner nodes retain their Fano positions and the $42$
blend nodes surround them in $21$ antipodal pairs on an outer ring, one pair
per parent pair.  The previously computed pair $(D_{04},\,S_{04})$ is
highlighted with its $12$ connecting edges.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    fnode/.style={circle, draw, thick, fill=white, minimum size=6mm, inner sep=1pt,
                  font=\small\bfseries},
    mnode/.style={circle, draw, thick, fill=orange!30, minimum size=6mm,
                  inner sep=1pt, font=\small\bfseries},
    dsmall/.style={circle, draw, fill=blue!25, minimum size=4mm, inner sep=0pt},
    ssmall/.style={circle, draw, fill=red!25,  minimum size=4mm, inner sep=0pt},
    dlarge/.style={circle, draw, thick, fill=blue!35, minimum size=6mm,
                   inner sep=0pt, font=\tiny\bfseries},
    slarge/.style={circle, draw, thick, fill=red!35,  minimum size=6mm,
                   inner sep=0pt, font=\tiny\bfseries},
    repedge/.style={-Stealth, very thin, opacity=0.55},
]
  \def\RS{0.60}
  \def\Rout{2.85}

  % Faint guide circle and inner Fano lines
  \draw[gray!18] (0,0) circle (\Rout);
  \draw[gray!22, thin]
    (90:{2.2*\RS})--(210:{2.2*\RS})
    (210:{2.2*\RS})--(330:{2.2*\RS})
    (90:{2.2*\RS})--(330:{2.2*\RS});
  \draw[gray!22, thin]
    (90:{2.2*\RS})--(270:{1.1*\RS})
    (210:{2.2*\RS})--(30:{1.1*\RS})
    (330:{2.2*\RS})--(150:{1.1*\RS});
  \draw[gray!22, thin]
    (150:{1.1*\RS})--(270:{1.1*\RS})
    (270:{1.1*\RS})--(30:{1.1*\RS})
    (30:{1.1*\RS})--(150:{1.1*\RS});

  % All 42 blend nodes (small coloured dots on outer ring)
  \foreach \k in {0,...,20}{
    \pgfmathsetmacro{\bangle}{\k * 360.0 / 21.0}
    \node[dsmall] at ({\bangle - 4}:\Rout) {};
    \node[ssmall] at ({\bangle + 4}:\Rout) {};
  }

  % Highlighted pair {0,4} (k=3): overwrite with larger labelled nodes
  \pgfmathsetmacro{\bfour}{3.0 * 360.0 / 21.0}
  \node[dlarge, label={[font=\scriptsize]right:$D_{04}$}] (D04)
    at ({\bfour - 4}:\Rout) {};
  \node[slarge, label={[font=\scriptsize]right:$S_{04}$}] (S04)
    at ({\bfour + 4}:\Rout) {};

  % Inner Fano nodes (drawn after blend nodes so they sit on top)
  \node[mnode] (v0) at (0,0)                    {$0$};
  \node[fnode] (v1) at (90:{2.2*\RS})           {$1$};
  \node[fnode] (v2) at (210:{2.2*\RS})          {$2$};
  \node[fnode] (v3) at (330:{2.2*\RS})          {$3$};
  \node[mnode] (v4) at (150:{1.1*\RS})          {$4$};
  \node[fnode] (v5) at (270:{1.1*\RS})          {$5$};
  \node[fnode] (v6) at (30:{1.1*\RS})           {$6$};

  % Representative edges for D04: inSrc(0)={4,5,6}->D04->outTgt(4)={0,1,5}
  \draw[repedge, blue!70] (v4) to[bend left=10]  (D04);
  \draw[repedge, blue!70] (v5) to[bend left=20]  (D04);
  \draw[repedge, blue!70] (v6) to[bend right=22] (D04);
  \draw[repedge, blue!70] (D04) to[bend right=10] (v0);
  \draw[repedge, blue!70] (D04) to[bend right=15] (v1);
  \draw[repedge, blue!70] (D04) to[bend right=8]  (v5);

  % Representative edges for S04: inSrc(4)={2,3,6}->S04->outTgt(0)={1,2,3}
  \draw[repedge, red!70] (v2) to[bend right=22] (S04);
  \draw[repedge, red!70] (v3) to[bend left=10]  (S04);
  \draw[repedge, red!70] (v6) to[bend left=18]  (S04);
  \draw[repedge, red!70] (S04) to[bend left=10]  (v1);
  \draw[repedge, red!70] (S04) to[bend left=18]  (v2);
  \draw[repedge, red!70] (S04) to[bend left=8]   (v3);

  % Legend
  \node[dsmall, label=right:{\small $D_{ij}$: blend node}] at (3.8,  0.85) {};
  \node[ssmall, label=right:{\small $S_{ij}$: dual blend}]  at (3.8,  0.25) {};
\end{tikzpicture}
\caption{$G_1$: the first generation.  Seven inner Fano nodes
(nodes~$0$ and~$4$ shaded) are encircled by $42$ blend nodes in $21$
antipodal pairs---one per parent pair $\{i,j\}$---arranged on an outer ring.
The highlighted pair $(D_{04},\,S_{04})$ is the blend from
Figure~\ref{fig:fano-expand1}; thin arrows show its $12$ connecting edges.
$G_1$ has $49$ nodes and $273$ directed edges and is $3$-regular.}
\label{fig:fano-g1}
\end{figure}
...
graph DSL~\cite{meredith2025gist}.  The operation is simple, efficient, and
structurally well-behaved: it preserves the monoidal structure of the graph
category, satisfies an exact duality, and supports a clean concept-blending
interpretation.  A Scala implementation is provided in \Cref{app:scala}.
...
\subsection*{The Fixed Point of Iterated Expansion}

Expanding every pair of nodes in $G$ simultaneously and repeating yields a
sequence of graphs $G_0 \subset G_1 \subset G_2 \subset \cdots$.  The
\emph{blending closure} $G^* = \bigcup_{n \geq 0} G_n$ is the fixed point of
iterated expansion.  Starting from a finite graph, $G^*$ is necessarily
infinite: at each step the fresh-node naming scheme (using the reflective
quotation $D_{M,F} = \langle @(M \tensor F) \rangle$) introduces nodes whose
names are finite binary trees of original names, so the node set of $G^*$ is
isomorphic to the \emph{free magma} over the original node set of $G_0$.

\paragraph{Small-world structure and diameter.}

Each expansion step reifies a relational composition: $D_{M,F}$ bridges the
in-neighbourhood of $M$ to the out-neighbourhood of $F$, which is precisely
the set of pairs reachable via a path through $M$ then $F$.  Iterating this
construction shortcuts all longer paths by introducing intermediate blend
nodes.  Concretely, if $A \to \cdots \to M \to \cdots \to F \to \cdots \to B$
is a path in $G_0$, then after the appropriate expansion steps there exists a
path of length~2 through a blend node: $A \to D_{M,F} \to B$.  This suggests
that the limit graph $G^*$ has a strong small-world character.  We state the
following as a conjecture.

\begin{conj}[Diameter of the blending closure]
\label{conj:diameter}
Let $G_0$ be a finite connected directed graph with at least one edge, and let
$G^*$ be its blending closure under iterated expansion applied to all pairs of
nodes.  Then for any two nodes $u, v \in G^*$ such that $v$ is reachable from
$u$ in $G^*$, the distance $d(u, v) \leq 3$.  In particular, if $G_0$ is
strongly connected then $G^*$ has diameter at most~$3$.
\end{conj}

The bound of~$3$ rather than~$2$ accounts for the fact that fresh blend nodes
are not themselves in $G_0$, so a path from an original node to another
original node may require one step into a blend node, one hop across it, and
one step out.

\paragraph{Path algebra and the quantale structure of $G^*$.}

The path semiring of $G^*$---whose elements are sets of directed paths, with
union as addition and path concatenation as multiplication---is a
\emph{unital quantale}: it is complete under arbitrary joins (union of path
sets), and the identity paths at each node provide the unit.  This means the
fixed point of iterated expansion is not merely an infinite graph but an object
that \emph{generates} a quantale from first principles, with no additional
...
Goertzel has argued~\cite{goertzel2023} that Michael Timothy Bennett's notion of
\emph{weakness}---a measure of the degree to which a hypothesis constrains its
subject as weakly as possible while remaining consistent with the
evidence~\cite{bennett2023,bennett2022}---can be captured within a quantale-based
...
The observations above close a circle: the blending closure $G^*$ generates
precisely the kind of quantale over which Goertzel's framework operates.  The
path quantale of $G^*$ provides a canonical ambient structure in which weakness
between any two concepts is well-defined and computable as the join of
path-weights between their corresponding nodes.  The blending procedure is
therefore self-bootstrapping: iterating $\expand$ to its fixed point produces
the quantale-enriched setting required to interpret the resulting blended
concepts in terms of weakness and similarity, without any external enrichment
being imposed.

The expansion operation itself extends naturally to the quantaloid setting.
The port sets $\inSrc$ and $\outTgt$ generalise to \emph{weighted port sets}:
for each source $b$ of an edge into $M$, the edge carries a quantale value
$q \in Q$ representing the strength of that relationship.  The blended node $D$
inherits these weighted connections directly:
\[
  Q(b,\, D) \;=\; Q(b,\, M)
  \qquad
  Q(D,\, b) \;=\; Q(F,\, b)
\]
and analogously for $S$.  We conjecture that this makes the blending operation
a morphism of quantaloid-enriched graphs.  Establishing this conjecture
formally---in particular verifying the enriched composition axioms for $D$ and
$S$---is a priority for future work.

\subsection*{Further Directions}

Additional future directions include: (1) proving or disproving
Conjecture~\ref{conj:diameter} and characterising the full metric structure of
$G^*$; (2) investigating whether $G^*$ modulo bisimulation (in the sense of the
rho calculus) yields a compact canonical form with the same path-algebraic
properties; (3) equipping the operation with a categorical semantics as a
functor on a graph topos; (4) integrating the construction with the rho
calculus reduction semantics to give blended concepts a dynamic,
rewriting-based life-cycle; and (5) applying the operation to large-scale
knowledge graphs to evaluate its utility for automated analogy and metaphor
generation.

% --- References ---
\begin{thebibliography}{9}

\bibitem{meredith2025gist}
L.~G.~Meredith,
\textit{Graph DSL Grammar (LBNF)},
GitHub Gist, 2025.
\url{https://gist.github.com/leithaus/3feea82ddbdfb89bfc1f8482296ba6ba}
...
\bibitem{bennett2023}
M.~T.~Bennett,
\textit{The Optimal Choice of Hypothesis Is the Weakest, Not the Shortest},
arXiv:2301.12987, 2023.
\url{https://arxiv.org/abs/2301.12987}

\bibitem{bennett2022}
M.~T.~Bennett,
\textit{Computable Artificial General Intelligence},
arXiv:2205.10513, 2022.
