...
\S\ref{sec:dilation} the containment.
...
(Definition~\ref{def:linear}). In program text the third is written
\texttt{o-}, the reverse of linear logic's lollipop \cite{Girard}: the head sits
at the bound name, as it does in $\leftarrow$, because the datum flows from the
channel to the variable.
\end{definition}
...
A term is \emph{well formed} when every receipt using $\lolli$ is on quantum
channels (Definition~\ref{def:namespace}) and satisfies
Definition~\ref{def:linear}; every clause taking values outside $\Bool$ is at a
receipt on quantum channels; and every such clause is contractive
(Definition~\ref{def:contractive}). Nothing else is refused.
\end{definition}
