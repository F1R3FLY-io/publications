...
\normalsize Version 4: dilation, heralds, and the price of erasure}
...
matrices. The whole program is given --- the three gadget contracts, modular
exponentiation, and the instance at $N=15$ --- emitted from the same gate list
the simulator executes, so the code and the numbers cannot drift apart.
Universality is easy and modular exponentiation is ordinary rholang,
...
This version contains the hazard by construction rather than by side condition.
Every receipt is dilated at the site --- not at a global step, which
interleaving makes ill-defined --- and the dilation's ancilla is not traced, nor
projected, but \emph{addressed}: it is an ordinary message on a channel named by
quoting the site, carrying one bit that says whether the step preserved norm. We
show that this bit is the only thing the message may carry, since a herald naming
the candidate would destroy interference at every site, and that heralding is
therefore inert exactly on the fragment where isometry is decidable. Under the
amendment no step loses norm, so post-selection ceases to be an operation of the
semantics and becomes a retry loop the programmer writes and the resource ledger
charges: the filter that was free costs $2^n$ firings. We also record why we do
not take the obvious alternative --- restricting to the linear fragment, where
every bound name is used exactly once --- namely that the circuit encoding's
expressiveness is bought with weakening, and a weakening-free step is
block-diagonal, hence an isometry and not a gate.
...
\subsection{Revision note}\label{ssec:revision}

This is the fourth version of the note. Sections~\ref{sec:fa}--\ref{sec:shor}
and \S\S\ref{sec:pln}--\ref{sec:forager} are unchanged in substance. What is new
is \S\ref{sec:dilation}, and one correction it rests on.

The correction is to \S\ref{ssec:fork}. That section offered three resolutions of
the degenerate race and took the third, the unnormalised reading with the Born
rule at observation. The third is not an alternative to the second: it is the
second with the branch register traced out, and the trace is where the
post-selection of \S\ref{ssec:hazard} enters. Remark~\ref{rem:concession} states
this. The presentation of \S\ref{ssec:fork} is left standing with a pointer,
rather than rewritten, so that the reasoning which led to the defect remains
legible.

What follows from it is that the defect wants a construction and not a side
condition, and \S\ref{sec:dilation} supplies one. Definition~\ref{def:isometric}
survives but is demoted: it no longer refuses programs, and
Proposition~\ref{prop:decidable} becomes a compiler optimisation rather than an
admission test. Obligation~\ref{ob:unitary} of the previous version, which asked
for a static discipline enforcing isometry, is withdrawn and replaced.
...
the classical part costs nothing (Corollary~\ref{cor:modexp}). \S\ref{ssec:fullterm}
gives the term in full rather than in lemmas, generated from the gate list the
simulator runs. But with clauses
unconstrained the fragment decides $\mathsf{PP}$ by post-selection
(Proposition~\ref{prop:postbqp}).

\item \textbf{Heralds, and the containment} (\S\ref{sec:dilation}). The
overshoot is closed by construction. Every receipt is dilated at the \emph{site},
since Proposition~\ref{prop:causal} makes a global step the wrong unit, and the
dilation's ancilla is emitted as an ordinary message on a channel named by
quoting the site. Proposition~\ref{prop:defectbit} shows the message may carry
the defect bit and nothing else, on pain of destroying interference everywhere;
Corollary~\ref{cor:inert} shows heralding is therefore inert exactly on the
fragment Proposition~\ref{prop:decidable} identifies, so the amendment can be
uniform. Under it, post-selection is not a step
(Proposition~\ref{prop:nopostsel}) but a retry loop, and the loop is charged:
the free filter of \S\ref{ssec:hazard} costs $2^n$ firings
(Proposition~\ref{prop:attempts}, Conjecture~\ref{conj:ledger}).

\item \textbf{A negative result about linearity} (\S\ref{ssec:linear}).
Restricting to the fragment in which every bound name is used exactly once makes
isometry structural, and is not the right move: the second erasure that
Lemma~\ref{lem:1q} needs is bought by weakening, and a weakening-free step is
block-diagonal in the input basis (Proposition~\ref{prop:blockdiag}), hence an
isometry and not a gate. Theorem~\ref{thm:scattering} is unaffected, since its
continuations are already linear --- the two erasures come apart, and only one
...
could be sought. It does not prove Conjecture~\ref{conj:ledger}, whose soft
point is named in Remark~\ref{rem:adaptive}, and it does not develop the
mode-encoded alternative of Remark~\ref{rem:dualrail}. It
...
\emph{Version 4 note.} The trichotomy is not one; see
Remark~\ref{rem:concession}. The third option is the second with the register
traced out, and \S\ref{sec:dilation} replaces the choice below with a
construction. The paragraph is left as first written because the defect it
introduces is instructive.
...
Throughout, $\leftarrow$ is the \emph{linear} receipt: firing consumes the
continuation as well as the datum. Rholang also offers a persistent receipt,
written \texttt{<=}, which is not consumed. Linear is the default here, and in
one place the choice is load-bearing rather than stylistic.

Under a persistent receipt the term of \S\ref{sec:stage1} is not a race at all.
Both messages are eventually consumed, the receipt survives in either branch, and
the two derivations reconverge on
$P\{@Q_1/y\} \mid P\{@Q_2/y\} \mid \textsf{for}(y \Leftarrow x)P$. The two
firings consume disjoint data and no shared continuation, so they are independent
in the sense of Definition~\ref{def:causal}: the square is filled and there is
nothing left to weight. Equation~\eqref{eq:race}'s $w_1, w_2$ would then be
describing an \emph{ordering} rather than a choice. It is linearity that makes
the surviving message a which-path record, and so linearity that
Proposition~\ref{prop:stage1} rests on.

Elsewhere persistence is harmless but idle. A persistent join still consumes all
the data at its channel, and the surviving receipt is identical across
candidates, so it distinguishes none of them; Theorem~\ref{thm:scattering} goes
through unchanged and the site is merely left inert with nothing further to
receive.
\end{remark}
...
\subsection{The term, in full}\label{ssec:fullterm}

The gadgets above are stated as lemmas, and lemmas are easy to believe and hard
to run. This subsection gives the whole program. It is not transcribed: the
listings below are emitted by \texttt{shorterm.py} from the same gate list that
\texttt{gradedsim.py} executes to produce Table~\ref{tab:shor}, so the code and
the numbers cannot drift apart. Running the emitted list reproduces the
distribution of Table~\ref{tab:shor} with a worst deviation of $0$.

Three conventions. A \texttt{contract} is a persistent receipt on an installer
channel; the receipt it installs on a wire is linear, so by
Remark~\ref{rem:linear} the persistence never touches a race. Crisp guards are
written in brackets and multiply graded scalars, which is the normal form of
Theorem~\ref{thm:dnf}; \texttt{cis} denotes $\theta \mapsto e^{i\theta}$, a
scalar constant of $\V = \Cx$. Channels are passed as data in the usual way,
so an argument is dereferenced where it is used as a channel.
...
// Modular exponentiation: one candidate at every step, clause value 1.
...
The instance follows. Every wire carries an explicit stage index, because
gadgets compose in series on \emph{fresh} channels
(Proposition~\ref{prop:dag}): \texttt{q3\_5} is the fourth wire after five
gates. The indices are the circuit's wires, drawn out.

\begin{lstlisting}
new q0_0, q0_1, q0_2, q0_3, q0_4, q0_5, q0_6, q0_7, q1_0, q1_1, q1_2,
    q1_3, q1_4, q1_5, q1_6, q1_7, q2_0, q2_1, q2_2, q2_3, q2_4, q2_5,
    q2_6, q2_7, q3_0, q3_1, q3_2, q3_3, q3_4, q3_5, q3_6, q3_7, q4_0,
    q4_1, q5_0, q5_1, q6_0, q6_1, q7_0, q7_1, done in {
    ...
    // 2. modular exponentiation, entangling work with counting
    //    y0..y3 are discarded: the work register is overwritten
    for( x0 <- *q0_1 & x1 <- *q1_1 & x2 <- *q2_1 & x3 <- *q3_1
       & y0 <- *q4_0 & y1 <- *q5_0 & y2 <- *q6_0 & y3 <- *q7_0 ) {
      new r in {
        modexp!( *r, 7, 8 * *x0 + 4 * *x1 + 2 * *x2 + *x3, 15 ) |
        for( m <- r ) {
          q0_2!(*x0) | q1_2!(*x1) | q2_2!(*x2) | q3_2!(*x3) |
          q4_1!( (*m / 8) % 2 ) | q5_1!( (*m / 4) % 2 ) |
          q6_1!( (*m / 2) % 2 ) | q7_1!( *m % 2 )
        }
      }
    } |
    ...
    hadamard!( *q0_2, *q0_3 ) |
    cphase!( *q1_2, *q0_3, *q1_3, *q0_4, 2*pi/4 ) |
    cphase!( *q2_2, *q0_4, *q2_3, *q0_5, 2*pi/8 ) |
    cphase!( *q3_2, *q0_5, *q3_3, *q0_6, 2*pi/16 ) |
    hadamard!( *q1_3, *q1_4 ) |
    cphase!( *q2_3, *q1_4, *q2_4, *q1_5, 2*pi/4 ) |
    cphase!( *q3_3, *q1_5, *q3_4, *q1_6, 2*pi/8 ) |
    hadamard!( *q2_4, *q2_5 ) |
    cphase!( *q3_4, *q2_5, *q3_5, *q2_6, 2*pi/4 ) |
    hadamard!( *q3_5, *q3_6 ) |
    swap!( *q0_6, *q3_6, *q0_7, *q3_7 ) |
    swap!( *q1_6, *q2_6, *q1_7, *q2_7 ) |

    // 4. read the counting register
    for( c0 <- *q0_7 & c1 <- *q1_7 & c2 <- *q2_7 & c3 <- *q3_7 ) {
      done!( 8 * *c0 + 4 * *c1 + 2 * *c2 + *c3 )
    }
}
\end{lstlisting}

\begin{remark}[What the listing exposes]\label{rem:listing}
Three things are visible here that the lemmas state but do not show. The first
is that the whole quantum content of the program sits in the \texttt{where}
clauses of three contracts, and everything else is rholang. The second is that
the ancilla patterns \texttt{u2}, \texttt{ua2}, \texttt{ub2} are bound and never
used, and neither are \texttt{y0}--\texttt{y3} in the modular exponentiation
step; those four discards are the weakening that \S\ref{ssec:linear} shows the
construction cannot do without. The third is that the term contains no
scheduling of any kind. The order of the transform is carried entirely by the
stage indices --- each gadget waits for its input channel --- so the twelve
lines of \S\S2--3 may be written in any order at all without changing the
program.
\end{remark}
...
--- far more than Shor, and far more than any device. Something must contain it.

The previous version of this note responded with a side condition, stated next.
That response was insufficient, for the reason given in
Remark~\ref{rem:concession}: the deficiency it names is not a property of
badly-chosen clauses but of a semantics that defers normalisation, so it wants a
construction rather than a filter. \S\ref{sec:dilation} supplies the
construction; the condition below survives as the classification of the sites at
which that construction is trivial.
...
In version 4 the stratification keeps that role and loses its other one. It no
longer decides which programs are admissible --- Corollary~\ref{cor:inert} shows
it decides which heralds a compiler may erase.
...
\S\ref{ssec:hazard} left the fragment with a containment requirement and no
mechanism. This section supplies the mechanism. It is not a restriction on which
programs may be written; it is an amendment to the reduction rule that makes
every step norm-preserving, together with an accounting rule that charges for the
part of a program that would otherwise have been free.

\subsection{The fork of \S\ref{ssec:fork} was not a trichotomy}\label{ssec:concession}

The first version of this note offered three resolutions of the degenerate race
and took the third. That presentation was wrong in a way worth stating plainly,
because the correction is what drives everything below.

\begin{remark}[Two options, not three]\label{rem:concession}
The unnormalised reading is not an alternative to the branch register; it
\emph{is} the branch register with the register traced out. Deferring
normalisation to observation and then dividing by the surviving norm is the same
operation as retaining the derivation index, discarding it, and conditioning on
the survivors. Post-selection is the deferral of normalisation, and nothing else.
\end{remark}

The consequence is that \S\ref{ssec:hazard}'s hazard and the absence of a
per-step normalisation are one defect seen from two sides. Definition
\ref{def:isometric} responded to it with a side condition where a construction
was available, and \S\ref{sec:semantics} already applies that construction once:
Definition~\ref{def:fict}'s fictitious identity jump at normal forms exists
precisely to complete a deficient row so that histories of different lengths are
comparable. What follows does uniformly what the semantics already does in one
special case.

\subsection{The defect, dilated per site}\label{ssec:defect}

\begin{definition}[Defect operators]\label{def:defect}
Let $A$ be the clause matrix of a receipt site in the sense of
Definition~\ref{def:isometric}, and suppose $\|A\| \leq 1$. Its \emph{defect
operators} are
\[
  D_A := \sqrt{\Id - A^{\dagger}A},
  \qquad
  D_{A^{\dagger}} := \sqrt{\Id - AA^{\dagger}} .
\]
\end{definition}

\begin{definition}[Heralded step]\label{def:heraldedstep}
The \emph{heralded step} at a site with clause matrix $A$ is the map
\[
...
\]
where the second factor is a fresh two-dimensional register attached to the
site. Its unitary completion is the Julia--Halmos block
\[
  U_A \;=\;
  \begin{pmatrix} A & D_{A^{\dagger}} \\ D_A & -A^{\dagger}\end{pmatrix}.
\]
\end{definition}

The two-defect form is used rather than the familiar one-defect shorthand
because a clause matrix arising from a directed rewrite need not be normal, and
the shorthand is only unitary when it is.

\begin{proposition}[The dilation is faithful]\label{prop:dilation}
For every contraction $A$, the map $V_A$ is an isometry and $U_A$ is unitary;
and the $\mathsf{ok}$ block of $U_A$ is $A$ itself, unaltered.
\end{proposition}

\begin{proof}
$V_A^{\dagger}V_A = A^{\dagger}A + D_A^{\dagger}D_A = \Id$ by
Definition~\ref{def:defect}; the block identities for $U_A^{\dagger}U_A$ are the
standard ones and use $AD_A = D_{A^{\dagger}}A$. The second claim is immediate
from the shape of the block, and is what makes the amendment conservative: on
the branch where nothing was lost, the semantics of \S\ref{sec:semantics} is
unchanged.
\end{proof}

Computed over the filter clause $\mathrm{diag}(1,10^{-3})$ together with $200$
pseudorandom contractions: the worst deviation of $U_A^{\dagger}U_A$ from the
identity is $2.220\times 10^{-15}$, and the worst deviation of the
$\mathsf{ok}$ block from $A$ is exactly zero.

\begin{remark}[Contraction is the whole bill]\label{rem:contraction}
Dilation requires $\|A\| \leq 1$: a clause may not manufacture weight. That is a
scale bound, not a reversibility condition, and its $\Rnn$ shadow is
substochasticity, which the Gillespie reading wants in any case. Completion then
turns the inequality into an equality. Nothing further is asked of a clause ---
in particular, purity of guard evaluation, which the existing condition
evaluator already enforces, is all the reversibility the construction consumes.
\end{remark}

\begin{remark}[Per site, not per step]\label{rem:persite}
A companion note dilates the \emph{global} one-step operator, adjoining one
ancilla per step and recovering the $n$-step map by post-selecting all ancillas
on zero. That construction is correct for a sequential machine and wrong here,
for a reason internal to this note: Proposition~\ref{prop:causal} says a global
step is not the right unit, since the number of steps taken to reach a
configuration depends on the interleaving and not on the causal history --- the
measured inflation is $2$, $6$ and $24$ for two, three and four independent
sites. An ancilla count indexed by global steps is therefore not an invariant of
the derivation. Indexing instead by \emph{site firing} makes the ancilla family
coindexed with the derivations themselves, which is the same index the
compilation route uses for its branch register. We record the difference rather
than reconciling it here; the two notes disagree about what a step is, and this
one is committed by Proposition~\ref{prop:causal}.
\end{remark}

\begin{remark}[The fictitious jump, generalised]\label{rem:fictitious}
When $A$ is diagonal in the configuration basis --- the case of a clause that
gates rather than mixes --- $D_A$ is diagonal too, and the $\mathsf{fail}$
branch is literally the unfired term carrying amplitude
$\sqrt{1-|a|^2}$. Definition~\ref{def:fict}'s fictitious identity jump is then
exactly the $\mathsf{fail}$ branch of a site whose success amplitude is zero.
One construction, two uses.
\end{remark}

\subsection{The herald}\label{ssec:herald}

The register of Definition~\ref{def:heraldedstep} is not a metatheoretic object.
It is a message.

\begin{definition}[Herald channel]\label{def:heraldchan}
For a receipt site $r$, its \emph{herald channel} is $h(r) := @r$, the quotation
of the site. Freshness is by quoting, so the address is deterministic, computable
before execution, and identical across the candidates of the site.
\end{definition}

\begin{definition}[Heralded reduction]\label{def:heraldedred}
The communication rule is amended so that the outcome of a firing at $r$ carries
one additional message: $h(r)!(\mathsf{ok})$ on the $A$ branch, and
$h(r)!(\mathsf{fail})$ on the $D_A$ branch, where the latter leaves the term
component in the input sector. No term former is added, and no new sort of name:
$\mathsf{ok}$ and $\mathsf{fail}$ are ordinary processes and $h(r)$ is an
ordinary channel.
\end{definition}

Two things now have to be checked, because a herald is a message and a message
is part of a configuration. By Corollary~\ref{cor:whichpath} anything a
continuation writes that distinguishes its inputs destroys the interference at
that site. A herald is written by every site, so if it distinguished anything it
would destroy everything.

\begin{proposition}[The herald carries the defect bit only]\label{prop:defectbit}
Let $r$ be a site whose candidates satisfy the symmetry condition of
Definition~\ref{def:sym}. If the herald emitted at $r$ ranges over
$\{\mathsf{ok},\mathsf{fail}\}$, the outcomes of coinciding candidates remain
congruent and Theorem~\ref{thm:scattering} holds unchanged. If instead the
herald names the candidate, the outcomes separate, every fibre of $\Out$ becomes
a singleton, and the coherent sum degenerates to the incoherent one at every
site of the program.
\end{proposition}

\begin{proof}
Immediate from Corollary~\ref{cor:whichpath}: coincidence of outcomes is
congruence of the resulting terms, and appending the same message to congruent
terms preserves congruence, while appending distinct messages destroys it.
\end{proof}

So the herald is the branch register \emph{quotiented} onto success --- the
coarsening that forgets which candidate fired and remembers only whether the
step preserved norm. Measured on the minimal interferometer of
\S\ref{ssec:interferometer}, with relative phases $0$, $\pi$ and $\pi/2$: the
bare coherent weights are $2.000000$, $0.000000$, $1.000000$; with an
$\mathsf{ok}$ herald appended they are $2.000000$, $0.000000$, $1.000000$; with
a candidate-naming herald they are $1.000000$, $1.000000$, $1.000000$. The
cancellation survives the first and does not survive the second.

\begin{corollary}[Uniform heralding is free where it should be]\label{cor:inert}
At a site whose clause is isometric in the sense of
Definition~\ref{def:isometric}, the $\mathsf{fail}$ branch carries amplitude
zero, so the herald reads $\mathsf{ok}$ on every candidate and is inert. Hence
heralding every receipt costs nothing exactly on the fragment
Proposition~\ref{prop:decidable} can identify.
\end{corollary}

This is why the amendment can be uniform rather than selective. One does not have
to decide in advance which sites are deficient: the deficient ones herald
informatively and the rest herald constantly, and Proposition~\ref{prop:decidable}
is demoted from an admission criterion to an optimisation, telling a compiler
which heralds may be elided.

\begin{remark}[Uncompute, or read]\label{rem:uncompute}
A herald that is never consumed sits in the term as a record. At an isometric
site the record is constant and therefore not a record at all. At a deficient
site it is a genuine one, and leaving it unread is a decoherence event --- which
is the correct semantics for a program that discards information, not a
malformedness to be refused. Reading it is the retry. This is the operational
content of the requirement that a dilation ancilla be uncomputed rather than
traced or projected: here it is neither traced nor projected but
\emph{addressed}.
\end{remark}

\subsection{Post-selection becomes a program}\label{ssec:rus}

With Definition~\ref{def:heraldedred} in force, a programmer who wants the
behaviour that Proposition~\ref{prop:postbqp} obtained for free must write it:
...
Under Definition~\ref{def:heraldedred} no step of the semantics loses norm.
Consequently the renormalisation performed at observation in
\S\ref{ssec:pathsum} has nothing to do, and the operation
$\mathrm{diag}(1,\epsilon)$ of Proposition~\ref{prop:postbqp} is no longer a
step: it is a heralded step followed by a projection onto $\mathsf{ok}$, and
projection is not among the constructs of the semantics. It is available only as
the retry loop above, whose failures are firings.
\end{proposition}

Definition~\ref{def:isometric} accordingly loses its status as a containment
requirement and becomes what it always described: a classification of sites, the
isometric ones being those at which the dilation is trivial. Nothing is refused.

\subsection{The price}\label{ssec:price}

The retry loop is a program, and programs are funded. The resource discipline
already in place admits a payment exactly when the balance covers it, and a loop
that does not terminate within its endowment simply stops.

\begin{proposition}[Expected attempts]\label{prop:attempts}
Let a site herald $\mathsf{ok}$ with probability $p$, and let the retry loop
restore the site's inputs before each attempt, so that attempts are independent.
Then the expected number of firings before an $\mathsf{ok}$ is $1/p$, and the
expected charge is $\kappa/p$ where $\kappa$ is the cost of one firing.
\end{proposition}

Applied to the hazard of \S\ref{ssec:hazard}, whose figures were computed for
that section and are here re-read as a bill:

\begin{center}
\begin{tabular}{@{}rrrr@{}}
\toprule
$n$ & surviving norm & $\Pr[\text{all heralds } \mathsf{ok}]$ & expected attempts \\
\midrule
$4$  & $0.250001$ & $0.062500$ & $16$ \\
$6$  & $0.125000$ & $0.015625$ & $64$ \\
$8$  & $0.062500$ & $0.003906$ & $256$ \\
$10$ & $0.031250$ & $0.000977$ & $1024$ \\
\bottomrule
\end{tabular}
\end{center}

The filter that was free in \S\ref{ssec:hazard} costs $2^{n}$ firings. This is
the whole of the containment argument, and it is worth stating as such.

\begin{conjecture}[Containment by the ledger]\label{conj:ledger}
A graded program whose execution is funded by a budget polynomial in the size of
the term does not decide $\mathsf{PP}$. The post-selection witnessing
Proposition~\ref{prop:postbqp} requires a success probability exponentially small
in the number of wires, hence by Proposition~\ref{prop:attempts} an
exponentially large expected charge, which no polynomial endowment funds.
\end{conjecture}

\begin{remark}[Where the conjecture is soft]\label{rem:adaptive}
Proposition~\ref{prop:attempts} assumes attempts are independent. A retry loop
whose success probability depends on what earlier attempts returned is not
geometric, and a sufficiently clever adaptive schedule is exactly the resource
that makes linear optics universal. So Conjecture~\ref{conj:ledger} and
Obligation~\ref{ob:klm} are the same question asked with opposite hopes: one
wants adaptivity to be cheap enough for universality and expensive enough for
containment, and the two demands meet somewhere that has not been located.
\end{remark}

\subsection{Why not the linear fragment}\label{ssec:linear}

There is an alternative to paying at the herald, and it should be recorded
because it is the first thing a reader who knows linear logic will propose:
restrict the calculus so that the defect cannot arise. Weakening --- a bound name
not used in the continuation --- is the partial trace, and contraction is the
copy. A discipline in which every name bound by a for-comprehension is used
exactly once aligns the term language's structural rules with those of the
target, and makes the isometry of a step structural rather than conditional.

The reason not to take it is that the expressiveness of \S\ref{sec:shor} is
bought with weakening, and precisely with weakening.

\begin{proposition}[Weakening-free steps are block-diagonal]\label{prop:blockdiag}
Suppose every name bound at a site is used at least once in the continuation, so
that the payload multiset of the outcome is the consumed multiset together with
whatever the continuation writes literally. Then outcomes arising from distinct
input configurations are never congruent, the one-step operator is block-diagonal
with respect to the input basis, and no site realises a basis-changing map.
\end{proposition}

\begin{proof}[Proof sketch]
Distinct input basis states present distinct payload multisets at the site's
channels. If nothing is discarded, the outcome's payload multiset determines the
input's, so no outcome lies in the image of two distinct inputs. The fibres of
$\Out$ over distinct inputs are therefore disjoint, and disjoint supports give a
block-diagonal matrix.
\end{proof}

The gate gadget of Lemma~\ref{lem:1q} discards both the input datum and the
unselected ancilla, and Remark~\ref{rem:twoerasures}'s second erasure --- the
coincidence of outcomes across distinct \emph{inputs}, as opposed to across
distinct candidates --- is exactly what the discard buys. Patching the gadget by
re-emitting what it discarded does restore the isometry, structurally and with no
side condition; it restores it by making the map block-diagonal. Computed on the
patched gadget with a clause that was to have been a Hadamard: the two rows have
supports $\{0,1\}$ and $\{2,3\}$, the intersection is empty, and
$\|MM^{\dagger}-\Id\| = 2.220\times10^{-16}$. An isometry, and not a gate.

\begin{remark}[The two erasures come apart]\label{rem:twolinear}
The restriction does not damage \S\ref{sec:erasure}. The continuations there ---
$z!(*y_1) \mid z!(*y_2)$ and its asymmetric sibling --- already use each bound
name exactly once, so Theorem~\ref{thm:scattering} is a theorem of the linear
fragment. Erasure across candidates is achieved by symmetry and is linear;
erasure across inputs is achieved by discard and is not. Only the second is
implicated.
\end{remark}

\begin{remark}[The road not taken]\label{rem:dualrail}
Proposition~\ref{prop:blockdiag} is a statement about the encoding of
Definition~\ref{def:wire}, in which a qubit is \emph{which datum} sits on a
channel. Encode instead by \emph{which channel} a fixed datum sits on, and mixing
becomes redistribution, which is what a linear join does; the payload multiset is
then conserved by construction and the model is linear optics rather than
circuits. Reflection makes this natural rather than laborious, since a received
name may be used as the channel of a send and so the output rail can be selected
by the candidate. That route buys unitarity by construction and pays for
universality at Obligation~\ref{ob:klm} instead. It is a different note, and this
one keeps the circuits.
\end{remark}

\begin{principle}[Where the bill falls]\label{prin:bill}
A design may have structural unitarity or circuit-shaped expressiveness, and the
choice determines where the price of erasure is paid: at the encoding, or at the
herald. Paying at the herald leaves the language alone and makes the payment
...
\paragraph{Heralds.} The amendment of Definition~\ref{def:heraldedred} touches
the communication rule and nothing else: on firing, emit one further message on
the quotation of the site. Two consequences for an implementation. The herald's
payload must be the defect bit, never the candidate index
(Proposition~\ref{prop:defectbit}); and heralds at isometric sites should be
elided at elaboration, which is the residual job of
Proposition~\ref{prop:decidable}. Unelided heralds accumulate as messages, and
a garbage discipline for them is a real cost that this note does not price.
...
\emph{Partly discharged in version 4.} Remark~\ref{rem:concession} settles that
the unnormalised reading is the branch register traced out, so the fork of
\S\ref{ssec:fork} has two branches and not three, and
\S\ref{sec:dilation} takes the register and keeps it addressable. What remains
open is the observable question that motivated the original choice: whether the
$m$-fold degeneracy of a race should be read as weight $m$ or $m^2$ is now a
question about which herald a program reads, and that reading should be worked
out against Remark~\ref{rem:permanent}.
\end{obligation}

\begin{obligation}[Enforcing isometry --- withdrawn]\label{ob:unitary}
\emph{Withdrawn in version 4 and replaced by
Obligation~\ref{ob:price}.} The previous version asked for a static discipline
refusing non-isometric clauses in a declared coherent mode.
\S\ref{sec:dilation} refuses nothing: the defect is dilated rather than
excluded, so a non-isometric clause is a legal program with a herald worth
reading. Definition~\ref{def:isometric} and
Proposition~\ref{prop:decidable} survive as the classification telling a
compiler which heralds are constant.
\end{obligation}

\begin{obligation}[The price, made exact]\label{ob:price}
Conjecture~\ref{conj:ledger} claims that a polynomially funded graded program
does not decide $\mathsf{PP}$. Two gaps. Proposition~\ref{prop:attempts} assumes
independent attempts, and Remark~\ref{rem:adaptive} notes that an adaptive retry
schedule is not geometric --- which is the same resource
Obligation~\ref{ob:klm} needs for universality, so the two must be settled
together. And the charge for a herald has not been fixed against the existing
cost model; until it is, `polynomially funded' names an intention rather than a
judgement.
\end{obligation}

\begin{obligation}[Herald garbage]\label{ob:garbage}
Heralds at deficient sites accumulate as messages. Left unread they decohere
their sites, which is semantically correct and operationally a leak. Whether a
discipline exists that collects them without either reading them --- which
commits to an outcome --- or discarding them --- which is the trace this note
set out to avoid --- is open, and is the point at which
Remark~\ref{rem:uncompute}'s `neither traced nor projected but addressed'
stops being free.
\end{obligation}
...
a verdict about advantage ultimately rests. Version 4 sharpens it in two
directions. Remark~\ref{rem:adaptive} makes it the same question as
Conjecture~\ref{conj:ledger}, asked with the opposite hope. And
Remark~\ref{rem:dualrail} identifies the setting in which it would be asked
natively: under a mode encoding, the linear fragment is linear optics, mixing is
a join, and heralded measurement is already available as a herald read --- so a
mode-encoded graded rho would be the natural home of a KLM-style construction,
and this note's circuit encoding is not.
\end{obligation}
...
and the fragment computes $\mathsf{PP}$.

Version 4 closes that by amending the reduction rule rather than the admissible
programs. The deferral of normalisation was the whole defect --- the
unnormalised reading is the branch register with the register traced out --- so
the register is kept, and kept where a rho program can reach it: as an ordinary
message on a channel named by quoting the site, carrying one bit that says
whether the step preserved norm. Three things fall out. The bit is the only
thing the message may carry, since a herald naming the candidate would write a
which-path record at every site and collapse the coherent sum everywhere. Because
of that, heralding is inert precisely where isometry holds, so the amendment can
be applied uniformly and the isometry condition, no longer needed to refuse
anything, becomes the classification telling a compiler which heralds to erase.
And post-selection stops being an operation of the semantics: it is a retry loop
that a programmer writes and a ledger charges, which turns the free filter of
the hazard into a bill of $2^n$ firings.

The obvious alternative was tried and rejected, and the rejection is worth
keeping. Restricting to the fragment in which every bound name is used exactly
once does make each step an isometry structurally, with no dilation and no
herald. It also makes each step block-diagonal in the input basis, because the
coincidence of outcomes across distinct inputs that a gate needs is bought
precisely by discarding the input. The recombiner is untouched --- its
continuations were already linear --- so the two erasures of the design come
apart, and only the one that circuits depend on is implicated. A design may have
structural unitarity or circuit-shaped expressiveness; what the choice fixes is
whether the price of erasure is paid at the encoding or at the herald. Paying it
at the herald leaves the language alone and puts the payment where it can be
read.
