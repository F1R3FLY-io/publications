...
\noindent
By the uniform-signing sugar~\eqref{eq:sugar-uniform}, each
$\signed{\recv{y}{x}{\cdots}}{v}$ expands to
$\signed{\recv{y}{x}{\signed{\cdots}{v}}}{v}$, so the fully
expanded form is:
\begin{equation}
  \mathit{VH}^{\ast}
  \;=\;
  ...
  \label{eq:validator-handler-expanded}
\end{equation}

\noindent
The sugar eliminates the need to manually double every
$\signed{\cdot}{v}$ layer.  In the sugared form, six
validator tokens fund the three for-comprehension steps
(two tokens each: rendezvous + matching per
Section~\ref{sec:for-as-txn}).  More precisely:
one $\signed{\cdot}{v}$ per for-comprehension from the
explicit syntax, plus one per continuation from the sugar
expansion.
