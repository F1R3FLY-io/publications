...
\fancyhead[R]{\small\textsc{Cost-Accounted Rholang}}
\fancyfoot[C]{\thepage}
...
\title{%
  ...
  \large Folding Phlogiston Accounting into the Rholang Core}
\author{%
...
// Signed by Alice; costs 1 Alice-token to invoke
...
credit:
\begin{lstlisting}[style=rholang]
// Signed by Alice; costs 1 Alice-token for the
// orchestrator's own for-comprehension
{ for( amt <- aliceDebit?!( request, ... );
       rcpt <- bobCredit?!( amt, ... ) ){
     P
} }_{ Alice }
...
\begin{center}
  \begin{tabular}{lc}
    \textbf{Signed layer} & \textbf{Tokens} \\
    \midrule
...
\noindent
If Alice's stack is $\mathit{Alice} : \mathit{Alice} :
\mathit{Alice} : S$ (at least three tokens), all three
signed layers are funded and the transaction completes.  If
the stack is only $\mathit{Alice} : \mathit{Alice} : ()$
(two tokens), the debit fires (consuming two tokens: one for
the orchestrator, one for the debit handler) but the credit
handler's signed layer finds no token and \emph{blocks}.
Funds are stranded in the purse.

\subsection{Decomposing the Transaction Cost}
\label{sec:txn-cost-decomp}

Each for-comprehension's token demand decomposes into the
two computational actions of
Section~\ref{sec:for-as-txn}:
...
\noindent
(The $\mathtt{?!}$ operator desugars to a send followed by
a for-comprehension on the return channel, so each
$\mathtt{?!}$ call involves a for-comprehension on each
side --- the caller's and the handler's.)
...
names.  We introduce three auxiliary channels (pure-rho names):
...
$\send{v}{\signed{D}{c}}$ sends the signed deployment to
the validator on channel~$v$.
...
\begin{enumerate}[nosep]
  \item receives a signed deployment on channel~$v$;
...
\noindent
We define the handler as the signed process:
...
\noindent
Each nested $\forc$-comprehension is wrapped in its own
$\signed{\cdot}{v}$, so each step of the handler's execution
consumes one validator token from the validator's own stake.
Three tokens total are needed: one to receive the deployment,
one to look up the client's tokens, and one to execute the
...
rules, it cannot execute unless validator tokens are present.
These tokens represent the validator's \emph{stake}: the
validator must lock up stake tokens to earn the right to
...
\begin{equation}
  \boxed{
    ...
    \underbrace{
      \mathit{VH}
    }_{\text{validator handler}}
    \;\Par\;
    \underbrace{
      v : v : v : ()
    }_{\text{validator stake (3 tokens)}}
    \;\Par\;
    \underbrace{
      C
...
\noindent
The three validator tokens $v : v : v : ()$ are matched
exactly to the three signed layers of the handler: one
token per $\signed{\cdots}{v}$.  This is the bootstrap
invariant: the validator's stake must be at least as deep as
the handler's nesting depth.
...
validator token and receives the signed deployment
(\textbf{Rule~1}):

The signed handler's outermost layer is
$\signed{\recv{\mathit{dep}}{v}{\cdots}}{v}$, and the send
$\send{v}{\signed{D}{c}}$ provides a message on~$v$.  With
token~$v : v : v : ()$, decomposed as
$(v : ()) \Par (v : v : ())$:
\begin{align*}
  &\signed{
    \recv{\mathit{dep}}{v}{\cdots} \Par \send{v}{\signed{D}{c}}
  }{v}
  \;\Par\; v : ()
  \\
  &\quad\red\quad
...
$\quot{\signed{D}{c}}$ and one validator token is consumed.
The system becomes:
\begin{align*}
  &\signed{
    \recv{\mathit{tok}}{\quot{A_c}}{
...
\bigskip
\noindent
\textbf{Step~2} --- The second layer consumes one validator
...
validator token is consumed.  One validator token remains.

\bigskip
\noindent
\textbf{Step~3} --- The innermost layer consumes the last
validator token and executes fee extraction
(\textbf{Rule~1}):
\[
  \signed{\mathit{FeeExtract}}{v}
  \;\Par\; v : ()
...
\begin{enumerate}[label=\textbf{Step \arabic*.}, leftmargin=3em]
  \item \textbf{Validator fuel gate~1} fires on~$n_v$.
    The outermost send in the validator's token chain is
  ...
  \item \textbf{Validator fuel gate~2} fires on~$n_v$.
    One more validator token consumed.
  ...
  \item \textbf{Validator fuel gate~3} fires on~$n_v$.
    Last validator token consumed.  $\mathit{FeeExtract}$
    is now active.
...
\paragraph{Bootstrap economics.}
The validator must hold at least as many $v$-tokens as the
nesting depth of its handler.  In the example, three
$\signed{\cdots}{v}$ layers require three tokens.  In
production, this depth corresponds to the cost of the
validator's own computation: receiving a deploy, looking up
tokens, and executing the fee logic.  The validator's
\emph{stake} is the supply of $v$-tokens it holds; a
validator that exhausts its stake can no longer process
deploys.  Stake replenishment comes from fee revenue
(discussed below) and from external funding by the
...
The example handler processes a single deployment and
halts.  A production validator runs persistently,
re-advertising itself after each deployment.  Using the
standard reflective replication encoding, the persistent
handler is:
\[
  !\, \mathit{VH}
\]
Each iteration consumes validator tokens from stake,
collects fees, and (via the exchange contract) converts
fees to replenish stake --- establishing the economic
feedback loop that sustains the validator.

\paragraph{Connection to Casper CBC.}
In the F1R3FLY platform, the validator's stake
($v$-tokens) is locked via a staking contract that
participates in Casper CBC consensus.  The number of
$v$-tokens held determines the validator's weight in the
consensus algorithm.  The cost-accounted calculus makes
this relationship \emph{intrinsic}: the validator
literally cannot act without tokens, and the rate of
