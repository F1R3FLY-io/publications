...
\section{Decorating the cuts: the factorisation, graded by choice}\label{sec:graded}
...
\cite{weihrauch}, equivalently an altitude in the stalk of \S\ref{sec:landscape} --- required to
...
\S\ref{sec:macro} collapse into a single labelling,
\[
...
macro-component factorisation of \S\ref{sec:macro} is read straight off the decoration: the islands
...
\begin{definition}[$C$-island]\label{def:cisland}
...
\begin{proposition}[The factorisation is a filtration]\label{prop:filtration}
...
partition is the confluent-islands / non-confluent-seas factorisation of \S\ref{sec:macro}; at
...
\S\ref{sec:landscape} the stratification was \emph{external}: a fibration over a category of models,
...
cleavage of a flat jumble'' (\S\ref{sec:flat}) sharpens into the statement that the cleavage
\emph{is} $\chi$, and the jumble is the single $\chi$-decorated term beneath all of its thresholds.

\begin{remark}[Do Smith's whole and our islands nest after all?]\label{rmk:nest}
At $C = \bot$ Smith's biosphere is not one of our islands: selection and predation are seas
(\S\ref{sec:smith}). The filtration reframes the question. There may be a level $C^{*}$ --- the
...
insofar as subject reduction holds (\cite{choicesched}; \S\ref{sec:crux}): if scheduling can climb
...
decision manufactured strength inside. The crux of \S\ref{sec:crux} is therefore exactly the
...
internally, of which threshold in the filtration of \S\ref{sec:graded} we choose to view the term
at. The
...
different invariant --- and, via the filtration of \S\ref{sec:graded}, determine whether they
nonetheless nest at some higher level $C^{*}$ (Remark~\ref{rmk:nest}). \emph{(vi)} Test the
...
$\chi$ and the filtration by choice strength of \S\ref{sec:graded}: prove the monotonicity of
Proposition~\ref{prop:filtration}, establish that the $C$-factorisation is a reduction invariant
...
cuts. Chart and world meet in the decoration: label each cut with the choice strength its race
demands, and the tower reappears as a filtration of one flat term, its strata the level-sets of
that labelling --- the stratification internal, derived, and observer-relative rather than imposed,
with the confluent islands its ground floor and the higher seas the levels at which a stronger
observer is required. Computation, taught by biology, learns to expect this; biology, taught by
computation, learns what it costs and where it can bite. And the single open hinge --- whether a
strong agent, reaching through an aperture, can crack open the determinism of a weaker one --- is
one confluence lemma: whether the graded factorisation is stable as a term runs, equivalently
whether scheduling preserves its type. It would tell us, in one stroke and on both axes, whether
agency is a fixed allotment or something agents continually make and unmake in each other.
...
\bibitem{choicesched} L.~G. Meredith. \emph{From the Axiom of Choice to Fair Scheduling: Toward a
Curry--Howard Correspondence for Nondeterminism}. Research Note, F1R3FLY, 2026.
