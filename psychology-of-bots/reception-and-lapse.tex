...
\textbf{Weakening and contraction are revoked three times over.} Logically, by
linear logic (\S\ref{sec:girard}). Physically, by no-cloning and no-deleting,
with Landauer pricing erasure at $kT\ln 2$ per bit \cite{landauer1961} and
Bennett showing the charge is avoidable only by declining to forget
\cite{bennett1973}. Economically, by cost accounting
\cite{meredithcost}. Three unrelated communities, working with three unrelated
methods across half a century, each arrive at the conclusion that the object may
not be freely duplicated and may not be freely destroyed. If the reading of
\S\ref{sec:church} were merely a figure of speech, one would not expect the
figure to be independently rediscovered by proof theory, thermodynamics and
accounting.
