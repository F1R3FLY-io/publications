...
\newcommand{\rhoc}{\ensuremath{\rho}-calculus}
...
\cite{church1936}. There is no tape. There is no environment at all, not even a
passive one. But closure must not be confused with self-consumption: the fact that
nothing lies outside the term says nothing about the relation obtaining among its
parts, and that relation is directed, asymmetric, and psychologically specific.
...
or an argument according to where it sits. Nothing in the model confers a stable
status on anything, so there is no other---only positions conferred by
juxtaposition. This is a fair description of a world before persons.
...
but the object so named is a lookup table: the world as a dictionary of things
already known. \emph{Side effect} is a pathologising coinage in which one's
...
\midrule
Turing, Church & Neither world nor interior \\
...
The \rhoc{} \cite{meredith2005} changes the shape of the answer rather than its
subject, and three changes read directly as psychology.
...
The historical survey yields a specification, in that each condition below is the
thing some earlier tradition supplied and its reception discarded.
...
resource sensitivity for the reflective act itself. Mortality is not a decoration on this list: an agent that can run
out is an agent for which reflective expenditure is a real decision.
\item \textbf{Compatible with a society.} The agent must be able to hold a
representation of \emph{another} agent's behaviour, or reflection remains solitary
and the resulting system cannot participate in the peer relations that Petri,
Hewitt and Milner established as the real setting of computation. This is the
condition Rosette met and its dispersal cost us.
...
These conditions are jointly satisfiable; a calculus in which names are quoted
processes, equipped with cost accounting and a spatial-behavioural logic, satisfies
them by construction, and the point of stating them as a list is to make the claim
falsifiable rather than to advertise a solution. The substantive prediction is
that recursive self-improvement in any strong sense will arrive, if it arrives, on
a substrate meeting C1--C5, and not by scaling a stack that meets none of them.
...
with an inert world (Turing, Church); an agent with a body and a location but no
...
itself in the act (Smith), that can do so among peers (Rosette), and that can do so
in a closed and priceable loop (the reflective higher-order calculus).
