...
\begin{abstract}
A graph-structured lambda theory (GSLT) is a language presentation: a signature of
sorts and operators, a set of equations, and a set of rewrite rules. This document is a
reference account of what can be built once a language is presented that way. We
develop the definition and two successive enrichments --- \emph{interactive} GSLTs,
...
A language is a triple. Sorts and operators say what can be written; equations say
which writings are the same; rewrites say how writings move. Everything this document
...
lineage of structured operational semantics and climbs a three-rung ladder from
algebraic data types through universal algebra to domain-specific languages.
...
\begin{remark}[Why ``graph-structured'']
The term-formation structure of a presentation makes terms into trees; the equations
identify some of those trees; the rewrites connect the resulting classes by directed
edges. The object of interest is therefore not a set of terms but a graph, and the
theory is graph-structured in the sense that the algebraic structure and the reduction
structure live on the same carrier. The framing of operational semantics in this
algebraic, graph-enriched style follows Stay and Meredith~\cite{StayMeredith2016} and
Baez and Williams~\cite{BaezWilliams2020}.
\end{remark}
...
\label{def:gslt}
A \textbf{graph-structured lambda theory} is a multi-sorted term theory
$G = (\Sigma, E, R)$ consisting of
\begin{itemize}[leftmargin=2em]
  \item a signature $\Sigma$ of sorts and operators,
  \item a set $E$ of equations, imposing a structural congruence $\equiv$, and
  \item a set $R$ of rewrite rules.
\end{itemize}
\end{definition}

The definition is deliberately thin. A GSLT is any multi-sorted rewrite theory; nothing
in Definition~\ref{def:gslt} says that the theory is about computation, or that it has
processes, or that anything interacts with anything. The enrichments of
Sections~\ref{sec:interactive} and~\ref{sec:continued} add exactly the structure that
later constructions require, and the discipline of the present document is to add it
only when a construction needs it.

\subsection{Presentation and classifying theory}
\label{subsec:classifying}

Two descriptions of the same object appear in the literature, and it is worth
reconciling them once so that we may pass between them silently thereafter.

The \emph{presentation} is the syntactic triple of Definition~\ref{def:gslt}. It is what
a developer writes and what a machine accepts.

The \emph{classifying theory} of a presentation is a category $\mathsf{T}$ with finite
limits and a chosen cartesian closed structure, equipped with its subobject fibration
...
We take the presentation as primary throughout, for the practical reason that it is
what one can type; and we use the classifying theory when a construction is more
naturally stated in the internal language. The passage from one to the other is the
usual one and we do not belabour it.
...
The adjective names a strict enrichment. Clauses (i)--(iii) are data and conditions
...
The final column of Table~\ref{tab:verdict} is the discrimination the two enrichments
...
says what happens. Rholang~1.4 has both, and the interest of the arrangement lies in what
