...
\section{Coordination is humanity's superpower}
...
\section{Three layers of robustness}
...
\section{Rholang: a community's shared medium}
...
The rest of this note argues that the integration phase is a real and growing business (\S5), that aiming it at service rather than capture is both right and defensible (\S6), that its differentiator is genuinely hard to copy (\S7--\S8), and --- honestly --- what remains to be quantified (\S12).

\section{Why the demand is real: differentiation, not consolidation}
...
A discovery engine with nothing to discover is a promise, not a product. The supply side is not a second system: as \S3 noted, \texttt{channel!(data)} makes it easy to put anything anywhere, and it is the inverse of the for-comprehension. Write and read --- contribute and find --- are one primitive seen from two sides. Better query is a reason to store; more stored is better query.
