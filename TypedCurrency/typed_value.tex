...
\subsection{What typed value resists}\label{sec:resistances}
...
\subsection{Linearity: why the calculus must reject contraction}\label{sec:linearity}

The resistances enumerated above have so far been gestured at without exhibiting the structural commitment that gives them their force. The commitment is \emph{linearity}: the calculus over typed payments must reject the contraction rule. This subsection makes the commitment explicit and shows how it sharpens the framework's central claims from normative pronouncements into structural impossibilities.

\subsubsection*{Why reputation is non-rivalrous and value is not}

The reputation framework on which this paper builds uses an unrestricted (intuitionistic-shape) sequent calculus. Hypotheses in $\Gamma$ may be used any number of times. A capability claim $\Sig(\mathsf{Bob}) : T_{\text{store}}$ that appears in Alice's environmental context can be cut against Bob's published claim repeatedly without depleting Bob's standing. Contraction is sound on reputational hypotheses because reputation is \emph{non-rivalrous}: Bob's assertion that he provides reliable storage is not consumed by being referenced, and many agents may simultaneously ground their own claims on it.

Value does not have this property and cannot have it under any honest construction. If Alice pays Bob a hundred quid, the hundred quid is consumed in the act of being paid. Bob has it; Alice does not. A proof tree that uses Alice's hundred-quid hypothesis twice would assert that Alice can pay two different bills with the same hundred quid simultaneously --- which is double-spending, the structural defect that every digital currency from DigiCash forward has been designed to prevent. \emph{The value calculus must therefore reject contraction on value hypotheses.}

The natural setting is linear logic. Each hypothesis is used exactly once, and the connectives carry resource interpretation: $\otimes$ for joint possession (both held simultaneously), $\multimap$ for resource-consuming implication (input is consumed to produce output), $\oplus$ for additive disjunction (one or the other, chosen by the producer), $\&$ for additive conjunction (one or the other, chosen by the consumer). The bookkeeping is what a currency requires by definition.

\subsubsection*{A mixed-zone calculus}

A typed-value polity requires both disciplines simultaneously: reputation in an unrestricted regime, value in a linear one. The cleanest formulation is the dual-context (DILL-style) sequent calculus familiar from the mixed linear/non-linear systems of Benton, Wadler, and others. Hypotheses split into two zones:

\[
\underbrace{\Gamma}_{\text{unrestricted}} \;;\; \underbrace{\Delta}_{\text{linear}} \;\vdash\; J
\]

where $\Gamma$ contains capability claims about identities (reputation: shareable without depletion), $\Delta$ contains payment witnesses (value: consumed by transfer), and $J$ is the judgment under construction. Weakening and contraction are admissible on $\Gamma$ and disallowed on $\Delta$. Cut splits accordingly:

\begin{center}
\begin{tabular}{c@{\hspace{2em}}c}
\AxiomC{$\Gamma \,;\, \Delta_1 \vdash \Sig(A) : T_1$}
\AxiomC{$\Gamma,\, \Sig(A){:}T_1 \,;\, \Delta_2 \vdash J$}
\RightLabel{\textsc{Cut-unr}}
\BinaryInfC{$\Gamma \,;\, \Delta_1, \Delta_2 \vdash J$}
\DisplayProof
&
\AxiomC{$\Gamma \,;\, \Delta_1 \vdash v : \Pay(\tau)$}
\AxiomC{$\Gamma \,;\, \Delta_2,\, v{:}\Pay(\tau) \vdash J$}
\RightLabel{\textsc{Cut-lin}}
\BinaryInfC{$\Gamma \,;\, \Delta_1, \Delta_2 \vdash J$}
\DisplayProof
\end{tabular}
\end{center}

The unrestricted cut composes capability claims through environmental dependencies, as in the reputation framework. The linear cut transfers a typed payment from one position in the derivation to another --- and because $v$ disappears from the hypotheses on consumption, it cannot be cut a second time. \emph{The calculus does not double-spend because it has no rule that lets it.}

Vouching, the higher-order bridge between reputation and value developed in \S\ref{sec:constitutive}, gets a clean formulation in this calculus: it is the operation that places a linear stake (a payment witness in $\Delta$) against the truth of an unrestricted capability claim (in $\Gamma$). The constitutive/possessive asymmetry of \S\ref{sec:constitutive} is mirrored exactly in the unrestricted/linear asymmetry of the calculus. Two distinctions, one structural fact.

\subsubsection*{What linearity strengthens}

The arbitrage-as-type-error argument of \S\ref{sec:resistances} becomes structurally sharper under linearity. In an unrestricted calculus, ``the same typed object cannot have two prices'' is a normative statement about extensional equality. In a linear calculus it becomes a claim about resource consumption: a payment witness $v : \Pay(\tau)$ consumed in $\Sig(\mathsf{Alice}) \xrightarrow{v} \Sig(\mathsf{Bob})$ literally cannot be consumed again in a different transaction, because contraction is rejected and no second copy of $v$ exists in the context. Classical arbitrage requires the \emph{same} unit of value to participate in two transactions simultaneously; linearity makes this a structural impossibility rather than a normative violation. \emph{The type system does not flag arbitrage as wrong; it leaves no syntax for it.}

The provenance argument also tightens. In an unrestricted calculus, provenance must be tracked as metadata attached to typed objects. In a linear calculus, provenance is the proof structure itself. The derivation of $v : \Pay(\tau)$ in Bob's possession is precisely the linear path through the calculus that brought $v$ to Bob, with each step a typed transaction consuming and producing payment witnesses. The proof tree \emph{is} the provenance ledger; there is nothing additional to record because the structure of the proof is what records it.

\position{Under unrestricted calculus, arbitrage and laundering are normative violations the system polices. Under linear calculus, they are unrepresentable: the calculus has no syntax for them. The shift from policing to unrepresentability is the structural payoff of honest linearity.}

\subsubsection*{Linearity in the SPJ combinators and the rho substrate}

The SPJ combinator suite of \S\ref{sec:spj} reads naturally as the constructor algebra of a linear calculus. $\texttt{give}$ is involutive transfer with $\texttt{give} \circ \texttt{give} = \texttt{id}$, acting on a linear $\text{Contract}$ type. $\texttt{and}$ is the multiplicative conjunction $\otimes$. $\texttt{or}$ is the additive disjunction $\oplus$. $\texttt{scale}$ is parametric in a non-linear observable but linear in its contract argument. $\texttt{when}$ and $\texttt{anytime}$ introduce temporal quantification over linear hypotheses. The combinators were almost-linear in the original literature; the rho-with-shard substrate (\S\ref{sec:rho-vs-pi}) is where they can be \emph{honestly} linear, because the resource discipline can be enforced at substrate level via the rho calculus's own linearity properties on channel usage. The shard provides operational unforgeability of identifiers; the rho calculus provides operational linearity of channel-based communication; the combinators inherit both and become typed-money primitives whose linearity is operational, not merely advisory.

\subsubsection*{Linearity and the quantum future}

The connection to the complex-amplitude direction flagged in \S\ref{sec:quantum} also tightens here. Linear logic is well-known to be the proof-theoretic skeleton of quantum mechanics: the no-cloning theorem is the structural content of the absence of contraction on quantum types. The complex-amplitude generalization of the framework is therefore not an exotic extension grafted onto a classical framework --- it is the natural next step once the calculus is honestly linear in the first place. \emph{Entangled currency is what linear typed value becomes when amplitudes are complex; classical typed value is the magnitude-squared shadow of an object that already lives in a richer space.}

\section{Reduction to practice}

The position would be vacuous if the substrate did not exist. It does.

\subsection{SPJ combinators as the typed-money primitive}\label{sec:spj}
